...
try {
    start_session();send_private_workspace_headers();if(empty($_SESSION['user'])){http_response_code(401);throw new RuntimeException('Authentication required.');}$user=current_user();$uid=(int)($user['id']??0);$role=(string)($user['role_name']??'');
    ...
    if($_SERVER['REQUEST_METHOD']==='GET'){
        ...
        $raw=(string)file_get_contents('php://input');$payload=json_decode($raw,true,32,JSON_THROW_ON_ERROR);if(!is_array($payload)||!str_starts_with(ltrim($raw),'{'))throw new InvalidArgumentException('JSON object required.');$id=isset($payload['id'])?(int)$payload['id']:null;
