...
try {
    ...
    $uid=(int)$user['id'];$db=push_db();if(!push_employee_active($db,$uid)){http_response_code(403);throw new RuntimeException('Active employee required.');}$method=strtoupper((string)($_SERVER['REQUEST_METHOD']??'GET'));$action=(string)($_GET['action']??'status');$cfg=push_config();
    if($method==='GET'){
        ...
        if($action==='subscribe'){
            ...
            $data=['device'=>push_register($uid,$body['subscription']??[] ,(string)($body['label']??''),(string)($_SERVER['HTTP_USER_AGENT']??''),$db)];
        }elseif($action==='revoke'){$id=$body['id']??null;if(!is_int($id)||$id<1)throw new InvalidArgumentException('Invalid device ID.');$data=['revoked'=>push_revoke($uid,$id,$db)];if($data['revoked']===0){http_response_code(404);throw new RuntimeException('Device not found.');}}
...
}catch(Throwable $e){if(http_response_code()<400)http_response_code($e instanceof InvalidArgumentException||$e instanceof JsonException?422:($e instanceof DomainException?409:500));echo json_encode(['ok'=>false,'error'=>['code'=>'push_request_failed','message'=>http_response_code()>=500?'Push request failed.':$e->getMessage()]]);}
