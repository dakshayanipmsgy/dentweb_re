...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'set_standalone_payment_status') {
        $invoiceId=safe_text($_POST['invoice_id']??'');$paymentInvoice=documents_get_invoice($invoiceId);
        if(!is_array($paymentInvoice))invoice_workspace_redirect('', 'error', 'Invoice not found.');
        $result=documents_standalone_set_manual_payment_status($paymentInvoice,safe_text($_POST['payment_status']??''));
        if(empty($result['ok']))invoice_workspace_redirect($invoiceId,'error',(string)$result['error']);
        $saved=documents_save_invoice($result['invoice']);if(empty($saved['ok']))invoice_workspace_redirect($invoiceId,'error','Unable to save payment status.');
        invoice_sync_sales_record($result['invoice']);invoice_workspace_redirect($invoiceId,'success','Manual payment status saved. No receipt was created.');
    }
    ...
    if ($action === 'save_invoice_draft') {
        ...
        if(documents_invoice_is_standalone($doc)){
            ...
            $doc['tax_type']=in_array((string)($_POST['tax_type']??''),['CGST_SGST','IGST'],true)?(string)$_POST['tax_type']:'CGST_SGST';
            $doc['special_requests_text']=safe_multiline_text((string)($_POST['special_requests_text']??''));
        ...
        $doc['component_details'] = invoice_component_details_sanitize($_POST['component_details'] ?? []);
        $doc['updated_at'] = date('c');
        ...
        $catalogueSaved = invoice_component_catalogue_learn($doc['component_details']);
        if (empty($catalogueSaved['ok'])) {
            documents_log('Invoice component catalogue update failed after saving ' . $invoiceId . ': ' . ($catalogueSaved['error'] ?? 'unknown error'));
        }
...
$componentCatalogue = invoice_component_catalogue_read();
if ($isStandaloneWorkspace) {
    // Load invoice-builder masters only for the direct-invoice editor. In particular, never
    // serialize the complete Customer User records into HTML: production records can contain
    // large handover payloads and exceed the web server's response/resource limits.
...
<header class="card commercial-header"><div><p class="admin-kicker">Commercial workspace</p><h1>Invoices</h1><p>Create direct invoices from the Customer Users list, then review and maintain every invoice through the existing lifecycle.</p></div><nav class="commercial-header__actions"><a class="btn secondary" href="admin-customers.php">Customer Users</a><a class="btn secondary" href="admin-dashboard.php">Dashboard</a><a class="btn secondary" href="admin-documents.php">Document Center</a><a class="btn secondary" href="admin-documents.php?tab=accepted_customers">Accepted Customers</a></nav></header>
...
<div class="form-section-card"><h3>Customer snapshot</h3><p class="muted-helper"><?= documents_invoice_is_standalone($doc) ? 'Linked to the Customer User selected in Admin Customers. Name and mobile are locked to preserve portal ownership.' : 'Customer details copied from the accepted quotation.' ?></p><div class="form-grid"><div><label>Name</label><input type="text" name="customer_name" value="<?= htmlspecialchars((string)($selectedSnap['name'] ?? ''), ENT_QUOTES) ?>" <?= (!$isDraft || documents_invoice_is_standalone($doc)) ? 'readonly' : '' ?>></div><div><label>Mobile</label><input type="text" name="customer_mobile" value="<?= htmlspecialchars((string)($doc['customer_mobile'] ?? $selectedSnap['mobile'] ?? ''), ENT_QUOTES) ?>" <?= (!$isDraft || documents_invoice_is_standalone($doc)) ? 'readonly' : '' ?>></div><div class="full-span"><label>Address</label><textarea name="customer_address" rows="3" <?= $isDraft ? '' : 'readonly' ?>><?= htmlspecialchars((string)($selectedSnap['address'] ?? ''), ENT_QUOTES) ?></textarea></div><?php if(documents_invoice_is_standalone($doc)): foreach([['City','city'],['District','district'],['State','state'],['PIN','pin_code'],['Meter number','meter_number'],['Meter serial number','meter_serial_number'],['Consumer / account number','jbvnl_account_number'],['Application ID','application_id']] as [$label,$field]): ?><div><label><?=htmlspecialchars($label,ENT_QUOTES)?></label><input name="customer_<?=$field?>" value="<?=htmlspecialchars((string)($selectedSnap[$field]??''),ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></div><?php endforeach; endif; ?></div></div>
<div class="form-section-card"><h3>Component / Equipment Details</h3><p class="muted-helper">Identification details only. Component, brand and model suggestions are learned after a successful save; serial numbers remain specific to this invoice.</p><datalist id="invoiceComponentNames"><?php foreach($componentCatalogue['components'] as $catalogueComponent): ?><option value="<?=htmlspecialchars((string)($catalogueComponent['name']??''),ENT_QUOTES)?>"></option><?php endforeach; ?></datalist><div class="responsive-table"><table id="invoiceComponentDetails"><thead><tr><th>Component Name</th><th>Brand</th><th>Model</th><th>Serial No.</th><?php if($isDraft): ?><th></th><?php endif; ?></tr></thead><tbody><?php $componentRows=invoice_component_details_sanitize($doc['component_details']??[]); foreach($componentRows as $i=>$componentRow): ?><tr data-component-row><td><input name="component_details[<?=$i?>][component_name]" class="component-name" list="invoiceComponentNames" value="<?=htmlspecialchars($componentRow['component_name'],ENT_QUOTES)?>" <?=$isDraft?'':'readonly'?>></td><td><input name="component_details[<?=$i?>][brand]" class="component-brand" value="<?=htmlspecialchars($componentRow['brand'],ENT_QUOTES)?>" <?=$isDraft?'':'readonly'?>><datalist class="brand-options"></datalist></td><td><input name="component_details[<?=$i?>][model]" class="component-model" value="<?=htmlspecialchars($componentRow['model'],ENT_QUOTES)?>" <?=$isDraft?'':'readonly'?>><datalist class="model-options"></datalist></td><td><input name="component_details[<?=$i?>][serial_no]" value="<?=htmlspecialchars($componentRow['serial_no'],ENT_QUOTES)?>" <?=$isDraft?'':'readonly'?>></td><?php if($isDraft): ?><td><button type="button" class="btn secondary component-remove-row">Remove</button></td><?php endif; ?></tr><?php endforeach; ?></tbody></table></div><?php if($isDraft): ?><button type="button" class="btn secondary" id="invoiceAddComponent">Add Component</button><?php endif; ?></div>
...
<div class="sticky-action-footer"><a class="btn secondary" href="admin-invoices.php">Back to list</a><a class="btn secondary" href="invoice-view.php?id=<?= urlencode((string)$doc['id']) ?>" target="_blank" rel="noopener">Open/View Invoice</a><?php if ($isDraft): ?><button class="btn" type="submit">Save draft</button><button class="btn commercial-header__primary" type="submit" name="action" value="finalize_invoice">Finalize / Issue invoice (<?= htmlspecialchars($selectedInvoiceDate, ENT_QUOTES) ?>)</button><?php else: ?><span class="muted-helper">Issued/final invoices are read-only in this workspace.</span><?php endif; ?></div></form><?php if(documents_invoice_is_standalone($doc)): $manualPaymentLocked=(int)($selectedPaymentSummary['receipt_count']??0)>0; ?><div class="form-section-card"><h3>Manual payment status</h3><p class="muted-helper">Use this only when no receipt exists. It changes the received/balance display without creating a receipt. Canonical receipts remain authoritative.</p><form method="post"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars(csrf_token(),ENT_QUOTES)?>"><input type="hidden" name="action" value="set_standalone_payment_status"><input type="hidden" name="invoice_id" value="<?=htmlspecialchars((string)$doc['id'],ENT_QUOTES)?>"><label>Payment Status</label><select name="payment_status" <?= $manualPaymentLocked?'disabled':'' ?>><option value="unpaid" <?=($doc['manual_payment_status']??'unpaid')==='unpaid'?'selected':''?>>Unpaid</option><option value="paid" <?=($doc['manual_payment_status']??'')==='paid'?'selected':''?>>Paid</option></select><button class="btn" type="submit" <?= $manualPaymentLocked?'disabled':'' ?>>Save payment status</button><?php if($manualPaymentLocked): ?><p class="muted-helper">Disabled because canonical receipts exist; the reconciled receipt total is authoritative.</p><?php endif; ?></form></div><div class="form-section-card"><h3>Add payment receipt</h3><form method="post"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars(csrf_token(),ENT_QUOTES)?>"><input type="hidden" name="action" value="add_standalone_payment"><input type="hidden" name="invoice_id" value="<?=htmlspecialchars((string)$doc['id'],ENT_QUOTES)?>"><div class="form-grid"><div><label>Amount</label><input type="number" step="0.01" min="0.01" name="amount" required></div><div><label>Date received</label><input type="date" name="date_received" value="<?=date('Y-m-d')?>" required></div><div><label>Payment mode</label><input name="mode"></div><div><label>Transaction / reference</label><input name="transaction_reference"></div><div><label>Receipt number (optional)</label><input name="receipt_number"></div><div class="full-span"><label>Note</label><textarea name="note"></textarea></div></div><button class="btn" type="submit">Add canonical payment receipt</button></form></div><?php endif; ?><div class="form-section-card"><h3>Lifecycle actions</h3><?php if (!$isDraft && documents_invoice_is_finalized($doc)): ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES) ?>"><input type="hidden" name="action" value="start_invoice_revision"><input type="hidden" name="invoice_id" value="<?= htmlspecialchars((string)$doc['id'], ENT_QUOTES) ?>"><label>Revision reason (required)</label><textarea name="revision_reason" rows="2" required></textarea><button class="btn" type="submit">Revise finalized invoice</button></form><?php endif; ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES) ?>"><input type="hidden" name="action" value="cancel_invoice"><input type="hidden" name="invoice_id" value="<?= htmlspecialchars((string)$doc['id'], ENT_QUOTES) ?>"><label>Cancellation reason (required)</label><textarea name="cancel_reason" rows="2" required></textarea><label class="checkbox-field"><input type="checkbox" name="confirm_cancel" value="1" required> I understand receipts are preserved and a replacement invoice is optional/manual.</label><button class="btn warn" type="submit">Cancel invoice</button></form></div><?= document_signature_admin_controls($doc, 'invoice', 'admin-invoices.php?id=' . urlencode((string)$doc['id']), $isDraft) ?></section>
<?php elseif ($id !== ''): ?><section class="card empty-state">Invoice not found.</section><?php endif; ?>
<?php if($isDebug && $doc!==null):?><details class="card advanced-fields" open><summary>Admin-only raw invoice debug record</summary><pre class="debug-record"><?=htmlspecialchars(json_encode($doc,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES)?:'',ENT_QUOTES)?></pre></details><?php endif;?>
<script>
const invoiceVariants=<?=json_encode($invoiceVariantsByComponent,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>, invoiceKits=<?=json_encode(array_column($invoiceKits,null,'id'),JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>, invoiceComponents=<?=json_encode(array_column($invoiceComponents,null,'id'),JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>, invoiceDefaultHsn=<?=json_encode(safe_text((string)($invoiceQuoteDefaults['defaults']['hsn_solar']??'8541'))?:'8541')?>, invoiceRates=<?=json_encode($invoiceQuoteDefaults['rate_chart']??[],JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>;
const componentCatalogue=<?=json_encode($componentCatalogue,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>;let componentRowIndex=document.querySelectorAll('#invoiceComponentDetails tbody tr').length;const catalogueKey=value=>String(value||'').trim().replace(/\s+/g,' ').toLocaleLowerCase();function catalogueComponent(value){return (componentCatalogue.components||[]).find(row=>catalogueKey(row.name)===catalogueKey(value))}function refreshComponentSuggestions(row){const component=catalogueComponent(row.querySelector('.component-name')?.value),brandInput=row.querySelector('.component-brand'),modelInput=row.querySelector('.component-model'),brandList=row.querySelector('.brand-options'),modelList=row.querySelector('.model-options'),suffix='equipment-'+Math.random().toString(36).slice(2);if(brandList&&brandInput){brandList.id='brands-'+suffix;brandInput.setAttribute('list',brandList.id);brandList.innerHTML=(component?.brands||[]).map(x=>`<option value="${String(x.name||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;')}"></option>`).join('')}const brand=(component?.brands||[]).find(x=>catalogueKey(x.name)===catalogueKey(brandInput?.value));if(modelList&&modelInput){modelList.id='models-'+suffix;modelInput.setAttribute('list',modelList.id);modelList.innerHTML=(brand?.models||[]).map(x=>`<option value="${String(x||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;')}"></option>`).join('')}}document.querySelectorAll('[data-component-row]').forEach(refreshComponentSuggestions);document.getElementById('invoiceAddComponent')?.addEventListener('click',()=>{const i=componentRowIndex++,tr=document.createElement('tr');tr.dataset.componentRow='';tr.innerHTML=`<td><input name="component_details[${i}][component_name]" class="component-name" list="invoiceComponentNames"></td><td><input name="component_details[${i}][brand]" class="component-brand"><datalist class="brand-options"></datalist></td><td><input name="component_details[${i}][model]" class="component-model"><datalist class="model-options"></datalist></td><td><input name="component_details[${i}][serial_no]"></td><td><button type="button" class="btn secondary component-remove-row">Remove</button></td>`;document.querySelector('#invoiceComponentDetails tbody')?.append(tr);refreshComponentSuggestions(tr)});document.addEventListener('input',event=>{if(event.target.matches('.component-name,.component-brand'))refreshComponentSuggestions(event.target.closest('[data-component-row]'))});document.addEventListener('click',event=>{if(event.target.matches('.component-remove-row'))event.target.closest('[data-component-row]')?.remove()});
function fillModels(){const t=document.getElementById('invoiceSystemType'),m=document.getElementById('invoiceRateModel');if(!t||!m)return;const current=m.value||<?=json_encode((string)($doc['rate_chart_snapshot']['model_number']??''))?>,rows=invoiceRates[t.value==='hybrid'?'hybrid':'on_grid']||[];m.innerHTML='<option value="">-- select model --</option>'+rows.map(r=>`<option value="${String(r.model_number||'').replace(/"/g,'&quot;')}">${r.model_number||''}</option>`).join('');m.value=current;}const invoiceSystemType=document.getElementById('invoiceSystemType');if(invoiceSystemType)invoiceSystemType.addEventListener('change',fillModels);fillModels();
let itemIndex=document.querySelectorAll('#invoiceMasterItems tbody tr').length;document.getElementById('invoiceAddItem')?.addEventListener('click',()=>{const tr=document.createElement('tr'),i=itemIndex++;tr.innerHTML=`<td><select name="items[${i}][type]" class="invoice-item-type"><option value="kit">Kit</option><option value="component" selected>Component</option></select></td><td><select name="items[${i}][kit_id]" class="invoice-item-kit"><option value="">-- kit --</option><?php foreach($invoiceKits as $k):?><option value="<?=htmlspecialchars((string)$k['id'],ENT_QUOTES)?>"><?=htmlspecialchars((string)$k['name'],ENT_QUOTES)?></option><?php endforeach;?></select></td><td><select name="items[${i}][component_id]" class="invoice-item-component"><option value="">-- component --</option><?php foreach($invoiceComponents as $c):?><option value="<?=htmlspecialchars((string)$c['id'],ENT_QUOTES)?>"><?=htmlspecialchars((string)$c['name'],ENT_QUOTES)?></option><?php endforeach;?></select></td><td><select name="items[${i}][variant_id]" class="invoice-item-variant"><option value="">-- none --</option></select></td><td><input name="items[${i}][hsn]" class="invoice-item-hsn"></td><td><input type="number" min="0" step="0.01" name="items[${i}][quantity]" value="1"></td><td><input type="number" min="0" step="0.01" name="items[${i}][unit_price_incl_gst]"></td><td><textarea name="items[${i}][custom_description]"></textarea></td><td><button type="button" class="btn secondary invoice-remove-item">Remove</button></td>`;document.querySelector('#invoiceMasterItems tbody').append(tr)});document.addEventListener('click',e=>{if(e.target.matches('.invoice-remove-item'))e.target.closest('tr').remove()});function invoiceAutofillHsn(tr){const type=tr.querySelector('.invoice-item-type')?.value||'component',master=type==='kit'?invoiceKits[tr.querySelector('.invoice-item-kit')?.value]:invoiceComponents[tr.querySelector('.invoice-item-component')?.value],variant=(invoiceVariants[tr.querySelector('.invoice-item-component')?.value]||[]).find(x=>x.id===tr.querySelector('.invoice-item-variant')?.value),input=tr.querySelector('.invoice-item-hsn');if(input)input.value=(variant?.hsn_override||master?.hsn||invoiceDefaultHsn||'8541')}document.addEventListener('change',e=>{if(!e.target.matches('.invoice-item-type,.invoice-item-kit,.invoice-item-component,.invoice-item-variant'))return;const tr=e.target.closest('tr');if(e.target.matches('.invoice-item-component')){const v=tr.querySelector('.invoice-item-variant'),rows=invoiceVariants[e.target.value]||[];v.innerHTML='<option value="">-- none --</option>'+rows.map(x=>`<option value="${x.id}">${x.display_name}</option>`).join('')}invoiceAutofillHsn(tr)});
