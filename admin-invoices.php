...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_standalone_invoice') {
        ...
        $newInvoiceId=(string)($created['invoice_id']??'');
        $newInvoice=$newInvoiceId!==''?documents_get_invoice($newInvoiceId):null;
        if(!is_array($newInvoice)) invoice_workspace_redirect('', 'error', 'Standalone invoice draft was created but could not be loaded.');
        $newInvoice['standalone_builder']=['mode'=>'quotation_style','version'=>1,'created_at'=>date('c'),'updated_at'=>date('c')];
        $newInvoice['updated_at']=date('c');
        $savedBuilder=documents_save_invoice($newInvoice);
        if(empty($savedBuilder['ok'])) invoice_workspace_redirect($newInvoiceId,'error','Unable to initialise the quotation-style invoice builder.');
        header('Location: admin-invoice-create.php?id='.urlencode($newInvoiceId));
        exit;
    }
    ...
    if ($action === 'save_invoice_draft') {
        ...
        if(documents_invoice_is_standalone($doc)){
            ...
            if(!standalone_invoice_quote_builder_is_enabled($doc)){
                ...
                $itemResult=documents_standalone_apply_items($doc,(array)($_POST['items']??[]));if(empty($itemResult['ok']))invoice_workspace_redirect($invoiceId,'error',(string)$itemResult['error']);$doc=$itemResult['invoice'];
            }
        }
...
if ($doc !== null && standalone_invoice_quote_builder_is_enabled($doc) && documents_invoice_is_draft($doc) && $_SERVER['REQUEST_METHOD'] === 'GET' && !$isDebug) {
    header('Location: admin-invoice-create.php?id=' . urlencode((string)$doc['id']));
    exit;
}
...
<div class="form-section-card"><h3>Invoice items</h3><div class="responsive-table"><table><thead><tr><th>Item</th><th>Description</th><th>HSN</th><th>Qty</th><th>Unit</th><th>Unit price incl GST</th><th>GST %</th></tr></thead><tbody><?php $editItems=(array)($doc['commercial_items']??[]);for($i=0;$i<max(3,count($editItems)+1);$i++):$item=$editItems[$i]??[];$rate=(float)($item['slabs'][0]['rate_pct']??18);?><tr><td><input name="items[<?=$i?>][name]" value="<?=htmlspecialchars((string)($item['name']??''),ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></td><td><input name="items[<?=$i?>][description]" value="<?=htmlspecialchars((string)($item['description']??''),ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></td><td><input name="items[<?=$i?>][hsn]" value="<?=htmlspecialchars((string)($item['hsn']??''),ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></td><td><input type="number" step="0.001" min="0" name="items[<?=$i?>][quantity]" value="<?=htmlspecialchars((string)($item['quantity']??$item['qty']??''),ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></td><td><input name="items[<?=$i?>][unit]" value="<?=htmlspecialchars((string)($item['unit']??''),ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></td><td><input type="number" step="0.01" min="0" name="items[<?=$i?>][unit_price_incl_gst]" value="<?=htmlspecialchars((string)($item['unit_price_incl_gst']??''),ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></td><td><input type="number" step="0.01" min="0" name="items[<?=$i?>][gst_rate]" value="<?=htmlspecialchars((string)$rate,ENT_QUOTES)?>" <?= $isDraft?'':'readonly' ?>></td></tr><?php endfor;?></tbody></table></div><p class="muted-helper">Totals and GST are calculated in paise from quantity × GST-inclusive unit price.</p></div>
<?php endif; ?>
