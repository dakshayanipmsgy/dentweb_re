...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && safe_text($_GET['action'] ?? '') === 'search_customers') {
    $query = strtolower(safe_text($_GET['q'] ?? ''));
    $digits = preg_replace('/\D+/', '', $query) ?: '';
    $matches = [];
    if (strlen($query) >= 2 || strlen($digits) >= 3) {
        foreach ((new CustomerFsStore())->listActiveCustomers() as $customer) {
            $name = strtolower((string) ($customer['name'] ?? ''));
            $mobile = preg_replace('/\D+/', '', (string) ($customer['mobile'] ?? '')) ?: '';
            if (!str_contains($name, $query) && ($digits === '' || !str_contains($mobile, $digits))) {
                continue;
            }
            $matches[] = array_intersect_key($customer, array_flip(['id','name','mobile','address','city','district','state','pin_code','meter_number','meter_serial_number','jbvnl_account_number','application_id']));
            if (count($matches) >= 20) {
                break;
            }
        }
    ...
    echo json_encode(['customers' => $matches], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}
...
if ($isStandaloneWorkspace) {
    // Customer Users are searched through the bounded JSON endpoint above. Never embed the
    // entire customer filesystem in this page: production stores can exceed PHP/web-server limits.
...
<?php if($isDebug && $doc!==null):?><details class="card advanced-fields" open><summary>Admin-only raw invoice debug record</summary><pre class="debug-record"><?=htmlspecialchars(json_encode($doc,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES)?:'',ENT_QUOTES)?></pre></details><?php endif;?><datalist id="invoiceCustomerOptions"></datalist>
<script>
let invoiceCustomers=[];const invoiceVariants=<?=json_encode($invoiceVariantsByComponent,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>, invoiceRates=<?=json_encode($invoiceQuoteDefaults['rate_chart']??[],JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT)?>;
const search=document.getElementById('invoiceCustomerSearch'),customerOptions=document.getElementById('invoiceCustomerOptions');let customerTimer;if(search)search.addEventListener('input',()=>{clearTimeout(customerTimer);const q=search.value.trim();if(q.length<2)return;customerTimer=setTimeout(async()=>{try{const response=await fetch(`admin-invoices.php?action=search_customers&q=${encodeURIComponent(q)}`,{headers:{Accept:'application/json'}});if(!response.ok)return;const payload=await response.json();invoiceCustomers=Array.isArray(payload.customers)?payload.customers:[];customerOptions.replaceChildren(...invoiceCustomers.flatMap(c=>{const byName=document.createElement('option'),byMobile=document.createElement('option');byName.value=String(c.name||'');byName.label=String(c.mobile||'');byMobile.value=String(c.mobile||'');byMobile.label=String(c.name||'');return [byName,byMobile]}));}catch(error){invoiceCustomers=[];}},250)});if(search)search.addEventListener('change',()=>{const q=search.value.trim().toLowerCase(),c=invoiceCustomers.find(x=>String(x.name||'').toLowerCase()===q||String(x.mobile||'').replace(/\D/g,'')===q.replace(/\D/g,''));if(!c)return;const map={customer_name:'name',customer_mobile:'mobile',customer_address:'address',customer_city:'city',customer_district:'district',customer_state:'state',customer_pin_code:'pin_code',customer_meter_number:'meter_number',customer_meter_serial_number:'meter_serial_number',customer_jbvnl_account_number:'jbvnl_account_number',customer_application_id:'application_id'};Object.entries(map).forEach(([n,k])=>{const e=document.querySelector(`[name="${n}"]`);if(e&&c[k]!=null)e.value=c[k]});});
