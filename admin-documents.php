...
<body class="admin-shell admin-documents">
  <main class="page commercial-shell" data-document-action-endpoint="<?= htmlspecialchars($documentActionEndpoint, ENT_QUOTES) ?>">
    ...
    <nav class="tabs admin-documents__tabs workspace-tabs" data-workspace-tabs="fetch" aria-label="Document Center tabs">
      ...
      <a class="tab" href="admin-discom-requests.php">DISCOM Requests</a>
    ...
    <?php if ($activeTab === 'completed_customers'): ?>
      <section class="panel">
        ...
        <div class="responsive-table operations-table-wrap"><table class="operations-table"><thead><tr><th><span class="visually-hidden">Select</span></th><th>Customer / quotation</th><th>Completion date</th><th>Final amount</th><th>Paid at completion</th><th>Documents</th><th>Review</th><th>Customer User</th><th>Action</th></tr></thead><tbody>
...
<style>
.discom-selection-tray{position:sticky;bottom:12px;z-index:40;display:none;align-items:center;gap:10px;max-width:760px;margin:16px auto;padding:12px 16px;background:#172033;color:#fff;border-radius:12px;box-shadow:0 8px 30px #0005}.discom-selection-tray.is-visible{display:flex}.discom-selection-tray form{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.visually-hidden{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0)}
</style>
<div class="discom-selection-tray" id="discom-selection-tray" role="region" aria-label="DISCOM selection"><strong><span id="discom-selection-count">0</span> selected</strong><form method="post" action="admin-discom-requests.php" id="discom-selection-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><span id="discom-selection-inputs"></span><button class="btn secondary" name="action" value="review">Review selection</button><button class="btn" name="action" value="review">Create DISCOM request</button><button type="button" class="btn secondary" id="discom-selection-clear">Clear selection</button></form></div>
<script>
(function(){var KEY='dentweb.discom.selection.v1',selected={};try{JSON.parse(sessionStorage.getItem(KEY)||'[]').forEach(function(id){selected[id]=true;});}catch(e){}var tray=document.getElementById('discom-selection-tray'),count=document.getElementById('discom-selection-count'),inputs=document.getElementById('discom-selection-inputs');function paint(){document.querySelectorAll('.js-discom-select').forEach(function(c){c.checked=!!selected[c.value];});var ids=Object.keys(selected);count.textContent=String(ids.length);tray.classList.toggle('is-visible',ids.length>0);inputs.innerHTML='';ids.forEach(function(id){var i=document.createElement('input');i.type='hidden';i.name='quotation_ids[]';i.value=id;inputs.appendChild(i);});sessionStorage.setItem(KEY,JSON.stringify(ids));}document.addEventListener('change',function(e){if(!e.target.matches('.js-discom-select'))return;if(e.target.checked)selected[e.target.value]=true;else delete selected[e.target.value];paint();});document.getElementById('discom-selection-clear').addEventListener('click',function(){selected={};paint();});paint();})();
</script>
