      ...
      <section class="panel">
        ...
        <?php elseif ($itemsSubtab === 'inventory'): ?>
          ...
          <table id="inventorySummaryTable"><thead><tr><th>Component</th><th>Type</th><th>Summary</th></tr></thead><tbody>
            <?php foreach ($inventoryComponents as $component): if (!is_array($component) || !empty($component['archived_flag'])) { continue; } $componentId=(string) ($component['id'] ?? ''); $isCuttable=!empty($component['is_cuttable']); $hasVariants=!empty($component['has_variants']); $componentEntry = documents_inventory_component_stock($inventoryStock, $componentId, ''); $variantRows = (array) ($variantsByComponent[$componentId] ?? []); $lots = []; if ($isCuttable && $hasVariants) { foreach ($variantRows as $variantRow) { if (!is_array($variantRow)) { continue; } $variantIdForLots = (string) ($variantRow['id'] ?? ''); if ($variantIdForLots === '') { continue; } $variantEntryForLots = documents_inventory_component_stock($inventoryStock, $componentId, $variantIdForLots); foreach ((array) ($variantEntryForLots['lots'] ?? []) as $variantLot) { if (!is_array($variantLot)) { continue; } $variantLot['variant_id'] = $variantIdForLots; $variantLot['variant_name_snapshot'] = (string) ($variantRow['display_name'] ?? $variantIdForLots); $lots[] = $variantLot; } } } else { $lots = is_array($componentEntry['lots'] ?? null) ? $componentEntry['lots'] : []; } usort($lots, static function ($a, $b): int { return strcmp((string) (($a['received_at'] ?? '')), (string) (($b['received_at'] ?? ''))); }); $totalFt = documents_inventory_compute_on_hand($inventoryStock, $componentId, '', true); $variantTotalQty = documents_inventory_compute_on_hand($inventoryStock, $componentId, '', false); $summaryText = $isCuttable ? (rtrim(rtrim((string) $totalFt, '0'), '.') . ' ft') : ($hasVariants ? (rtrim(rtrim((string) $variantTotalQty, '0'), '.') . ' qty total') : (rtrim(rtrim((string) documents_inventory_compute_on_hand($inventoryStock, $componentId, '', false), '0'), '.') . ' ' . (string) ($component['default_unit'] ?? 'qty'))); $searchHaystack = strtolower(trim((string) ($component['name'] ?? '') . ' ' . implode(' ', array_map(static function ($vr): string { return (string) ($vr['display_name'] ?? ''); }, $variantRows)))); $locationBreakdownText = array_map(static function (array $row) use ($component): string { return documents_inventory_resolve_location_name((string) ($row['location_id'] ?? '')) . ': ' . rtrim(rtrim((string) ((float) ($row['qty'] ?? 0)), '0'), '.') . ' ' . (string) ($component['default_unit'] ?? 'qty'); }, (array) ($componentEntry['location_breakdown'] ?? [])); $componentEditable = !isset($inventoryComponentBlocked[$componentId]); ?>
