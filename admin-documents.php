...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $employeeAllowedActions = ['prepare_handover_whatsapp', 'create_inventory_tx', 'edit_inventory_tx', 'save_inventory_edits', 'import_inventory_stock_in_csv', 'create_receipt', 'save_receipt_draft', 'finalize_receipt', 'create_payment_request', 'cancel_payment_request', 'mark_payment_request_sent', 'archive_payment_request', 'unarchive_payment_request', 'create_payment_link', 'refresh_payment_link'];
    ...
    if (in_array($action, ['create_payment_request', 'cancel_payment_request', 'mark_payment_request_sent', 'archive_payment_request', 'unarchive_payment_request', 'create_payment_link', 'refresh_payment_link'], true)) {
        $tab = 'accepted_customers';
        ...
        if (in_array($action, ['create_payment_link','refresh_payment_link'], true)) {
            $requestId=safe_text($_POST['payment_request_id']??''); $request=documents_get_payment_request($requestId);
            if ($request===null) $redirectDocuments($tab,'error','Payment request not found.',$view!==''?['view'=>$view]:[]);
            $existing=documents_payment_request_link_status($request);
            if ($action==='create_payment_link' && !empty($existing['usable'])) $redirectDocuments($tab,'error','A usable payment link already exists. Use Refresh Link to replace it.',['view'=>(string)$request['quotation_id']]);
            $made=documents_create_payment_request_link($request,$viewer);
            if (!empty($made['ok'])) $_SESSION['payment_request_link_tokens'][$requestId]=['generation'=>(int)$made['generation'],'token'=>(string)$made['token']];
            $redirectDocuments($tab,empty($made['ok'])?'error':'success',empty($made['ok'])?(string)($made['error']??'Unable to create payment link.'):($action==='refresh_payment_link'?'Payment link refreshed; the old link is revoked.':'Payment link created.'),['view'=>(string)$request['quotation_id']]);
        }
...
if ($activeTab === 'accepted_customers' && $packAction === 'print_payment_request' && $printPaymentRequestId !== '') {
    ...
    foreach (['bank_name'=>'Bank','bank_account_name'=>'Account Name','bank_account_no'=>'Account No','bank_ifsc'=>'IFSC','upi_id'=>'UPI'] as $field => $label) { if ((string)($company[$field] ?? '') !== '') { $paymentInstructionLines[] = $label . ': ' . (string)$company[$field]; } }
