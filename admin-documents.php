...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $employeeAllowedActions = ['create_inventory_tx', 'edit_inventory_tx', 'save_inventory_edits', 'import_inventory_stock_in_csv', 'create_receipt', 'save_receipt_draft', 'finalize_receipt', 'create_payment_request', 'cancel_payment_request', 'mark_payment_request_sent', 'archive_payment_request', 'unarchive_payment_request'];
    ...
    if ($action === 'create_project_customer_account') {
        ...
        if ($quote === null || documents_quote_normalize_status((string) ($quote['status'] ?? '')) !== 'accepted') {
            $redirectDocuments($returnTab, 'error', 'An accepted quotation is required.', $qid !== '' ? ['view' => $qid] : []);
        }
        $completed = documents_project_completion_state($quote) === 'completed';
        if (($returnTab === 'completed_customers') !== $completed) {
            $redirectDocuments($completed ? 'completed_customers' : 'accepted_customers', 'error', 'Project state changed; review it before creating the account.', ['view' => $qid]);
        }
        $actor = ['id' => (string) ($user['id'] ?? ''), 'name' => (string) ($user['full_name'] ?? 'Admin'), 'role' => 'admin'];
        $auditActor = audit_current_actor();
        $result = project_create_customer_account(
            $quote, $completed, $actor, new CustomerFsStore(),
            static fn(array $updated): array => documents_save_quote($updated),
            static function (string $event, string $quoteId, array $details) use ($auditActor): void {
                log_audit_event((string) $auditActor['actor_type'], (string) $auditActor['actor_id'], 'quotation', $quoteId, $event, $details);
            }
        );
        if (empty($result['ok'])) $redirectDocuments($returnTab, 'error', (string) ($result['error'] ?? 'Unable to create customer account.'), ['view' => $qid]);
        $customer = (array) ($result['customer'] ?? []);
        $message = !empty($customer['archived']) ? 'Archived customer found and linked; no duplicate was created.' : (!empty($result['created']) ? 'Customer account created and linked.' : 'Existing customer linked; no duplicate was created.');
        $redirectDocuments($returnTab, 'success', $message, ['view' => $qid, 'customer_url' => (string) $result['url']]);
    }
...
$customerSuccessUrl = safe_text($_GET['customer_url'] ?? '');
if (!preg_match('/^admin-users\.php\?tab=customers&view=[6-9][0-9]{9}$/', $customerSuccessUrl)) $customerSuccessUrl = '';
...
$projectCustomerStore = new CustomerFsStore();
$projectCustomerForQuote = static function (array $quote) use ($projectCustomerStore): ?array {
    $payload = project_customer_payload($quote, documents_project_completion_state($quote) === 'completed');
    return (string) ($payload['mobile'] ?? '') !== '' ? $projectCustomerStore->findByMobile((string) $payload['mobile']) : null;
};
$packQuote = null;
$packVersions = [];
...
<body class="admin-shell admin-documents">
  <main class="page commercial-shell" data-document-action-endpoint="<?= htmlspecialchars($documentActionEndpoint, ENT_QUOTES) ?>">
    ...
    <?php if ($message !== '' && ($status === 'success' || $status === 'error')): ?>
      <div class="banner <?= htmlspecialchars($status, ENT_QUOTES) ?>"><?= htmlspecialchars($message, ENT_QUOTES) ?><?php if ($status === 'success' && $customerSuccessUrl !== ''): ?> <a class="btn secondary" href="<?= htmlspecialchars($customerSuccessUrl, ENT_QUOTES) ?>">View customer</a><?php endif; ?></div>
    <?php endif; ?>
    ...
    <?php if ($activeTab === 'accepted_customers'): ?>
      <section class="panel">
        <?php if ($packQuote !== null): ?>
          ...
          <?php $packLinkedCustomer = $projectCustomerForQuote($packQuote); ?>
          <div class="card" style="padding:10px;margin-bottom:10px"><strong>Customer account</strong>
            <?php if (is_array($packLinkedCustomer) && project_customer_account_link($packQuote) !== null): ?>
              <?php if (!empty($packLinkedCustomer['archived'])): ?><span class="pill archived">Archived</span><?php endif; ?>
              <a class="btn secondary" href="admin-users.php?tab=customers&amp;view=<?= rawurlencode((string)$packLinkedCustomer['mobile']) ?>">View customer</a>
            <?php elseif ($isAdmin): ?>
              <?php if (is_array($packLinkedCustomer) && !empty($packLinkedCustomer['archived'])): ?><span class="pill archived">Archived customer found</span><?php endif; ?>
              <form class="inline-form" method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><input type="hidden" name="action" value="create_project_customer_account"><input type="hidden" name="quotation_id" value="<?= htmlspecialchars($packQuoteId,ENT_QUOTES) ?>"><input type="hidden" name="return_tab" value="<?= documents_project_completion_state($packQuote)==='completed'?'completed_customers':'accepted_customers' ?>"><button class="btn" type="submit">Create customer account</button></form>
            <?php endif; ?>
          </div>
