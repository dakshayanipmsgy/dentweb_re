...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['save_component','save_component_edit','toggle_component_archive','save_kit_create','save_kit_edit','toggle_kit_archive','save_tax_profile','save_tax_profile_edit','toggle_tax_profile_archive','save_variant','save_variant_edit','toggle_variant_archive','save_location','save_location_edit','toggle_location_archive','create_inventory_tx','edit_inventory_tx','save_inventory_edits','inventory_verification_update','inventory_verification_bulk_update','import_components_csv','import_variants_csv','import_locations_csv','import_kits_csv','import_inventory_stock_in_csv'], true)) {
        ...
        if ($action === 'toggle_component_archive') {
            ...
            $components = documents_inventory_components(true);
            $isArchiving = $archiveState === 'archive';
            foreach ($components as &$component) {
                if ((string) ($component['id'] ?? '') === $componentId) {
                    $component['archived_flag'] = $isArchiving;
                    $component['updated_at'] = date('c');
                }
            }
            unset($component);
            $result = documents_inventory_save_components($components);
            if (($result['ok'] ?? false) && $isArchiving) {
                $cleanup = documents_inventory_cleanup_kits_for_archived_component($componentId);
                if (!($cleanup['ok'] ?? false)) {
                    $redirectDocuments('items', 'error', (string) ($cleanup['error'] ?? 'Failed to cleanup kits BOM.'), ['items_subtab' => 'components']);
                }
            }
    ...
    <?php if ($activeTab === 'items'): ?>
      <section class="panel">
        ...
        <nav class="tabs" style="margin-top:0.5rem;">
          ...
          <a class="tab <?= $itemsSubtab === 'archived' ? 'active' : '' ?>" href="?<?= htmlspecialchars(http_build_query(['tab' => 'items', 'items_subtab' => 'archived']), ENT_QUOTES) ?>">Archived</a>
        </nav>
        ...
        <?php if ($itemsSubtab === 'components'): ?>
          ...
          <table><thead><tr><th>ID</th><th>Name</th><th>Unit</th><th>Tax Profile</th><th>Variants</th><th>Cuttable</th><th>Status</th><th>Action</th></tr></thead><tbody>
            <?php foreach ($inventoryComponents as $component): if (!is_array($component) || !empty($component['archived_flag'])) { continue; } ?>
              <tr>
        ...
        <?php elseif ($itemsSubtab === 'kits'): ?>
          ...
          <table><thead><tr><th>ID</th><th>Name</th><th>Category</th><th>Tax</th><th>Items</th><th>Status</th><th>Action</th></tr></thead><tbody><?php foreach ($inventoryKits as $kit): if (!is_array($kit) || !empty($kit['archived_flag'])) { continue; } ?><tr><td><?= htmlspecialchars((string) ($kit['id'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($kit['name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($kit['category'] ?? ''), ENT_QUOTES) ?></td><td><?php $kitTax = documents_inventory_get_tax_profile((string) ($kit['tax_profile_id'] ?? '')); ?><?= htmlspecialchars((string) ($kitTax['name'] ?? ''), ENT_QUOTES) ?></td><td><?= count((array) ($kit['items'] ?? [])) ?></td><td>Active</td><td class="row-actions"><?php if ($isAdmin): ?><a class="btn secondary" href="?<?= htmlspecialchars(http_build_query(['tab' => 'items', 'items_subtab' => 'kits', 'sub' => 'kits', 'edit' => (string) ($kit['id'] ?? '')]), ENT_QUOTES) ?>">Edit</a><a class="btn secondary" href="?<?= htmlspecialchars(http_build_query(['tab' => 'items', 'items_subtab' => 'kits', 'sub' => 'kits', 'clone' => (string) ($kit['id'] ?? '')]), ENT_QUOTES) ?>">Clone</a><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_kit_archive" /><input type="hidden" name="kit_id" value="<?= htmlspecialchars((string) ($kit['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="archive" /><button class="btn secondary" type="submit">Archive</button></form><?php endif; ?></td></tr><?php endforeach; ?></tbody></table>
        <?php elseif ($itemsSubtab === 'tax_profiles'): ?>
          ...
          <?php endif; ?>
          <table><thead><tr><th>ID</th><th>Name</th><th>Mode</th><th>Slabs</th><th>Status</th><th>Action</th></tr></thead><tbody><?php foreach ($inventoryTaxProfiles as $profile): if (!is_array($profile) || !empty($profile['archived_flag'])) { continue; } ?><tr><td><?= htmlspecialchars((string) ($profile['id'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($profile['name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($profile['mode'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) json_encode((array) ($profile['slabs'] ?? [])), ENT_QUOTES) ?></td><td>Active</td><td class="row-actions"><?php if ($isAdmin): ?><a class="btn secondary" href="?<?= htmlspecialchars(http_build_query(['tab' => 'items', 'items_subtab' => 'tax_profiles', 'sub' => 'tax_profiles', 'edit' => (string) ($profile['id'] ?? '')]), ENT_QUOTES) ?>">Edit</a><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_tax_profile_archive" /><input type="hidden" name="tax_profile_id" value="<?= htmlspecialchars((string) ($profile['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="archive" /><button class="btn secondary" type="submit">Archive</button></form><?php endif; ?></td></tr><?php endforeach; ?></tbody></table>
        <?php elseif ($itemsSubtab === 'variants'): ?>
          ...
          <?php if ($isAdmin): ?>
            ...
            <form method="post" class="grid" style="margin-bottom:1rem;">
              ...
              <div><label>Component</label><select name="component_id" required><option value="">-- select --</option><?php foreach ($inventoryComponents as $component): if (!is_array($component) || !empty($component['archived_flag'])) { continue; } ?><option value="<?= htmlspecialchars((string) ($component['id'] ?? ''), ENT_QUOTES) ?>" <?= ((string) ($variantForm['component_id'] ?? '') === (string) ($component['id'] ?? '')) ? 'selected' : '' ?>><?= htmlspecialchars((string) ($component['name'] ?? ''), ENT_QUOTES) ?></option><?php endforeach; ?></select></div>
        ...
        <?php elseif ($itemsSubtab === 'locations'): ?>
          ...
          <?php endif; ?>
          <table><thead><tr><th>ID</th><th>Name</th><th>Type</th><th>Notes</th><th>Status</th><th>Action</th></tr></thead><tbody><?php foreach ($inventoryLocations as $location): if (!is_array($location) || !empty($location['archived_flag'])) { continue; } ?><tr><td><?= htmlspecialchars((string) ($location['id'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($location['name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($location['type'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($location['notes'] ?? ''), ENT_QUOTES) ?></td><td>Active</td><td class="row-actions"><?php if ($isAdmin): ?><a class="btn secondary" href="?<?= htmlspecialchars(http_build_query(['tab' => 'items', 'items_subtab' => 'locations', 'edit' => (string) ($location['id'] ?? '')]), ENT_QUOTES) ?>">Edit</a><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_location_archive" /><input type="hidden" name="location_id" value="<?= htmlspecialchars((string) ($location['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="archive" /><button class="btn secondary" type="submit">Archive</button></form><?php else: ?><span class="muted">View only</span><?php endif; ?></td></tr><?php endforeach; ?></tbody></table>
        ...
        <?php elseif ($itemsSubtab === 'archived'): ?>
          ...
          <div class="grid" style="grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;">
            <section>
              <h4>Components</h4>
              <table><thead><tr><th>Name</th><th>ID</th><th>Action</th></tr></thead><tbody>
              <?php foreach ($inventoryComponents as $row): if (!is_array($row) || empty($row['archived_flag'])) { continue; } ?>
                <tr><td><?= htmlspecialchars((string) ($row['name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?></td><td><?php if ($isAdmin): ?><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_component_archive" /><input type="hidden" name="component_id" value="<?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="unarchive" /><button class="btn secondary" type="submit">Unarchive</button></form><?php else: ?><span class="muted">—</span><?php endif; ?></td></tr>
              <?php endforeach; ?></tbody></table>
              <h4>Kits</h4>
              <table><thead><tr><th>Name</th><th>ID</th><th>Action</th></tr></thead><tbody>
              <?php foreach ($inventoryKits as $row): if (!is_array($row) || empty($row['archived_flag'])) { continue; } ?>
                <tr><td><?= htmlspecialchars((string) ($row['name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?></td><td><?php if ($isAdmin): ?><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_kit_archive" /><input type="hidden" name="kit_id" value="<?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="unarchive" /><button class="btn secondary" type="submit">Unarchive</button></form><?php else: ?><span class="muted">—</span><?php endif; ?></td></tr>
              <?php endforeach; ?></tbody></table>
            </section>
            <section>
              <h4>Variants</h4>
              <table><thead><tr><th>Name</th><th>ID</th><th>Action</th></tr></thead><tbody>
              <?php foreach ($inventoryVariants as $row): if (!is_array($row) || empty($row['archived_flag'])) { continue; } ?>
                <tr><td><?= htmlspecialchars((string) ($row['display_name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?></td><td><?php if ($isAdmin): ?><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_variant_archive" /><input type="hidden" name="variant_id" value="<?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="unarchive" /><button class="btn secondary" type="submit">Unarchive</button></form><?php else: ?><span class="muted">—</span><?php endif; ?></td></tr>
              <?php endforeach; ?></tbody></table>
              <h4>Tax Profiles</h4>
              <table><thead><tr><th>Name</th><th>ID</th><th>Action</th></tr></thead><tbody>
              <?php foreach ($inventoryTaxProfiles as $row): if (!is_array($row) || empty($row['archived_flag'])) { continue; } ?>
                <tr><td><?= htmlspecialchars((string) ($row['name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?></td><td><?php if ($isAdmin): ?><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_tax_profile_archive" /><input type="hidden" name="tax_profile_id" value="<?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="unarchive" /><button class="btn secondary" type="submit">Unarchive</button></form><?php else: ?><span class="muted">—</span><?php endif; ?></td></tr>
              <?php endforeach; ?></tbody></table>
              <h4>Locations</h4>
              <table><thead><tr><th>Name</th><th>ID</th><th>Action</th></tr></thead><tbody>
              <?php foreach ($inventoryLocations as $row): if (!is_array($row) || empty($row['archived_flag'])) { continue; } ?>
                <tr><td><?= htmlspecialchars((string) ($row['name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?></td><td><?php if ($isAdmin): ?><form method="post" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="action" value="toggle_location_archive" /><input type="hidden" name="location_id" value="<?= htmlspecialchars((string) ($row['id'] ?? ''), ENT_QUOTES) ?>" /><input type="hidden" name="archive_state" value="unarchive" /><button class="btn secondary" type="submit">Unarchive</button></form><?php else: ?><span class="muted">—</span><?php endif; ?></td></tr>
              <?php endforeach; ?></tbody></table>
            </section>
          </div>
