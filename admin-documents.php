...
// Keep legacy accepted-customer bookmarks compatible while making Receipts a dedicated workspace.
if (!defined('ADMIN_RECEIPTS_WORKSPACE') && ($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'GET' && safe_text($_GET['tab'] ?? '') === 'accepted_customers') {
    $query = $_GET;
    unset($query['tab']);
    header('Location: admin-receipts.php' . ($query !== [] ? '?' . http_build_query($query) : ''));
    exit;
}
...
<body class="admin-shell admin-documents">
  <main class="page commercial-shell">
    ...
    <nav class="commercial-flow-strip" aria-label="Commercial lifecycle"><a href="admin-quotations.php">Quotation</a><span>→</span><a href="admin-agreements.php">Agreement</a><span>→</span><a href="admin-challans.php">Challan</a><span>→</span><a href="admin-invoices.php">Invoice</a><span>→</span><a href="admin-receipts.php">Receipt</a></nav>
