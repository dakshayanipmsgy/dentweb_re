...
<script>
// Copying a public payment-request URL is a local browser action. It neither
// submits a form nor makes an AJAX request, so the workbench cannot refresh.
(function () {
  ...
  document.addEventListener('click', function (event) {
    var button = event.target.closest && event.target.closest('[data-copy-payment-request-url]');
    if (!button) return;
    event.preventDefault();
    var value = button.getAttribute('data-copy-payment-request-url') || '';
    var status = button.parentElement && button.parentElement.querySelector('[data-copy-payment-request-status]');
    var operation = navigator.clipboard && window.isSecureContext
      ? navigator.clipboard.writeText(value)
      : legacyCopy(value);
    Promise.resolve(operation).then(function () {
      if (status) { status.setAttribute('role', 'status'); status.textContent = 'Copied'; }
    }).catch(function () {
      if (status) {
        status.setAttribute('role', 'alert');
        status.textContent = 'Unable to copy link. Please copy the public URL manually.';
      }
    });
  });
}());
