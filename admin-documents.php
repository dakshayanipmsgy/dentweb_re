...
$documentActionEndpoint = strtok((string) ($_SERVER['REQUEST_URI'] ?? 'admin-documents.php'), '?');
if ($documentActionEndpoint === '' || $documentActionEndpoint === false) {
    $documentActionEndpoint = (string) ($_SERVER['SCRIPT_NAME'] ?? 'admin-documents.php');
}
...
$redirectDocuments = static function (string $tab, string $type, string $msg, array $extra = []): void {
    ...
    $url = 'admin-documents.php?' . http_build_query($query);
    $accept = strtolower((string) ($_SERVER['HTTP_ACCEPT'] ?? ''));
    $requestedWith = strtolower((string) ($_SERVER['HTTP_X_REQUESTED_WITH'] ?? ''));
    if ($requestedWith === 'fetch' || str_contains($accept, 'application/json') || (string) ($_POST['response_format'] ?? '') === 'json') {
...
$wantsJsonResponse = static function (): bool {
    $accept = strtolower((string) ($_SERVER['HTTP_ACCEPT'] ?? ''));
    $requestedWith = strtolower((string) ($_SERVER['HTTP_X_REQUESTED_WITH'] ?? ''));
    return $requestedWith === 'fetch' || str_contains($accept, 'application/json') || (string) ($_POST['response_format'] ?? '') === 'json';
};

$sendJsonResponse = static function (array $payload, int $statusCode = 200): void {
...
$documentActionUrl = static function (string $type, string $id): string {
    $encoded = urlencode($id);
    return match ($type) {
        'agreement' => 'agreement-view.php?id=' . $encoded . '&mode=edit',
        'dispatch_advice' => 'admin-dispatch-advices.php?tab=editor&edit=' . $encoded,
        'delivery_challan' => 'challan-view.php?id=' . $encoded,
        'invoice' => 'admin-invoices.php?id=' . $encoded,
        default => '#',
    };
};

$documentActionLabel = static function (string $type): string {
    return match ($type) {
        'agreement' => 'Agreement',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? null)) {
        if ($wantsJson) {
            $sendJsonResponse(['ok' => false, 'error' => 'Security validation failed. Please retry.'], 403);
        }
    ...
    $employeeAllowedActions = ['prepare_handover_whatsapp', 'create_inventory_tx', 'edit_inventory_tx', 'save_inventory_edits', 'import_inventory_stock_in_csv', 'create_receipt', 'save_receipt_draft', 'finalize_receipt', 'create_payment_request', 'create_payment_request_upi_link', 'refresh_payment_request_upi_link', 'cancel_payment_request', 'mark_payment_request_sent', 'archive_payment_request', 'unarchive_payment_request'];
    ...
    if ($action === 'create_legacy_invoice') {
        $project=legacy_billing_get_project(safe_text((string)($_POST['legacy_project_id']??'')));
        if(!is_array($project)) $redirectDocuments('completed_customers','error','Legacy project not found.');
        $created=documents_create_invoice_from_legacy_project($project,['idempotency_key'=>safe_text((string)($_POST['creation_token']??''))]);
        if(empty($created['ok'])) $redirectDocuments('completed_customers','error',(string)$created['error'],['legacy_view'=>$project['id']]);
        header('Location: admin-invoices.php?id='.urlencode((string)$created['invoice_id'])); exit;
    }
    ...
    if (in_array($action, ['prepare_handover_whatsapp', 'mark_handover_sent'], true)) {
        ...
        $model = is_array($quote) ? customer_operations_view_model($quote) : [];
        $customer = is_array($model['customer'] ?? null) ? $model['customer'] : null;
        $handover = is_array($model['handover'] ?? null) ? $model['handover'] : [];
        if ($customer === null || empty($handover['ready'])) $redirectDocuments($returnTab, 'error', 'Generate the linked customer handover first.', ['view'=>$qid]);
        $actor = audit_current_actor();
        $mobile = (string) ($customer['mobile'] ?? '');
        if ($action === 'prepare_handover_whatsapp') {
            log_audit_event($actor['actor_type'], $actor['actor_id'], 'customer', $mobile, 'handover_message_prepared_opened', ['channel'=>'whatsapp','handover_version'=>(int)($handover['version']??0),'handover_hash'=>(string)($handover['hash']??'')]);
            $digits = complaint_normalize_mobile($mobile);
            $scheme = (!empty($_SERVER['HTTPS']) && strtolower((string)$_SERVER['HTTPS']) !== 'off') ? 'https' : 'http';
            $host = (string) ($_SERVER['HTTP_HOST'] ?? 'dakshayani.co.in');
            $url = $scheme.'://'.$host.'/'.ltrim((string)$handover['path'],'/');
            $message = "Dear ".trim((string)($customer['name']??'Customer')).",\n\nYour handover document is ready: ".$url;
            header('Location: https://wa.me/91'.$digits.'?text='.rawurlencode($message)); exit;
        }
        $channel = safe_text($_POST['handover_channel'] ?? 'whatsapp');
        if (!in_array($channel, ['whatsapp','email','in_person'], true)) $channel = 'whatsapp';
        $sent = ['sent_at'=>date('c'),'channel'=>$channel,'actor_type'=>$actor['actor_type'],'actor_id'=>$actor['actor_id'],'handover_version'=>(int)($handover['version']??0),'handover_hash'=>(string)($handover['hash']??'')];
        $saved = (new CustomerFsStore())->updateCustomer($mobile, ['handover_sent'=>$sent]);
        if (empty($saved['success'])) $redirectDocuments($returnTab, 'error', 'Could not record handover send status.', ['view'=>$qid]);
        log_audit_event($actor['actor_type'], $actor['actor_id'], 'customer', $mobile, 'handover_marked_sent', $sent);
        $redirectDocuments($returnTab, 'success', 'Handover explicitly marked sent.', ['view'=>$qid]);
    }
    ...
    if (in_array($action, ['create_payment_request', 'create_payment_request_upi_link', 'refresh_payment_request_upi_link', 'cancel_payment_request', 'mark_payment_request_sent', 'archive_payment_request', 'unarchive_payment_request'], true)) {
        ...
        if (in_array($action, ['create_payment_request_upi_link','refresh_payment_request_upi_link'], true)) {
            $requestId=safe_text($_POST['payment_request_id']??'');
            $result=documents_payment_request_generate_upi_link($requestId);
            if(!empty($result['ok'])) {
                $_SESSION['payment_request_upi_tokens'][$requestId]=(string)$result['token'];
                $message=$action==='refresh_payment_request_upi_link'?'Payment link refreshed; the old link was revoked.':'Payment link created.';
            } else $message=(string)($result['error']??'Unable to create payment link.');
            $request=documents_get_payment_request($requestId);
            $redirectDocuments($tab,!empty($result['ok'])?'success':'error',$message,['view'=>(string)($request['quotation_id']??$view)]);
        }
        ...
        if (in_array($action, ['archive_payment_request', 'unarchive_payment_request'], true)) {
            $requestId = safe_text($_POST['payment_request_id'] ?? '');
            $request = documents_get_payment_request($requestId);
            if ($request === null) { $redirectDocuments($tab, 'error', 'Payment request not found.', $view !== '' ? ['view' => $view] : []); }
            if ($action === 'archive_payment_request') {
                if (strtolower((string) ($request['status'] ?? '')) !== 'cancelled') { $redirectDocuments($tab, 'error', 'Only cancelled payment requests can be archived.', ['view' => (string) ($request['quotation_id'] ?? $view)]); }
                $request = documents_payment_request_archive($request, $viewer);
                $message = 'Cancelled payment request archived.';
            } else {
                $request = documents_payment_request_unarchive($request);
                $message = 'Payment request restored to cancelled list.';
            }
            $saved = documents_save_payment_request($request);
            $redirectDocuments($tab, ($saved['ok'] ?? false) ? 'success' : 'error', ($saved['ok'] ?? false) ? $message : 'Unable to update payment request archive state.', ['view' => (string) ($request['quotation_id'] ?? $view)]);
        }
    ...
    if (in_array($action, ['create_agreement', 'create_receipt', 'create_dispatch_advice', 'create_delivery_challan', 'create_invoice'], true)) {
        ...
        if ($view === '') {
            if ($wantsJson) { $sendJsonResponse(['ok' => false, 'error' => 'Quotation is required.'], 422); }
            $redirectDocuments($tab, 'error', 'Quotation is required.');
        }

        $quote = documents_get_quote($view);
        if ($quote === null) {
            if ($wantsJson) { $sendJsonResponse(['ok' => false, 'error' => 'Quotation not found.'], 404); }
        ...
        $quoteContext = [
            ...
            'quotation_no' => (string) ($quote['quote_no'] ?? ''),
            'customer_name' => safe_text((string) ($snapshot['name'] ?? $quote['customer_name'] ?? '')),
            'customer_mobile' => normalize_customer_mobile((string) ($snapshot['mobile'] ?? $quote['customer_mobile'] ?? '')),
            'capacity_kwp' => safe_text((string) ($quote['capacity_kwp'] ?? '')),
        ];
        $finishDocumentAction = static function (string $docType, string $docId, bool $created, string $message) use ($wantsJson, $sendJsonResponse, $documentActionUrl, $documentActionLabel, $quoteContext, $view): void {
            ...
            $sendJsonResponse([
                ...
                'refresh_url' => 'admin-documents.php?' . http_build_query(['tab' => 'accepted_customers', 'view' => $view]),
            ]);
        };
        $failDocumentAction = static function (string $message, int $statusCode = 422) use ($wantsJson, $sendJsonResponse, $redirectDocuments, $tab, $view): void {
            if ($wantsJson) {
                $sendJsonResponse(['ok' => false, 'error' => $message], $statusCode);
            }
            $redirectDocuments($tab, 'error', $message, ['view' => $view]);
        };
        ...
        if ($action === 'create_agreement') {
            ...
            if ($existingAgreementId !== '') {
                ...
                if ($existingAgreement !== null && !documents_is_archived($existingAgreement)) {
                    $finishDocumentAction('agreement', $existingAgreementId, false, 'Agreement already exists.');
                }
            }
            foreach (documents_list_sales_documents('agreement') as $existingAgreement) {
                if ((string) ($existingAgreement['quotation_id'] ?? '') === (string) ($quote['id'] ?? '') && !documents_is_archived($existingAgreement)) {
                    $finishDocumentAction('agreement', (string) ($existingAgreement['id'] ?? ''), false, 'Agreement already exists.');
                }
            }

            $number = documents_generate_agreement_number(safe_text((string) ($quote['segment'] ?? 'RES')) ?: 'RES');
            if (!$number['ok']) {
                $failDocumentAction((string) ($number['error'] ?? 'Unable to generate agreement number.'));
            }
            ...
            if (!$savedAgreement['ok']) {
                $failDocumentAction('Failed to create agreement draft.');
            }
            ...
            if (!$savedSalesAgreement['ok']) {
                $failDocumentAction('Failed to update agreement workflow.');
            }
            ...
            if (!$savedQuote['ok']) {
                $failDocumentAction('Agreement created, but quotation workflow update failed.');
            }

            $finishDocumentAction('agreement', (string) $agreement['id'], true, 'Agreement created from default template.');
        }

        if ($action === 'create_dispatch_advice') {
            $existingAdvices = array_values(array_filter(documents_dispatch_advices_for_quote((string) ($quote['id'] ?? '')), static fn(array $row): bool => !documents_is_archived($row)));
            if ($existingAdvices !== []) {
                $existingAdvice = $existingAdvices[0];
                $finishDocumentAction('dispatch_advice', (string) ($existingAdvice['id'] ?? ''), false, 'Dispatch Advice already exists.');
            }
            if (!documents_dispatch_quote_eligible($quote)) {
                $failDocumentAction('Only accepted current quotations are eligible for Dispatch Advice creation.');
            }
            ...
            $advice['agreement_id'] = (string) ($quote['workflow']['agreement_id'] ?? $quote['agreement_id'] ?? '');
            $advice['agreement_no'] = (string) ($quote['agreement_no'] ?? '');
            $advice['customer_name'] = (string) ($quoteContext['customer_name'] ?? '');
            ...
            if (empty($savedAdvice['ok'])) {
                $failDocumentAction('Unable to create Dispatch Advice draft.');
            }
            documents_quote_link_workflow_doc($quote, 'dispatch_advice', (string) $advice['id']);
            $quote['updated_at'] = date('c');
            documents_save_quote($quote);
            $finishDocumentAction('dispatch_advice', (string) $advice['id'], true, 'Dispatch Advice created. Review materials before finalizing.');
        }
        ...
        if ($action === 'create_delivery_challan') {
            $existingChallans = documents_challans_for_quote((string) ($quote['id'] ?? ''));
            if ($existingChallans !== []) {
                $finishDocumentAction('delivery_challan', (string) ($existingChallans[0]['id'] ?? ''), false, 'Challan already exists.');
            }
            $challan = documents_challan_defaults();
            $number = documents_generate_challan_number(safe_text((string) ($quote['segment'] ?? 'RES')) ?: 'RES');
            if (!$number['ok']) {
                $failDocumentAction((string) ($number['error'] ?? 'Unable to create challan number.'));
            }
            ...
            if (!$savedChallan['ok']) {
                $failDocumentAction('Unable to create delivery challan draft.');
            }
            ...
            if (!$savedSales['ok']) {
                $failDocumentAction('Delivery challan created, but pack workflow update failed.');
            }

            documents_quote_link_workflow_doc($quote, 'delivery_challan', (string) $challan['id']);
            $quote['updated_at'] = date('c');
            documents_save_quote($quote);
            $finishDocumentAction('delivery_challan', (string) $challan['id'], true, 'Delivery challan draft created.');
        }
        ...
        if ($action === 'create_invoice') {
            ...
            $salesInvoice['invoice_date'] = documents_invoice_authoritative_date($invoiceDoc);
...
foreach ($quotes as $quote) {
    ...
    $acceptedRow=array_merge($base,project_workspace_accepted_financial_fields($presentation),['due_since'=>$dueSince,'due_days'=>$days,'active_requests'=>$activeRequests,'finance_presentation'=>$presentation,'archived'=>$isArchived]);
    $acceptedAllRows[]=$acceptedRow;
    if (!$isArchived) $acceptedActiveRows[]=$acceptedRow;
}
$acceptedKpis=project_workspace_accepted_kpis($acceptedActiveRows);
$legacyProjects=legacy_billing_list_projects();
foreach ($legacyProjects as $legacyProject) {
    $pid=(string)$legacyProject['id']; $snap=(array)($legacyProject['customer_snapshot']??[]); $legacyInvoices=legacy_billing_invoices($pid); $amount=0.0;$paid=0.0;
    foreach($legacyInvoices as $invoice){if(documents_invoice_is_cancelled($invoice))continue;$amount+=documents_invoice_final_total($invoice);$paid+=(float)documents_invoice_payment_summary($invoice)['total_received'];}
    $completedAllRows[]=['quote'=>null,'legacy_project'=>$legacyProject,'source'=>'legacy','id'=>$pid,'customer'=>(string)($snap['name']??''),'mobile'=>normalize_customer_mobile((string)($snap['mobile']??'')),'quotation'=>$pid,'documents'=>['invoice'=>$legacyInvoices!==[]],'documents_ready'=>$legacyInvoices!==[],'link_state'=>'resolved','link_ready'=>true,'attention'=>false,'amount'=>$amount,'paid'=>$paid,'completed_date'=>(string)($legacyProject['completion_date']??substr((string)$legacyProject['created_at'],0,10)),'review_changed'=>false];
}
...
$acceptedFilteredRows=project_workspace_filter($acceptedAllRows,$acceptedState,'accepted');
$acceptedArchiveResultCount=count(array_filter($acceptedFilteredRows,static fn(array $row):bool=>!empty($row['archived'])));
$acceptedPage=project_workspace_paginate($acceptedFilteredRows,$acceptedState); $acceptedRows=$acceptedPage['rows']; $acceptedState['page']=$acceptedPage['page'];
...
$renderCustomerUserLink = static function (array $quote, string $returnTab, bool $isAdmin, bool $detail = false): string {
    $conflict = customer_conflict_detect($quote);
    $legacy = documents_project_customer_user_link($quote);
    $link = $legacy;
    if (in_array((string)$conflict['state'], ['conflict','missing','archived','stale_link'], true)) {
        $link['state']=(string)$conflict['state'];
        $link['label']=$conflict['state']==='stale_link'?'Stale Customer User link':($conflict['state']==='conflict'?'Link conflict':ucfirst((string)$conflict['state']));
        $link['customer']=$conflict['customer']; $link['mobile']=$conflict['mobile'];
    } elseif ($conflict['state']==='resolved') { $link['state']='existing'; $link['label']='Existing customer linked'; $link['customer']=$conflict['customer']; }
    $mobile = (string) ($link['mobile'] ?? '');
    $state = (string) ($link['state'] ?? 'invalid');
    $pillClass = in_array($state,['conflict','invalid','stale_link'],true) ? 'warn' : (($state === 'missing'||$state==='archived') ? 'archived' : '');
    $qid = (string)($quote['id'] ?? '');
    ...
    ?><div class="customer-user-link" style="display:flex;gap:.4rem;align-items:center;flex-wrap:wrap">
      ...
      <?php endif; ?>
      <?php if (in_array($state,['conflict','stale_link'],true) && $customer !== null): ?>
        ...
        <?= $isAdmin ? customer_conflict_render_resolver($quote,$returnTab) : '' ?>
...
if ($activeTab === 'accepted_customers' && $packAction === 'print_payment_request' && $printPaymentRequestId !== '') {
    ...
    $contactBits = array_values(array_filter([(string)($company['phone_primary'] ?? ''), (string)($company['email_primary'] ?? ''), (string)($company['website'] ?? '')], static fn($v): bool => trim((string)$v) !== ''));
    $addressBits = array_values(array_filter([(string)($company['address_line'] ?? ''), trim((string)($company['city'] ?? '') . ' ' . (string)($company['district'] ?? '')), trim((string)($company['state'] ?? '') . ' ' . (string)($company['pin'] ?? ''))], static fn($v): bool => trim((string)$v) !== ''));
    $bankDetails = documents_company_bank_transfer_details($company);
    $paymentInstructionLines = array_map(static fn(array $field): string => $field['label'] . ': ' . $field['value'], $bankDetails['fields']);
    if (trim((string) ($company['upi_id'] ?? '')) !== '') { $paymentInstructionLines[] = 'UPI: ' . trim((string) $company['upi_id']); }
    ?><!doctype html><html><head><meta charset="utf-8"><title>Payment Request <?= htmlspecialchars((string)($request['id'] ?? ''), ENT_QUOTES) ?></title><style>:root{--teal:#0f766e;--ink:#12302b;--muted:#64748b;--line:#dbe7e3;--soft:#f4faf8;--gold:#f59e0b}*{box-sizing:border-box}body{margin:0;background:#eef6f4;color:var(--ink);font:14px/1.55 Arial,Helvetica,sans-serif}.toolbar{max-width:900px;margin:18px auto 0;text-align:right}.btn-print{border:0;border-radius:999px;background:var(--teal);color:#fff;padding:10px 18px;font-weight:700}.sheet{max-width:900px;margin:18px auto 30px;background:#fff;border:1px solid var(--line);box-shadow:0 18px 48px rgba(15,48,43,.14)}.brand{display:grid;grid-template-columns:1fr auto;gap:24px;align-items:start;padding:28px 32px;background:linear-gradient(135deg,#0b4f49,#0f766e);color:#fff}.brand-main{display:flex;gap:16px;align-items:center}.logo-box{display:grid;place-items:center;min-width:92px;min-height:72px;padding:8px;background:#fff;border-radius:14px}.logo-box img{display:block;max-width:150px;max-height:58px;object-fit:contain}.logo-fallback{font-size:28px;font-weight:900;color:var(--teal)}.brand h1{margin:0;font-size:25px;line-height:1.15}.brand p{margin:4px 0 0;color:#dff7f1}.brand-meta{text-align:right;font-size:12px;color:#e6fffb}.content{padding:30px 32px}.doc-title{display:flex;justify-content:space-between;gap:20px;align-items:flex-start;margin-bottom:22px}.doc-title h2{margin:0;color:var(--ink);font-size:34px;letter-spacing:-.03em}.ref-card,.card,.metric{border:1px solid var(--line);border-radius:14px;background:#fff}.ref-card{min-width:240px;padding:13px 16px;background:var(--soft)}.label{display:block;color:var(--muted);font-size:11px;font-weight:800;letter-spacing:.08em;text-transform:uppercase}.value{font-weight:800}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin:16px 0}.card{padding:16px}.summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:18px 0}.metric{padding:14px;background:#f8fbfa}.metric strong{display:block;margin-top:4px;font-size:17px}.metric.requested{grid-column:span 2;background:linear-gradient(135deg,#ecfdf5,#fff7ed);border:2px solid #9bd8ca}.metric.requested strong{font-size:30px;color:var(--teal)}.section-title{margin:24px 0 8px;color:var(--teal);font-size:15px;text-transform:uppercase;letter-spacing:.06em}.note{white-space:pre-wrap}.instructions{display:grid;gap:6px}.closing{margin-top:26px;padding-top:18px;border-top:1px solid var(--line)}.footer{padding:14px 32px;color:var(--muted);font-size:12px;background:#f8fbfa;border-top:1px solid var(--line);display:flex;justify-content:space-between;gap:12px}@media print{body{background:#fff}.toolbar{display:none}.sheet{margin:0;max-width:none;border:0;box-shadow:none}.brand{print-color-adjust:exact;-webkit-print-color-adjust:exact}.card,.metric,.ref-card{break-inside:avoid}}@media(max-width:720px){.brand,.grid,.summary{grid-template-columns:1fr}.doc-title{display:block}.brand-meta{text-align:left}.metric.requested{grid-column:auto}}</style></head><body><div class="toolbar"><button class="btn-print" onclick="window.print()">Print Payment Request</button></div><main class="sheet"><header class="brand"><div class="brand-main"><div class="logo-box"><?php if ($logoPath !== ''): ?><img src="<?= htmlspecialchars($logoPath, ENT_QUOTES) ?>" alt="<?= htmlspecialchars($companyName, ENT_QUOTES) ?> logo"><?php else: ?><span class="logo-fallback">DE</span><?php endif; ?></div><div><h1><?= htmlspecialchars($companyName, ENT_QUOTES) ?></h1><p><?= htmlspecialchars(implode(' • ', $contactBits), ENT_QUOTES) ?></p><p><?= htmlspecialchars(implode(', ', $addressBits), ENT_QUOTES) ?></p></div></div><div class="brand-meta"><strong>Not a receipt</strong><br>Receipts remain the source of truth for payment received.</div></header><section class="content"><div class="doc-title"><div><span class="label">Customer payment document</span><h2>Payment Request</h2></div><div class="ref-card"><span class="label">Request No.</span><div class="value"><?= htmlspecialchars((string)($request['id'] ?? ''), ENT_QUOTES) ?></div><span class="label" style="margin-top:8px">Request Date</span><div><?= htmlspecialchars(substr((string)($request['created_at'] ?? ''),0,10), ENT_QUOTES) ?></div></div></div><div class="grid"><div class="card"><span class="label">Customer Details</span><strong><?= htmlspecialchars((string)($request['customer_name'] ?? ''), ENT_QUOTES) ?></strong><br><?= htmlspecialchars((string)($request['customer_mobile'] ?? ''), ENT_QUOTES) ?></div><div class="card"><span class="label">Project / Quotation Reference</span><strong><?= htmlspecialchars((string)($request['quotation_id'] ?? ''), ENT_QUOTES) ?></strong><br>Status: <?= htmlspecialchars(ucwords(str_replace('_',' ',(string)($request['status'] ?? 'draft'))), ENT_QUOTES) ?></div></div><div class="summary"><div class="metric"><span class="label">Total Project Amount</span><strong><?= htmlspecialchars($inr((float)($request['quotation_amount'] ?? ($summary['quotation_amount'] ?? 0))), ENT_QUOTES) ?></strong></div><div class="metric"><span class="label">Paid So Far</span><strong><?= htmlspecialchars($inr((float)($summary['total_received'] ?? 0)), ENT_QUOTES) ?></strong></div><div class="metric"><span class="label">Outstanding</span><strong><?= htmlspecialchars($inr((float)($summary['outstanding'] ?? 0)), ENT_QUOTES) ?></strong></div><div class="metric requested"><span class="label">Requested Amount</span><strong><?= htmlspecialchars($inr((float)($request['amount_requested'] ?? 0)), ENT_QUOTES) ?></strong></div><div class="metric"><span class="label">Due Date</span><strong><?= htmlspecialchars((string)(($request['due_date'] ?? '') ?: 'As discussed'), ENT_QUOTES) ?></strong></div></div><h3 class="section-title">Reason / Payment Purpose</h3><div class="card"><?= htmlspecialchars(documents_payment_request_reason_label($request), ENT_QUOTES) ?></div><?php if (trim((string)($request['message'] ?? '')) !== ''): ?><h3 class="section-title">Message</h3><div class="card note"><?= nl2br(htmlspecialchars((string)($request['message'] ?? ''), ENT_QUOTES)) ?></div><?php endif; ?><h3 class="section-title">Payment Instructions</h3><div class="card instructions"><?php if ($paymentInstructionLines !== []): foreach ($paymentInstructionLines as $line): ?><div><?= htmlspecialchars($line, ENT_QUOTES) ?></div><?php endforeach; else: ?>Please contact Dakshayani Enterprises for payment instructions.<?php endif; ?></div><div class="closing"><p>Kindly make the payment so that we can proceed with the next stage of work.</p><p><strong>Regards,<br><?= htmlspecialchars($companyName, ENT_QUOTES) ?></strong></p></div></section><footer class="footer"><span>Generated: <?= htmlspecialchars(date('Y-m-d H:i'), ENT_QUOTES) ?></span><span>Payment Request <?= htmlspecialchars((string)($request['id'] ?? ''), ENT_QUOTES) ?></span></footer></main>
</body></html><?php
    exit;
}
...
<head>
  ...
  <style>
    ...
    .customer-conflict-resolver { width:min(920px,calc(100vw - 2rem)); max-height:90vh; overflow:auto; border:0; border-radius:16px; padding:1.25rem; box-shadow:0 24px 70px rgba(15,23,42,.3); color:#0f172a; }
    .customer-conflict-resolver[open] { position:fixed; inset:5vh auto auto 50%; transform:translateX(-50%); z-index:10000; }
    .resolver-open-error { display:inline-block; color:#991b1b; font-weight:700; margin:.35rem; }
...
<body class="admin-shell admin-documents">
  <main class="page commercial-shell" data-document-action-endpoint="<?= htmlspecialchars($documentActionEndpoint, ENT_QUOTES) ?>">
      ...
      <section class="panel">
        <?php if ($packQuote !== null): ?>
          <?php
            ...
            $packDispatchAdvices = documents_dispatch_advices_for_quote($packQuoteId);
            if (!$includeArchivedPack) {
                $packDispatchAdvices = array_values(array_filter($packDispatchAdvices, static fn(array $row): bool => !documents_is_archived($row)));
            }
          ...
          <div class="document-pack-flow" aria-label="Document pack lifecycle">
            ...
            <div class="<?= $packDispatchAdvices !== [] ? 'done' : '' ?>"><strong>3</strong><span>Dispatch Advice<small><?= $packDispatchAdvices !== [] ? 'Planned' : 'Plan dispatch' ?></small></span></div>
            <div class="<?= $packChallans !== [] ? 'done' : '' ?>"><strong>4</strong><span>Challan<small><?= $packChallans !== [] ? 'Dispatch recorded' : 'Create for delivery' ?></small></span></div>
          ...
          <p>
            ...
            <?php if ($isAdmin && documents_quote_normalize_status((string) ($packQuote['status'] ?? 'draft')) === 'accepted'): ?>
              <form class="inline-form" method="post" data-accepted-ajax-form="1" style="display:inline-flex; margin-left:0.45rem;">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
          ...
          <?php if ($packAgreements === []): ?>
            <form method="post" data-accepted-ajax-form="1" class="inline-form">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
                ...
                <?php foreach ($packAgreements as $row): ?>
                  <tr>
                    ...
                    <td class="row-actions">
                      ...
                      <?php if ($isAdmin): ?>
                        <form class="inline-form" method="post" data-accepted-ajax-form="1">
                          <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
          ...
          <?php endif; ?>
          <?php
            $paymentRequestGroups = ['active'=>'Active / Pending', 'paid'=>'Paid', 'cancelled'=>'Cancelled', 'archived'=>'Archived'];
            $groupedPaymentRequests = array_fill_keys(array_keys($paymentRequestGroups), []);
            foreach ($packPaymentRequests as $groupReq) {
                $groupReq = documents_payment_request_refresh_from_receipts($groupReq);
                $groupStatus = strtolower((string)($groupReq['status'] ?? 'draft'));
                $groupKey = !empty($groupReq['archived_flag']) ? 'archived' : ($groupStatus === 'paid' ? 'paid' : ($groupStatus === 'cancelled' ? 'cancelled' : 'active'));
                $groupedPaymentRequests[$groupKey][] = $groupReq;
            }
          ?>
          <?php foreach ($paymentRequestGroups as $groupKey => $groupTitle): ?>
            ...
            <?php foreach ($groupedPaymentRequests[$groupKey] as $payReq): $payStatus=strtolower((string)($payReq['status'] ?? 'draft')); $isPayArchived=!empty($payReq['archived_flag']); $payToken=(string)($_SESSION['payment_request_upi_tokens'][(string)($payReq['id']??'')]??''); $payLinkActive=documents_payment_request_link_available($payReq) && $payToken!=='' && documents_payment_request_find_by_token($payToken)!==null; $payPublicUrl=$payLinkActive?documents_payment_request_public_url($payToken):''; $payMsg=documents_build_payment_request_message($payReq,$packPaymentSummary,$payPublicUrl); $isOverdue=(!$isPayArchived && !in_array($payStatus,['paid','cancelled'],true) && (string)($payReq['due_date'] ?? '') !== '' && (string)$payReq['due_date'] < date('Y-m-d')); ?>
              <tr><td><?= htmlspecialchars((string)($payReq['id'] ?? ''), ENT_QUOTES) ?><br><span class="muted"><?= htmlspecialchars((string)($payReq['created_at'] ?? ''), ENT_QUOTES) ?></span><?= empty($payReq['visibility_to_customer']) ? '<br><span class="pill warn">Internal Only</span>' : '' ?></td><td><?= htmlspecialchars($inr((float)($payReq['amount_requested'] ?? 0)), ENT_QUOTES) ?><br><span class="muted">Due <?= htmlspecialchars((string)($payReq['due_date'] ?? '—'), ENT_QUOTES) ?></span></td><td><?= htmlspecialchars(documents_payment_request_reason_label($payReq), ENT_QUOTES) ?></td><td><span class="pill <?= $isPayArchived ? 'archived' : ($isOverdue ? 'warn' : '') ?>"><?= htmlspecialchars($isPayArchived ? 'Archived' : ($isOverdue ? 'Overdue' : ucwords(str_replace('_',' ',(string)($payReq['status'] ?? 'draft')))), ENT_QUOTES) ?></span><br><span class="muted"><?= htmlspecialchars((string)($payReq['sent_via'] ?? ''), ENT_QUOTES) ?> <?= htmlspecialchars((string)($payReq['sent_at'] ?? ''), ENT_QUOTES) ?></span></td><td><?= htmlspecialchars((string)($payReq['follow_up_date'] ?? ''), ENT_QUOTES) ?><br><span class="muted"><?= nl2br(htmlspecialchars((string)($payReq['internal_notes'] ?? ''), ENT_QUOTES)) ?></span></td><td class="row-actions"><a class="btn secondary" target="_blank" href="?<?= htmlspecialchars(http_build_query(['tab'=>'accepted_customers','action'=>'print_payment_request','payment_request_id'=>(string)($payReq['id'] ?? '')]), ENT_QUOTES) ?>">Print</a><?php if (!$isPayArchived && !in_array($payStatus,['cancelled','paid'],true)): ?><?php if($payLinkActive): ?><button class="btn secondary" type="button" onclick="navigator.clipboard.writeText(<?= htmlspecialchars(json_encode($payPublicUrl),ENT_QUOTES) ?>)">Copy payment link</button><a class="btn secondary" target="_blank" rel="noopener noreferrer" href="<?= htmlspecialchars($payPublicUrl,ENT_QUOTES) ?>">Open payment page</a><form method="post" data-accepted-ajax-form="1" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><input type="hidden" name="action" value="refresh_payment_request_upi_link"><input type="hidden" name="payment_request_id" value="<?= htmlspecialchars((string)($payReq['id']??''),ENT_QUOTES) ?>"><input type="hidden" name="quotation_id" value="<?= htmlspecialchars($packQuoteId,ENT_QUOTES) ?>"><button class="btn secondary" type="submit">Refresh link</button></form><span class="muted">Expires <?= htmlspecialchars((string)($payReq['upi_link']['expires_at']??''),ENT_QUOTES) ?> · UPI uses <?= (int)($payReq['upi_link']['launch_count']??0) ?>/2</span><a class="btn secondary" target="_blank" rel="noopener noreferrer" href="<?= htmlspecialchars(documents_payment_request_whatsapp_url($payReq,$payMsg), ENT_QUOTES) ?>">WhatsApp</a><a class="btn secondary" href="<?= htmlspecialchars(documents_payment_request_mailto($payReq,$payMsg), ENT_QUOTES) ?>">Email</a><?php else: ?><form method="post" data-accepted-ajax-form="1" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><input type="hidden" name="action" value="<?= (string)($payReq['upi_link']['token_hash']??'')===''?'create_payment_request_upi_link':'refresh_payment_request_upi_link' ?>"><input type="hidden" name="payment_request_id" value="<?= htmlspecialchars((string)($payReq['id']??''),ENT_QUOTES) ?>"><input type="hidden" name="quotation_id" value="<?= htmlspecialchars($packQuoteId,ENT_QUOTES) ?>"><button class="btn secondary" type="submit"><?= (string)($payReq['upi_link']['token_hash']??'')===''?'Create payment link':'Refresh link' ?></button></form><?php if((string)($payReq['upi_link']['token_hash']??'')!==''): ?><span class="muted">Expires <?= htmlspecialchars((string)($payReq['upi_link']['expires_at']??''),ENT_QUOTES) ?> · UPI uses <?= (int)($payReq['upi_link']['launch_count']??0) ?>/2 · Refresh required before sharing</span><?php endif; ?><?php endif; ?><form method="post" data-accepted-ajax-form="1" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><input type="hidden" name="action" value="mark_payment_request_sent"><input type="hidden" name="payment_request_id" value="<?= htmlspecialchars((string)($payReq['id']??''),ENT_QUOTES) ?>"><input type="hidden" name="quotation_id" value="<?= htmlspecialchars($packQuoteId,ENT_QUOTES) ?>"><input type="hidden" name="sent_via" value="generated"><button class="btn secondary" type="submit">Mark Sent</button></form><form method="post" data-accepted-ajax-form="1" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><input type="hidden" name="action" value="cancel_payment_request"><input type="hidden" name="payment_request_id" value="<?= htmlspecialchars((string)($payReq['id']??''),ENT_QUOTES) ?>"><input type="hidden" name="quotation_id" value="<?= htmlspecialchars($packQuoteId,ENT_QUOTES) ?>"><button class="btn warn" type="submit">Cancel</button></form><?php elseif (!$isPayArchived && $payStatus === 'cancelled'): ?><form method="post" data-accepted-ajax-form="1" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><input type="hidden" name="action" value="archive_payment_request"><input type="hidden" name="payment_request_id" value="<?= htmlspecialchars((string)($payReq['id']??''),ENT_QUOTES) ?>"><input type="hidden" name="quotation_id" value="<?= htmlspecialchars($packQuoteId,ENT_QUOTES) ?>"><button class="btn warn" type="submit">Archive</button></form><?php elseif ($isPayArchived): ?><form method="post" data-accepted-ajax-form="1" class="inline-form"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token']??''),ENT_QUOTES) ?>"><input type="hidden" name="action" value="unarchive_payment_request"><input type="hidden" name="payment_request_id" value="<?= htmlspecialchars((string)($payReq['id']??''),ENT_QUOTES) ?>"><input type="hidden" name="quotation_id" value="<?= htmlspecialchars($packQuoteId,ENT_QUOTES) ?>"><button class="btn secondary" type="submit">Unarchive</button></form><?php endif; ?></td></tr>
          ...
          <table>
            ...
            <tbody>
              <?php foreach ($packReceipts as $row): ?>
                <?php $rowHealth=documents_receipt_allocation_health($row); $rowAllocated=(int)$rowHealth['valid_paise']; $rowUnallocated=(int)$rowHealth['unallocated_paise']+(int)$rowHealth['stale_paise']+(int)$rowHealth['missing_paise']; ?>
                <tr>
                  ...
                  <td><?= htmlspecialchars((string) ($row['status'] ?? 'draft'), ENT_QUOTES) ?><?php if(documents_receipt_is_finalized_active($row)): ?><br>Valid allocated: <?= htmlspecialchars($inr(documents_invoice_paise_to_money($rowAllocated)),ENT_QUOTES) ?> · Health: <?= htmlspecialchars(ucwords(str_replace('_',' ',(string)$rowHealth['health'])),ENT_QUOTES) ?><?php if($rowUnallocated>0): ?><br><strong>Unallocated/stale project payment: <?= htmlspecialchars($inr(documents_invoice_paise_to_money($rowUnallocated)),ENT_QUOTES) ?></strong><?php if($isAdmin): $rowRepairPlan=documents_payment_allocation_repair_plan($packQuoteId); ?><br><?php if(!empty($rowRepairPlan['can_repair'])):?><a class="btn" href="admin-payment-allocation.php?quotation_id=<?=urlencode($packQuoteId)?>">Review payment allocation</a><?php else:?><small>Automatic repair unavailable: <?=htmlspecialchars(implode(' ',array_values($rowRepairPlan['blocked'])),ENT_QUOTES)?> Use manual allocation where offered.</small><?php endif;?><?php endif;?><?php endif; ?><?php endif; ?></td>
                  <td class="row-actions">
        ...
        <?php else: ?>
          ...
          <p class="result-count" role="status"><strong><?= number_format($acceptedPage['total']) ?></strong> filtered result<?= $acceptedPage['total']===1?'':'s' ?>; <strong><?= number_format($acceptedArchiveResultCount) ?></strong> archived. Active cards always summarize <?= number_format($acceptedKpis['count']) ?> non-archived projects.</p>
    ...
    <?php if ($activeTab === 'completed_customers'): ?>
      <section class="panel">
        ...
        <?php $legacyView=legacy_billing_get_project(safe_text((string)($_GET['legacy_view']??''))); if(is_array($legacyView)): $legacyViewInvoices=legacy_billing_invoices((string)$legacyView['id']);$legacySnap=(array)$legacyView['customer_snapshot']; ?>
        <div class="card"><div class="commercial-toolbar"><div><h3>Legacy Project <?=htmlspecialchars((string)$legacyView['id'],ENT_QUOTES)?></h3><p class="muted-helper">Direct Customer · no quotation or fabricated historical documents</p></div><a class="btn secondary" href="admin-customers.php?view=<?=urlencode((string)($legacySnap['mobile']??''))?>">Open Customer</a></div><p><strong><?=htmlspecialchars((string)($legacySnap['name']??''),ENT_QUOTES)?></strong> · <?=htmlspecialchars((string)($legacySnap['mobile']??''),ENT_QUOTES)?> · Capacity <?=htmlspecialchars((string)($legacyView['capacity_kwp']??'—'),ENT_QUOTES)?> kWp</p><p class="muted-helper">Installation/completion: <?=htmlspecialchars((string)($legacyView['completion_date']??'—'),ENT_QUOTES)?> · Enabled <?=htmlspecialchars((string)($legacyView['created_at']??''),ENT_QUOTES)?> by <?=htmlspecialchars((string)($legacyView['created_by']['name']??'Admin'),ENT_QUOTES)?></p><form method="post"><input type="hidden" name="csrf_token" value="<?=htmlspecialchars((string)$_SESSION['csrf_token'],ENT_QUOTES)?>"><input type="hidden" name="action" value="create_legacy_invoice"><input type="hidden" name="legacy_project_id" value="<?=htmlspecialchars((string)$legacyView['id'],ENT_QUOTES)?>"><input type="hidden" name="creation_token" value="<?=htmlspecialchars(bin2hex(random_bytes(16)),ENT_QUOTES)?>"><button class="btn" type="submit">Create Invoice</button></form><div class="responsive-table"><table><thead><tr><th>Invoice</th><th>Status</th><th>Amount</th><th>Actions</th></tr></thead><tbody><?php foreach($legacyViewInvoices as $invoice):?><tr><td><?=htmlspecialchars((string)($invoice['invoice_no']??$invoice['id']),ENT_QUOTES)?></td><td><?=htmlspecialchars(documents_invoice_status_label((string)($invoice['status']??'')),ENT_QUOTES)?></td><td><?=htmlspecialchars($inr(documents_invoice_final_total($invoice)),ENT_QUOTES)?></td><td><a class="btn" href="admin-invoices.php?id=<?=urlencode((string)$invoice['id'])?>">Open/Edit</a> <a class="btn secondary" href="invoice-view.php?id=<?=urlencode((string)$invoice['id'])?>" target="_blank">View/Print</a></td></tr><?php endforeach;if($legacyViewInvoices===[]):?><tr><td colspan="4">No invoices yet.</td></tr><?php endif;?></tbody></table></div></div>
        <?php endif; ?>
      ...
      <section class="panel">
        ...
        <?php if ($itemsSubtab === 'components'): ?>
          ...
          <?php if ($isAdmin): ?>
            ...
            <form method="post" data-accepted-ajax-form="1" class="grid" style="margin-bottom:1rem;">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
        ...
        <?php elseif ($itemsSubtab === 'kits'): ?>
          ...
          <?php if ($isAdmin): ?>
            ...
            <form method="post" data-accepted-ajax-form="1" style="margin-bottom:1rem;">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
...
<?php elseif ($itemsSubtab === 'csv_import'): ?>
          ...
          <div class="grid" style="margin-bottom:1rem;">
            <?php if ($isAdmin): ?>
              <form method="post" data-accepted-ajax-form="1" enctype="multipart/form-data">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
        ...
        <?php elseif ($itemsSubtab === 'inventory'): ?>
          ...
          <div class="grid" style="margin-bottom:1rem;">
            <form method="post" data-accepted-ajax-form="1" data-inventory-form="1" data-tx-type="IN">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
            ...
            <form method="post" data-accepted-ajax-form="1" data-inventory-form="1" data-tx-type="OUT">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
              <input type="hidden" name="action" value="create_inventory_tx" />
            ...
            <form method="post" data-accepted-ajax-form="1" data-inventory-form="1" data-tx-type="MOVE">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
              <input type="hidden" name="action" value="create_inventory_tx" />
            ...
            <?php if ($isAdmin): ?>
            <form method="post" data-accepted-ajax-form="1" data-inventory-form="1" data-tx-type="ADJUST">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
              <input type="hidden" name="action" value="create_inventory_tx" />
    ...
    <?php if ($activeTab === 'items' && $itemsSubtab === 'inventory_verification'): ?>
      <section class="panel">
        ...
        <?php else: ?>
          ...
          <form method="post" data-accepted-ajax-form="1" class="panel" style="margin-bottom:1rem;">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
          ...
          <?php if ($selectedVerificationTxnId !== '' && isset($inventoryVerificationByTxnId[$selectedVerificationTxnId])): $selectedTx = null; foreach ($inventoryTransactions as $txRow) { if ((string) ($txRow['id'] ?? '') === $selectedVerificationTxnId) { $selectedTx = $txRow; break; } } $selectedVerification = array_merge(documents_inventory_verification_defaults(), (array) $inventoryVerificationByTxnId[$selectedVerificationTxnId]); ?>
            <?php if (is_array($selectedTx)): $selectedComponentName = (string) (($componentMap[(string) ($selectedTx['component_id'] ?? '')]['name'] ?? ($selectedTx['component_id'] ?? ''))); ?>
              <div class="panel">
                ...
                <form method="post" data-accepted-ajax-form="1" style="display:grid;gap:0.75rem;max-width:760px;">
                  <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
    ...
    <?php if ($activeTab === 'numbering'): ?>
      <section class="panel">
        ...
        <h3>Add new rule</h3>
        <form method="post" data-accepted-ajax-form="1" class="grid">
          <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
              ...
              <tr>
                ...
                <td>
                  <details>
                    ...
                    <form method="post" data-accepted-ajax-form="1" enctype="multipart/form-data" class="grid">
                      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
                    ...
                    <form method="post" data-accepted-ajax-form="1" style="margin-top:0.5rem;">
                      <input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>" />
                      <input type="hidden" name="template_id" value="<?= htmlspecialchars((string) ($template['id'] ?? ''), ENT_QUOTES) ?>" />
...
<script>
(function(){
  var dialog=document.getElementById('document-action-dialog'); if(!dialog || !window.fetch) return;
  var title=document.getElementById('document-action-title'), subtitle=document.getElementById('document-action-subtitle'), context=document.getElementById('document-action-context'), status=document.getElementById('document-action-status'), openLink=document.getElementById('document-action-open');
  function esc(v){return String(v||'').replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c];});}
  document.querySelectorAll('[data-document-action-close]').forEach(function(btn){btn.addEventListener('click',function(){dialog.close();});});
  document.addEventListener('submit',function(ev){
    var form=ev.target.closest('form[data-document-action]'); if(!form) return; ev.preventDefault();
    var label=form.getAttribute('data-document-label')||'Document action';
    title.textContent=label; subtitle.textContent='Accepted customer document center';
    context.innerHTML='<strong>Quotation:</strong> '+esc(form.getAttribute('data-quote-no'))+'<br><strong>Customer:</strong> '+esc(form.getAttribute('data-customer-name'))+'<br><strong>Mobile:</strong> '+esc(form.getAttribute('data-customer-mobile'));
    ...
    var root=document.querySelector('[data-document-action-endpoint]');
    ...
    fetch(target,{method:'POST',body:new FormData(form),headers:{'Accept':'application/json','X-Requested-With':'fetch'},credentials:'same-origin'})
      ...
      .then(function(data){var doc=data.document||{}, openUrl=(doc.view_url||data.view_url||doc.url||data.open_url||''); status.className='document-action-modal__status is-success'; status.textContent=data.message || ((data.created?'Created ':'Opened ')+(doc.label||'document')+'.'); if(openUrl){openLink.href=openUrl; openLink.textContent=(doc.type==='invoice'?'Open/View Invoice':'Open/View '+(doc.label||'Document')); openLink.hidden=false;} if(data.refresh_url && window.fetch){fetch(data.refresh_url,{credentials:'same-origin',headers:{'X-Requested-With':'fetch'}}).then(function(r){return r.text();}).then(function(html){var docHtml=new DOMParser().parseFromString(html,'text/html'), fresh=docHtml.querySelector('[data-accepted-customer-workbench]'), old=document.querySelector('[data-accepted-customer-workbench]'); if(fresh&&old) old.replaceWith(fresh);});}})
...
(function(){
  ...
  function currentWorkbench(){ return document.querySelector('[data-accepted-customer-workbench]'); }
  function message(root, text, ok){
    var box = root && root.querySelector('[data-accepted-workbench-message]');
    if (!box) return;
    box.textContent = text || '';
    box.className = 'accepted-workbench-message ' + (ok ? 'is-success' : 'is-error');
  }
  function addJsonFlag(formData){ if (!formData.has('response_format')) formData.append('response_format','json'); return formData; }
  function replaceFromUrl(url, notice, ok){
    var oldRoot = currentWorkbench(); if (!oldRoot) return Promise.resolve();
    var y = window.scrollY;
    return fetch(url, {credentials:'same-origin', headers:{'X-Requested-With':'fetch'}})
      .then(function(resp){ return resp.text(); })
      .then(function(html){
        var doc = new DOMParser().parseFromString(html, 'text/html');
        var fresh = doc.querySelector('[data-accepted-customer-workbench]');
        if (!fresh) throw new Error('Updated workbench was not found in the server response.');
  ...
  document.addEventListener('submit', function(ev){
    ...
    fetch(form.getAttribute('action') || window.location.pathname, {method:'POST', body:data, credentials:'same-origin', headers:{'Accept':'application/json','X-Requested-With':'fetch'}})
      .then(function(resp){ return resp.text().then(function(text){ var json; try{ json=JSON.parse(text); }catch(e){ throw new Error('Server returned a non-JSON response.'); } if(!resp.ok || !json.ok){ throw new Error(json.error || json.message || 'Action failed.'); } return json; }); })
      .then(function(json){ return replaceFromUrl(json.refresh_url || window.location.href, json.message || 'Updated.', true); })
      .catch(function(err){ message(currentWorkbench(), err.message || 'Action failed.', false); })
      .finally(function(){ if (btn) { btn.disabled = false; btn.textContent = oldText; } });
  });
  document.addEventListener('click', function(ev){
    var link = ev.target.closest && ev.target.closest('[data-accepted-customer-workbench] a[href]');
    if (!link || link.target || link.origin !== window.location.origin || link.pathname !== window.location.pathname) return;
    var url = new URL(link.href);
    if (url.searchParams.get('tab') !== 'accepted_customers' || !url.searchParams.get('view')) return;
    ev.preventDefault();
    var root = currentWorkbench();
    message(root, 'Loading…', true);
    replaceFromUrl(url.toString(), 'Updated.', true).catch(function(err){ message(currentWorkbench(), err.message || 'Unable to update workbench.', false); });
  });
  document.addEventListener('change', function(ev){
    var input = ev.target.closest && ev.target.closest('[data-accepted-customer-workbench] input[name="include_archived_pack"]');
    if (!input || !input.form) return;
    ev.preventDefault();
    var data = new FormData(input.form), params = new URLSearchParams();
    data.forEach(function(v,k){ if (v !== '') params.append(k,v); });
    if (!input.checked) params.delete('include_archived_pack');
    var url = window.location.pathname + '?' + params.toString();
    message(currentWorkbench(), 'Loading archive view…', true);
...
(function () {
  document.addEventListener('click', function (event) {
    var button = event.target.closest && event.target.closest('[data-open-customer-conflict]');
    if (!button) return;
    var error = button.nextElementSibling;
    try {
      var resolver = document.getElementById(button.getAttribute('data-open-customer-conflict'));
      if (!resolver) throw new Error('The conflict resolver could not be found. Reload and try again.');
      if (typeof resolver.showModal === 'function') resolver.showModal();
      else { resolver.setAttribute('open', 'open'); resolver.setAttribute('role', 'dialog'); resolver.setAttribute('aria-modal', 'true'); }
      if (error) { error.hidden = true; error.textContent = ''; }
    } catch (problem) {
      if (error) { error.hidden = false; error.textContent = problem && problem.message ? problem.message : 'The conflict resolver could not be opened.'; }
    }
  });
  document.addEventListener('click', function (event) {
    var close = event.target.closest && event.target.closest('.customer-conflict-resolver .resolver-close button');
    if (!close) return;
    var resolver = close.closest('dialog');
    if (resolver && typeof resolver.close !== 'function') { event.preventDefault(); resolver.removeAttribute('open'); }
  });
}());
</script>
