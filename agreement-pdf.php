...
try {
    ...
    $row = documents_get_agreement((string) $row['id']) ?? $row;
    ...
    header('Content-Disposition: attachment; filename="' . safe_filename((string) (($row['agreement_no'] ?: $row['id']) . '_agreement')) . '.pdf"');
    readfile($pdfPath);
    exit;
} catch (Throwable $exception) {
    documents_log('Agreement PDF failed for ' . (string) $row['id'] . ': ' . $exception->getMessage());
