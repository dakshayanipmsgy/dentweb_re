...
<?php $theme = documents_get_effective_doc_theme((string) ($quote['template_set_id'] ?? '')); $assumptions = array_merge(documents_quote_assumptions_defaults(), is_array($quote['assumptions'] ?? null) ? $quote['assumptions'] : []); $filled=0; foreach($assumptions as $v){ if(trim((string)$v)!==''){$filled++;}} ?>
<div class="card"><h3>Theme Preview</h3><p>Primary <span style="display:inline-block;width:14px;height:14px;background:<?= htmlspecialchars((string)$theme['primary_color'], ENT_QUOTES) ?>"></span> Secondary <span style="display:inline-block;width:14px;height:14px;background:<?= htmlspecialchars((string)$theme['secondary_color'], ENT_QUOTES) ?>"></span> Accent <span style="display:inline-block;width:14px;height:14px;background:<?= htmlspecialchars((string)$theme['accent_color'], ENT_QUOTES) ?>"></span></p><p>Font scale: <?= htmlspecialchars((string)$theme['font_scale'], ENT_QUOTES) ?> | Background: <?= !empty($theme['enable_background'])?'On':'Off' ?></p><?php if($viewerType==='admin'): ?><a class="btn secondary" href="admin-templates.php?template_id=<?= urlencode((string)$quote['template_set_id']) ?>">Open Theme Settings</a><?php endif; ?></div>
<div class="card"><h3>Assumptions</h3><p>Filled fields: <?= (int)$filled ?> / <?= count($assumptions) ?></p><?php if($filled===0): ?><p>Add assumptions to see chart.</p><?php else: ?><table><tbody><?php foreach($assumptions as $k=>$v): if(trim((string)$v)==='') continue; ?><tr><th><?= htmlspecialchars(str_replace('_',' ',ucfirst((string)$k)), ENT_QUOTES) ?></th><td><?= htmlspecialchars((string)$v, ENT_QUOTES) ?></td></tr><?php endforeach; ?></tbody></table><?php endif; ?></div>
