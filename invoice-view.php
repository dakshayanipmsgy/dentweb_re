...
$invoiceTaxSummary = $invoice !== null ? documents_invoice_tax_summary($invoice) : ['taxable' => 0, 'gst' => 0, 'gross' => 0];
$paymentSummary = $invoice !== null ? documents_invoice_payment_summary($invoice) : ['invoice_total'=>0,'total_received'=>0,'outstanding'=>0,'overpayment'=>0,'payment_status'=>'unpaid','receipt_count'=>0,'receipts'=>[],'last_payment_at'=>''];
...
$solarSizeValue = static function ($raw): float {
...
<style>
:root{--ink:#0f172a;--muted:#64748b;--line:#dbe4ef;--soft:#f8fafc;--panel:#fff;--brand:#0f766e;--brand-dark:#115e59;--accent:#0ea5e9}*{box-sizing:border-box}body{margin:0;background:linear-gradient(180deg,#eef7f6 0,#f8fafc 260px);color:var(--ink);font:14px/1.55 "Inter","Segoe UI",Roboto,Arial,sans-serif}.invoice-page{max-width:1040px;margin:28px auto;padding:0 18px}.invoice-sheet{background:var(--panel);border:1px solid var(--line);border-radius:24px;box-shadow:0 24px 70px rgba(15,23,42,.10);overflow:hidden}.invoice-inner{padding:34px}.invoice-top{display:flex;justify-content:space-between;gap:28px;align-items:flex-start;padding:32px 34px;background:linear-gradient(135deg,#0f766e,#0f172a);color:#fff}.brand-block{display:flex;gap:16px;align-items:flex-start;max-width:650px}.brand-logo{width:88px;height:88px;object-fit:contain;background:#fff;border-radius:16px;padding:8px}.brand-name{font-size:24px;font-weight:850;margin:0 0 6px;letter-spacing:-.02em}.brand-block p,.invoice-title p{margin:3px 0;color:rgba(255,255,255,.84)}.invoice-title{text-align:right;min-width:210px}.invoice-title h1{font-size:36px;line-height:1;margin:0 0 12px;letter-spacing:-.04em}.invoice-pill{display:inline-flex;padding:5px 12px;border-radius:999px;background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.24);font-size:12px;text-transform:uppercase;letter-spacing:.08em}.meta-grid,.party-grid,.summary-grid,.detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px;margin-top:22px}.info-card{border:1px solid var(--line);border-radius:18px;padding:18px;background:linear-gradient(180deg,#fff,#fbfdff)}.info-card.accent{border-color:#b7e4dc;background:linear-gradient(180deg,#f0fdfa,#fff)}.info-card h2,.section-label{font-size:12px;margin:0 0 13px;text-transform:uppercase;color:var(--brand-dark);letter-spacing:.11em;font-weight:800}.detail-row{display:flex;justify-content:space-between;gap:16px;margin:8px 0;align-items:flex-start}.detail-row span:first-child{color:var(--muted)}.detail-row strong{text-align:right;font-weight:750}.solar-size-card{display:block}.solar-total{border-radius:18px;background:#0f766e;color:#fff;padding:18px}.solar-total span{display:block;font-size:12px;letter-spacing:.1em;text-transform:uppercase;color:rgba(255,255,255,.78)}.solar-total strong{display:block;font-size:32px;letter-spacing:-.04em;line-height:1.1}.solar-total small{font-size:15px;font-weight:650}.solar-breakdown{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}.solar-sub{display:inline-flex;gap:7px;padding:7px 10px;border:1px solid rgba(255,255,255,.25);border-radius:999px;background:rgba(255,255,255,.12);color:#fff;font-size:12px}.solar-sub b{font-size:12px}.table-panel{margin-top:22px;border:1px solid var(--line);border-radius:18px;overflow:hidden;background:#fff}.table-title{display:flex;justify-content:space-between;gap:12px;padding:15px 18px;border-bottom:1px solid var(--line);background:#fbfdff}.table-title h2{margin:0;font-size:16px}.invoice-table{width:100%;border-collapse:collapse}.invoice-table th,.invoice-table td{border-bottom:1px solid #edf2f7;padding:11px 12px;text-align:left;vertical-align:top}.invoice-table tr:last-child td{border-bottom:0}.invoice-table th{background:#f8fafc;color:#475569;font-size:11px;text-transform:uppercase;letter-spacing:.07em;font-weight:800}.invoice-table tbody tr:nth-child(even){background:#fcfdff}.num{text-align:right!important;white-space:nowrap}.notes{white-space:pre-wrap;color:#475569;font-size:13px}.summary-grid .info-card{padding:20px}.summary-grid .detail-row:last-child{padding-top:10px;border-top:1px solid var(--line);font-size:15px}.page-actions{display:flex;justify-content:space-between;gap:12px;margin-bottom:16px}.not-found{background:#fff;border:1px solid #fecaca;border-radius:16px;padding:28px;text-align:center}.print-signature{margin-top:46px;display:flex;justify-content:flex-end}.signature-box{text-align:center;min-width:250px;border-top:1px solid #0f172a;padding-top:10px;color:#334155}@media print{body{background:#fff;font-size:12px}.page-actions{display:none}.invoice-page{margin:0;max-width:none;padding:0}.invoice-sheet{box-shadow:none;border:0;border-radius:0}.invoice-top{background:#0f766e!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.invoice-inner{padding:18px}.info-card,.table-panel{break-inside:avoid}.invoice-table th,.solar-total{ -webkit-print-color-adjust:exact;print-color-adjust:exact}}@media(max-width:760px){.invoice-top,.meta-grid,.party-grid,.summary-grid,.detail-grid,.solar-size-card{grid-template-columns:1fr;display:grid}.invoice-title{text-align:left}.invoice-table{font-size:13px}.invoice-inner,.invoice-top{padding:22px}.solar-total strong{font-size:28px}}
</style>
...
<main class="invoice-page">
  ...
  <section class="invoice-sheet" aria-label="Invoice document">
    <header class="invoice-top">
      ...
      <div class="invoice-title"><h1>Tax Invoice</h1><p><strong><?= $esc($invoice['invoice_no'] ?: $invoice['id']) ?></strong></p><p><span class="invoice-pill"><?= $esc(documents_invoice_status_label((string)($invoice['status'] ?? 'draft'))) ?></span></p><p><span class="invoice-pill"><?= $esc(documents_invoice_payment_status_label($paymentSummary['payment_status'])) ?></span></p></div>
    </header>
    ...
    <section class="info-card accent" style="margin-top:22px"><h2>Solar Size Bifurcation</h2><div class="solar-size-card"><div class="solar-total"><span>Total Solar Size</span><strong><?= $displaySolarKwp > 0 ? $esc(number_format($displaySolarKwp, 2, '.', '')) : '—' ?> <small>kWp</small></strong><?php if ($hasSolarSizeBreakup): ?><div class="solar-breakdown"><?php if ($dcrSolarKwp > 0): ?><div class="solar-sub"><span>DCR Solar Size</span><b><?= $esc(number_format($dcrSolarKwp, 2, '.', '')) ?> kWp</b></div><?php endif; ?><?php if ($nonDcrSolarKwp > 0): ?><div class="solar-sub"><span>Non-DCR Solar Size</span><b><?= $esc(number_format($nonDcrSolarKwp, 2, '.', '')) ?> kWp</b></div><?php endif; ?></div><?php endif; ?></div><?php if (!$hasSolarSizeBreakup): ?><div class="notes" style="margin-top:10px">DCR / Non-DCR breakup was not stored for this invoice. The total solar size is shown from the invoice capacity.</div><?php endif; ?></div></section>
    ...
    <section class="summary-grid"><div class="info-card accent"><h2>Payment reconciliation</h2><?php if(($paymentSummary['allocation_attention_count']??0)>0): ?><div class="notes">Unallocated/stale project payment exists on <?= (int)$paymentSummary['allocation_attention_count'] ?> receipt(s). Administrator review is required and these amounts are not counted on this invoice.</div><?php endif; ?><div class="detail-row"><span>Payment status</span><strong><?= $esc(documents_invoice_payment_status_label($paymentSummary['payment_status'])) ?></strong></div><div class="detail-row"><span>Invoice total</span><strong><?= $money($paymentSummary['invoice_total']) ?></strong></div><div class="detail-row"><span>Amount received</span><strong><?= $money($paymentSummary['total_received']) ?></strong></div><div class="detail-row"><span>Balance payable</span><strong><?= $money($paymentSummary['outstanding']) ?></strong></div><?php if ((float)$paymentSummary['overpayment'] > 0): ?><div class="detail-row"><span>Customer credit</span><strong><?= $money($paymentSummary['overpayment']) ?></strong></div><?php endif; ?><div class="detail-row"><span>Receipt count</span><strong><?= (int)$paymentSummary['receipt_count'] ?></strong></div><?php if (($paymentSummary['receipts'] ?? []) !== []): ?><div class="notes">Receipts: <?php foreach($paymentSummary['receipts'] as $pr): ?><?= $esc(($pr['receipt_number'] ?? $pr['id']) . ' (' . $money($pr['amount_rs']) . ')') ?> <?php endforeach; ?></div><?php endif; ?></div></section><section class="summary-grid"><div class="info-card"><h2>Pricing Summary</h2><?php foreach ($pricingRows as $row): ?><div class="detail-row"><span><?= $esc($row['label']) ?><?php if (($row['note'] ?? '') !== ''): ?><br><small><?= $esc($row['note']) ?></small><?php endif; ?></span><strong><?= $money($row['amount']) ?></strong></div><?php endforeach; ?></div><div class="info-card"><h2>Tax Summary</h2><div class="detail-row"><span>Basic Value</span><strong><?= $money($invoiceTaxSummary['taxable']) ?></strong></div><div class="detail-row"><span>Total GST</span><strong><?= $money($invoiceTaxSummary['gst']) ?></strong></div><div class="detail-row"><span>Total incl GST</span><strong><?= $money($invoiceTaxSummary['gross']) ?></strong></div></div></section>

    <section class="table-panel"><div class="table-title"><h2>Tax Breakup</h2><span class="section-label">GST</span></div><table class="invoice-table"><thead><tr><th>Sr No</th><th>Item</th><th>HSN</th><th class="num">Taxable value</th><th>GST Rate</th><th class="num">CGST</th><th class="num">SGST</th><th class="num">IGST</th><th class="num">GST Amount</th><th class="num">Gross incl GST</th></tr></thead><tbody>
