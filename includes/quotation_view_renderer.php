...
function documents_quote_resolve_finance_scenarios_for_render(array $quote, array $calc, array $snapshot): array
{
    ...
    foreach ($order as $scenarioKey => $scenarioLabel) {
        ...
        if ($isSelfFunded) {
            ...
            $remainingSubsidyAfterMargin = max(0, $scenarioSubsidy - $marginMoney);
            $effectivePrincipal = $isSubsidyNotToLoan
                ? max(0, $toFloat($row['effective_loan_principal_rs'] ?? ($loanAmount - $remainingSubsidyAfterMargin)))
        ...
        if (is_array($row['cumulative_series'] ?? null) && $row['cumulative_series'] !== []) {
            ...
            for ($year = 0; $year <= 25; $year++) {
                ...
                $value = $startValue + ($emiMonths * $emi) + ($months * $residualBillScenario);
        ...
        $resolvedScenarios[$scenarioKey] = [
            ...
            'remaining_subsidy_after_reducing_initial_investment_rs' => $remainingSubsidyAfterMargin,
...
orderedApplicableScenarios.forEach((scenarioKey)=>{
  ...
  const data=Array.isArray(row.cumulative_series)&&row.cumulative_series.length===years.length
    ...
    : years.map((year)=>{
      ...
      return start+(emiMonths*Math.max(0,num(row.emi_rs)))+(months*Math.max(0,num(row.residual_bill_rs)));
    });
