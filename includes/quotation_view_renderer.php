...
function quotation_lines_to_array($raw): array
{
    if (is_array($raw)) {
        return array_values(array_filter(array_map(static fn($v): string => trim((string) $v), $raw), static fn(string $v): bool => $v !== ''));
    }
    return array_values(array_filter(array_map(static function ($line): string {
        return trim(preg_replace('/^([\-*•]|\d+[.)])\s+/u', '', (string) $line) ?? (string) $line);
    }, preg_split('/\r\n|\r|\n/', (string) $raw) ?: []), static fn(string $v): bool => $v !== ''));
}

function quotation_section_enabled(array $sections, string $key): bool
{
    return !empty($sections[$key]['show']);
}

function quotation_default_sections_for_render(): array
{
    if (function_exists('documents_detailed_proposal_default_sections')) {
        return documents_detailed_proposal_default_sections();
    }
    return [
        'display_style' => 'compact',
        'cover' => ['show'=>true,'title'=>'Detailed Techno-Commercial Proposal','subtitle'=>'Rooftop Solar PV System by Dakshayani Enterprises','intro'=>'A clear techno-commercial proposal covering design, commercial offer, scope, safety, warranty and next steps.','prepared_by'=>'Dakshayani Enterprises Solar Team','validity_wording'=>'Subject to site feasibility, approvals, taxes and policy updates.'],
        'project_snapshot' => ['show'=>true,'title'=>'Project Snapshot','intro'=>'Key proposal inputs at one place for quick review.'],
        'system_configuration' => ['show'=>true,'title'=>'System Configuration','intro'=>'Major components proposed for this project.','on_grid_points'=>'Solar modules\nGrid-tied inverter\nMounting structure\nACDB / DCDB\nEarthing and lightning protection\nCables and connectors\nMonitoring / RMS where available\nNet metering support','hybrid_points'=>'Solar modules\nHybrid inverter\nBattery bank\nMounting structure\nACDB / DCDB\nEarthing and lightning protection\nCables and connectors\nHybrid monitoring / RMS where available'],
        'system_working' => ['show'=>true,'on_grid_title'=>'How the On-Grid Solar System Works','on_grid_html'=>'<ul><li>Solar panels generate electricity.</li><li>The inverter converts DC to AC.</li><li>Your premises consume solar first and export excess through net metering where approved.</li><li>A standard on-grid system shuts down during grid failure unless backup is included.</li></ul>','hybrid_title'=>'How the Hybrid Solar System Works','hybrid_html'=>'<ul><li>Solar powers loads and can charge batteries.</li><li>The hybrid inverter manages solar, grid and battery.</li><li>Backup depends on selected inverter, battery and connected load.</li></ul>','diagram_url'=>''],
        'scope_matrix' => ['show'=>true,'title'=>'EPC Scope Matrix','intro'=>'Responsibility split for transparent execution.','rows'=>[]],
        'technical_assumptions' => ['show'=>true,'title'=>'Technical Assumptions & Site Readiness','intro'=>'Final execution depends on practical site conditions.','points'=>'Shadow-free roof area is required.\nFinal layout may change after site survey.\nCustomer must provide safe access.\nDISCOM approvals depend on authority timelines.'],
        'exclusions' => ['show'=>true,'title'=>'Exclusions / Not Included Unless Mentioned','intro'=>'Excluded unless specifically included.','points'=>'Major civil work\nWaterproofing\nRoof strengthening\nInternet/router\nExtra cable beyond included length\nStatutory charges unless included'],
        'safety_compliance' => ['show'=>true,'title'=>'Safety & Compliance','intro'=>'Planned with standard solar safety practices.','rows'=>[]],
        'commercial_offer' => ['show'=>true,'title'=>'Commercial Offer','notes'=>'Values are based on the selected primary finance scenario.'],
        'payment_terms' => ['show'=>true,'title'=>'Payment Terms','fallback_html'=>'<ul><li>Payment terms will follow the selected template / annexure.</li></ul>'],
        'warranty_om' => ['show'=>true,'title'=>'Warranty, O&M & Service Support','intro'=>'Warranty support follows OEM and service terms.','points'=>'Module manufacturer warranty.\nInverter manufacturer warranty.\nBattery warranty if included.\nWorkmanship/service support.'],
        'project_timeline' => ['show'=>true,'title'=>'Project Execution Timeline','intro'=>'Indicative milestones after approval.','milestones'=>'Document collection\nSite survey / design confirmation\nMaterial preparation\nDelivery\nInstallation\nTesting and commissioning\nNet meter / subsidy support'],
        'next_steps' => ['show'=>true,'title'=>'Next Steps','points'=>'Approve quotation.\nShare electricity bill.\nShare KYC/bank documents where required.\nComplete site survey and payment milestones.'],
        'bank_details' => ['show'=>true,'title'=>'Bank / Payment Details','intro'=>'Use official details below.','qr_image_url'=>''],
        'acceptance' => ['show'=>true,'title'=>'Acceptance / Signature','wording'=>'I/We accept this quotation and authorize Dakshayani Enterprises to proceed as per agreed scope and terms.'],
    ];
}
...
function quotation_render(array $quote, array $quoteDefaults, array $company, bool $showAdmin = false, string $shareUrl = '', string $viewerType = 'admin', string $viewerId = ''): void
{
    ...
    $proposalSections = array_replace_recursive(quotation_default_sections_for_render(), is_array($quoteDefaults['global']['quotation_sections'] ?? null) ? (array) $quoteDefaults['global']['quotation_sections'] : []);
    $proposalStyle = safe_text((string) ($quote['proposal_display_style'] ?? ''));
    if ($proposalStyle === '') {
        $proposalStyle = (string) ($proposalSections['display_style'] ?? 'compact');
    }
    $isDetailedProposal = $proposalStyle === 'detailed';
    $systemTypeNormalized = strtolower((string) ($quote['system_type'] ?? ''));
    $isHybridProposal = $systemTypeNormalized === 'hybrid';
    $hybridInverterKva = (string) ($quote['rate_chart_snapshot']['hybrid_inverter_kva'] ?? '');
    $hybridPhase = (string) ($quote['rate_chart_snapshot']['hybrid_phase'] ?? '');
    $hybridBatteryCount = (string) ($quote['rate_chart_snapshot']['hybrid_battery_count'] ?? '');
    $expectedMonthlyGeneration = $annualGeneration > 0 && (float) ($quote['capacity_kwp'] ?? 0) > 0 ? ($annualGeneration * (float) ($quote['capacity_kwp'] ?? 0) / 12) : 0;
    $expectedAnnualGeneration = $annualGeneration > 0 && (float) ($quote['capacity_kwp'] ?? 0) > 0 ? ($annualGeneration * (float) ($quote['capacity_kwp'] ?? 0)) : 0;
    $proposalRef = trim((string) ($quote['quote_no'] ?? ''));
    if ($proposalRef !== '' && (int) ($quote['revision'] ?? 0) > 0) {
        $proposalRef .= '-R' . (int) ($quote['revision'] ?? 0);
    }
    $snapshotFields = [
        ['Customer name', (string) ($quote['customer_name'] ?? '')],
        ['City / location', trim((string) ($quote['city'] ?? '') . ((trim((string) ($quote['state'] ?? '')) !== '') ? ', ' . (string) ($quote['state'] ?? '') : ''))],
        ['Site address', (string) ($quote['site_address'] ?? '')],
        ['Consumer account no.', (string) ($quote['consumer_account_no'] ?? '')],
        ['System type', (string) ($quote['system_type'] ?? '')],
        ['Solar capacity', trim((string) ($quote['capacity_kwp'] ?? '')) !== '' ? (string) ($quote['capacity_kwp'] ?? '') . ' kWp' : ''],
        ['Hybrid inverter', $isHybridProposal && $hybridInverterKva !== '' ? $hybridInverterKva . ' kVA' : ''],
        ['Hybrid phase', $isHybridProposal && $hybridPhase !== '' ? $hybridPhase . ' Phase' : ''],
        ['Battery count', $isHybridProposal && $hybridBatteryCount !== '' ? $hybridBatteryCount : ''],
        ['Estimated monthly bill before solar', $monthlyBillBeforeDisplay],
        ['Residual bill', 'Calculated below by scenario'],
        ['Subsidy', quotation_format_inr_indian((float) ($calc['subsidy_expected_rs'] ?? 0), $showDecimals)],
        ['Selected primary scenario price', quotation_format_inr_indian($pricingSummarySystemPrice, $showDecimals)],
        ['Expected monthly generation', $expectedMonthlyGeneration > 0 ? number_format($expectedMonthlyGeneration, 0) . ' kWh' : ''],
        ['Expected annual generation', $expectedAnnualGeneration > 0 ? number_format($expectedAnnualGeneration, 0) . ' kWh' : ''],
    ];
    $snapshotFields = array_values(array_filter($snapshotFields, static fn(array $row): bool => trim((string) ($row[1] ?? '')) !== '' && trim((string) ($row[1] ?? '')) !== '—'));
    $commercialRows = [
        ['System description', (string) ($quote['project_summary_line'] ?: (($quote['system_type'] ?? 'Solar') . ' rooftop solar PV system'))],
        ['Capacity', trim((string) ($quote['capacity_kwp'] ?? '')) !== '' ? (string) ($quote['capacity_kwp'] ?? '') . ' kWp' : ''],
        ['System type', (string) ($quote['system_type'] ?? '')],
        ['Selected primary scenario price', quotation_format_inr_indian($pricingSummarySystemPrice, $showDecimals)],
        ['Subsidy', quotation_format_inr_indian((float) ($calc['subsidy_expected_rs'] ?? 0), $showDecimals)],
        ['Transportation', quotation_format_inr_indian((float) ($calc['transportation_rs'] ?? 0), $showDecimals)],
        ['Discount', $discountApplicable ? quotation_format_inr_indian($discountRsDisplay, $showDecimals) : ''],
        ['Gross payable / final payable', quotation_format_inr_indian((float) ($calc['grand_total'] ?? $pricingSummarySystemPrice), $showDecimals)],
        ['GST note', 'Amounts are GST-inclusive unless specifically mentioned otherwise.'],
        ['Validity', $validUntilDisplay],
    ];
    $commercialRows = array_values(array_filter($commercialRows, static fn(array $row): bool => trim((string) ($row[1] ?? '')) !== '' && trim((string) ($row[1] ?? '')) !== '—'));
    $configPoints = quotation_lines_to_array($proposalSections['system_configuration'][$isHybridProposal ? 'hybrid_points' : 'on_grid_points'] ?? '');
    ...
    $configPoints = array_values(array_unique(array_slice($configPoints, 0, 18)));
    $scopeRows = is_array($proposalSections['scope_matrix']['rows'] ?? null) ? array_values((array) $proposalSections['scope_matrix']['rows']) : [];
    $scopeRows = array_values(array_filter($scopeRows, static function ($row) use ($isHybridProposal): bool {
        if (!is_array($row)) { return false; }
        $types = $row['system_types'] ?? ['all'];
        if (!is_array($types)) { $types = [(string) $types]; }
        $types = array_map(static fn($v): string => strtolower((string) $v), $types);
        return in_array('all', $types, true) || in_array($isHybridProposal ? 'hybrid' : 'on_grid', $types, true) || in_array($isHybridProposal ? 'hybrid' : 'ongrid', $types, true);
    }));
    $safetyRows = is_array($proposalSections['safety_compliance']['rows'] ?? null) ? array_values((array) $proposalSections['safety_compliance']['rows']) : [];
    $paymentTermsHtml = trim((string) ($ann['payment_terms'] ?? '')) ?: trim((string) ($proposalSections['payment_terms']['fallback_html'] ?? ''));
    $subsidyHtml = trim((string) ($ann['pm_subsidy_info'] ?? ''));
    $bankQrImage = trim((string) ($proposalSections['bank_details']['qr_image_url'] ?? ''));
?>
<!doctype html>
...
h1{font-size:var(--h1-size)}h2{font-size:var(--h2-size)}h3{font-size:var(--h3-size)}.wrap{max-width:1100px;margin:0 auto;padding:12px}.card{background:var(--card-bg);border:1px solid var(--border-color);border-radius:14px;padding:14px;margin-bottom:12px;box-shadow:var(--shadow-preset)}.h{font-weight:700}.sec{border-bottom:2px solid var(--color-primary);padding-bottom:5px;margin-bottom:8px}.grid2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.bank-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.grid4{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.metric{background:var(--page-bg);border:1px solid var(--border-color);border-radius:10px;padding:8px}.tax-summary-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-bottom:10px}.tax-rate-line{display:block}.annexure-stack{display:grid;gap:16px}.hero{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.hero .metric b{display:block;font-size:1.2rem;margin-top:4px}.save-line{margin-top:10px;padding:10px 12px;border:1px solid #86efac;background:#f0fdf4;color:#166534;border-radius:10px;font-weight:700}.chip{background:#ccfbf1;color:#134e4a;border-radius:99px;padding:5px 10px;display:inline-block;margin:3px 6px 0 0}.quote-status-row{display:flex;justify-content:flex-end;align-items:center;padding-top:10px;padding-bottom:10px}.quote-status-badge{display:inline-flex;align-items:center;border-radius:999px;padding:4px 11px;font-size:.86em;font-weight:700;letter-spacing:.01em;border:1px solid transparent}.quote-status-badge.draft{background:#fff7ed;color:#9a3412;border-color:#fed7aa}.quote-status-badge.accepted{background:#ecfdf3;color:#166534;border-color:#bbf7d0}.quote-meta-dates{margin-top:4px;font-size:.85em;line-height:1.5;text-align:right}.quote-meta-dates div{margin-top:2px}.muted{color:var(--color-muted)}.item-master-description{color:var(--color-muted);font-size:.88em;line-height:1.45;margin-top:2px;white-space:normal;word-break:break-word}.item-custom-description{color:#1e293b;font-size:.9em;line-height:1.5;margin-top:4px;font-weight:600;white-space:normal;word-break:break-word}table{width:100%;border-collapse:collapse}th,td{border:1px solid var(--border-color);padding:8px;text-align:left}th{background:#f8fafc}.right{text-align:right}.center{text-align:center}.pricing-gross-row td{font-weight:700;font-size:1.1em}.footer{background:var(--footer-bg);color:var(--footer-text-color)}.footer a,.footer a:visited{color:var(--footer-text-color)}.footer-brand-row{display:flex;align-items:center;gap:8px;margin-bottom:8px}.footer-logo{display:inline-flex;align-items:center;justify-content:center}.footer-logo img{max-height:38px;width:auto;display:block}.footer-brand-name{color:var(--footer-text-color);font-size:1.1em;font-weight:700;line-height:1.3}.header{background:var(--header-bg);color:var(--header-text-color)}.header a,.header a:visited{color:var(--header-text-color)}.header-top{display:flex;align-items:center;gap:12px}.header-logo{background:rgba(255,255,255,.18);padding:4px 8px;border-radius:10px;display:inline-flex;align-items:center;justify-content:center}.header-logo img{max-height:50px;width:auto;display:block}.header-main{min-width:0}.screen-only{display:block}.hide-print{display:block}.chart-block{margin-bottom:12px;break-inside:avoid;page-break-inside:avoid}.chart-title{font-weight:700;margin:2px 0 8px}.chart-legend{display:flex;flex-wrap:wrap;gap:10px;margin-top:8px}.legend-item{display:flex;align-items:center;gap:6px;font-size:.92rem}.legend-swatch{width:12px;height:12px;border-radius:3px;display:inline-block}.bar-chart,.bar-chart *{box-sizing:border-box}.bar-chart{position:relative;overflow:hidden;display:flex;align-items:flex-end;justify-content:space-around;gap:10px;min-height:220px;padding:10px 12px 14px;border:1px solid var(--border-color);border-radius:10px;background:var(--card-bg)}.bar-wrap{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;gap:8px;flex:1;min-width:0}.bar-area{height:160px;width:100%;padding:8px 8px 0;display:flex;align-items:flex-end;justify-content:center;overflow:hidden}.bar{width:min(52px,100%);max-height:100%;border-radius:8px 8px 4px 4px;min-height:2px}.bar-label{font-size:.82rem;text-align:center;line-height:1.35;width:100%;overflow-wrap:anywhere}.axis-label{font-size:.82rem;color:var(--color-muted);text-align:center;margin-top:6px}.line-chart svg{width:100%;height:220px;border:1px solid var(--border-color);border-radius:10px;background:var(--card-bg)}.chart-print-img{display:none;width:100%;height:auto;max-width:100%;border:1px solid var(--border-color);border-radius:10px;background:#fff}.sf-glance-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:10px}.sf-glance-group{background:#f8faff;border:1px solid #dbe6f7;border-radius:12px;padding:10px}.sf-glance-group h3{margin:0 0 8px;font-size:1rem}.sf-glance-list{display:grid;gap:4px}.sf-glance-item{display:flex;justify-content:space-between;gap:8px;padding:4px 0;border-bottom:1px dashed #d8e1ef}.sf-glance-item:last-child{border-bottom:none}.sf-glance-label{font-weight:600;color:#23324d;font-size:.88rem}.sf-glance-value{font-weight:700;text-align:right}.sf-kpis{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px}.sf-finance-table-wrap{margin-top:.65rem;overflow-x:auto}.sf-finance-table{width:100%;min-width:760px;border-collapse:collapse;background:#fbfdff}.sf-finance-table th,.sf-finance-table td{border:1px solid #dbe4f0;padding:.6rem .65rem;text-align:left;vertical-align:top}.sf-finance-table thead th{background:#f1f6ff;font-size:.92rem}.sf-finance-table tbody th{font-weight:700;color:#1f2d46;background:#f8fbff;white-space:nowrap}.sf-finance-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:.6rem;margin:.65rem 0}.sf-finance-line{background:#f8fbff;border:1px solid #dbe4f0;border-radius:10px;padding:.6rem .75rem;font-size:.92rem;color:#1f2d46}.payback-meter{width:100%;height:16px;background:#e2e8f0;border-radius:999px;overflow:hidden;margin-top:8px}.payback-meter-fill{height:100%;background:linear-gradient(to right,var(--color-primary),var(--color-accent));width:0}.scenario-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.chart-responsive-card canvas{display:block;width:100%!important;height:520px!important;max-height:520px}.chart-responsive-card.cumulative-chart-card canvas{height:680px!important;max-height:680px}.primary-badge{display:inline-block;font-size:.75rem;background:#ecfeff;color:#0e7490;border:1px solid #a5f3fc;border-radius:999px;padding:2px 8px;font-weight:700;margin-left:6px}@media (max-width:700px){.header-top{flex-direction:column;align-items:flex-start}.footer-brand-row{align-items:flex-start}.scenario-grid{grid-template-columns:1fr}.bank-grid{grid-template-columns:1fr}.tax-summary-grid{grid-template-columns:1fr}.quote-status-row{justify-content:flex-start}.chart-responsive-card{padding:10px 10px 8px}.chart-responsive-card .sec{font-size:.95rem;padding-bottom:4px;margin-bottom:6px}.chart-responsive-card canvas{height:400px!important;max-height:400px}.chart-responsive-card.cumulative-chart-card canvas{height:520px!important;max-height:520px}}@media print{.hide-print,.screen-only{display:none!important}.card{break-inside:avoid;box-shadow:none}.chart-block,.bar-chart,.line-chart{overflow:visible!important;height:auto!important}canvas{display:none!important}.chart-print-img{display:block!important}.item-master-description,.item-custom-description{white-space:normal;word-break:break-word;line-height:1.45}}
body.detailed-proposal{background:linear-gradient(180deg,#eefdf7 0,#f8fafc 340px,var(--page-bg) 100%)}
.detailed-proposal .wrap{max-width:1280px;padding:22px}
.detailed-proposal .card{border-radius:20px;padding:22px;margin-bottom:18px;box-shadow:0 16px 45px rgba(15,23,42,.08)}
.detailed-proposal .sec{border:0;margin:0 0 14px;padding:0;display:flex;align-items:center;gap:8px;font-size:1.18rem}
.detailed-proposal .sec:after{content:"";height:2px;flex:1;background:linear-gradient(90deg,var(--color-primary),rgba(34,197,94,.12));border-radius:99px}
.proposal-cover{overflow:hidden;position:relative;background:radial-gradient(circle at top right,rgba(34,197,94,.34),transparent 34%),linear-gradient(135deg,#064e3b,#0369a1);color:#fff;border:0!important}
.proposal-cover:before{content:"";position:absolute;inset:auto -80px -120px auto;width:260px;height:260px;border-radius:999px;background:rgba(255,255,255,.11)}
.cover-grid{display:grid;grid-template-columns:1.15fr .85fr;gap:22px;position:relative}.cover-logo{background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.28);border-radius:18px;padding:10px 14px;display:inline-flex;align-items:center;gap:10px}.cover-logo img{max-height:58px;width:auto}.cover-kicker{text-transform:uppercase;letter-spacing:.16em;font-size:.78rem;opacity:.85}.cover-title{font-size:clamp(1.8rem,4vw,3.1rem);line-height:1.05;margin:12px 0 8px;font-weight:850}.cover-subtitle{font-size:1.08rem;opacity:.94}.cover-intro{margin-top:18px;max-width:760px}.cover-meta{display:grid;gap:10px}.cover-pill{background:rgba(255,255,255,.13);border:1px solid rgba(255,255,255,.24);border-radius:14px;padding:10px 12px}.cover-pill b{display:block;font-size:.78rem;text-transform:uppercase;letter-spacing:.08em;opacity:.8}.snapshot-grid,.proposal-detail-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.proposal-detail-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.detail-tile{background:linear-gradient(180deg,#fff,#f8fafc);border:1px solid var(--border-color);border-radius:16px;padding:12px;min-width:0}.detail-tile b{display:block;color:var(--color-muted);font-size:.78rem;text-transform:uppercase;letter-spacing:.04em;margin-bottom:4px}.detail-tile div{font-weight:750;word-break:break-word}.proposal-section .intro{color:var(--color-muted);margin-bottom:14px}.premium-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin:0;padding:0;list-style:none}.premium-list li{background:#f8fafc;border:1px solid var(--border-color);border-radius:14px;padding:10px 12px}.premium-list li:before{content:"✓";display:inline-grid;place-items:center;width:21px;height:21px;margin-right:8px;border-radius:999px;background:#dcfce7;color:#166534;font-weight:800}.scope-table-wrap,.table-wrap{overflow-x:auto}.scope-check{display:inline-grid;place-items:center;width:24px;height:24px;border-radius:999px;background:#dcfce7;color:#166534;font-weight:900}.scope-empty{color:#94a3b8}.commercial-offer{background:linear-gradient(135deg,#ecfeff,#f0fdf4)}.timeline{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:12px;counter-reset:milestone}.timeline-step{position:relative;background:#fff;border:1px solid var(--border-color);border-radius:16px;padding:14px}.timeline-step:before{counter-increment:milestone;content:counter(milestone);display:grid;place-items:center;width:28px;height:28px;border-radius:999px;background:linear-gradient(135deg,var(--color-primary),var(--color-accent));color:#fff;font-weight:800;margin-bottom:8px}.signature-grid{display:grid;grid-template-columns:1fr 1fr;gap:18px}.signature-box{border:1px dashed #94a3b8;border-radius:16px;padding:18px;min-height:105px;background:#f8fafc}.signature-line{border-top:1px solid #64748b;margin-top:42px;padding-top:6px}.qr-img{max-width:160px;border:1px solid var(--border-color);border-radius:12px;padding:8px;background:#fff}.detailed-proposal table{border-collapse:separate;border-spacing:0;overflow:hidden;border-radius:14px}.detailed-proposal th{background:#eef6ff;color:#0f172a}.detailed-proposal th,.detailed-proposal td{border-color:#dbe7ef}
@media (max-width:760px){.detailed-proposal .wrap{padding:10px}.detailed-proposal .card{padding:14px;border-radius:16px}.cover-grid,.snapshot-grid,.proposal-detail-grid,.premium-list,.signature-grid{grid-template-columns:1fr}.cover-title{font-size:1.75rem}.hero,.grid4,.grid2,.bank-grid,.tax-summary-grid{grid-template-columns:1fr}.proposal-section{break-inside:auto}.table-wrap,.scope-table-wrap{margin-left:-4px;margin-right:-4px;padding-bottom:4px}}
@media print{body.detailed-proposal{background:#fff}.detailed-proposal .wrap{max-width:none;padding:0}.detailed-proposal .card{box-shadow:none;border-radius:10px;break-inside:avoid;page-break-inside:avoid}.proposal-cover{background:#075985!important;color:#fff!important}.premium-list{grid-template-columns:repeat(2,1fr)}.timeline{grid-template-columns:repeat(4,1fr)}.table-wrap,.scope-table-wrap{overflow:visible}.signature-box{break-inside:avoid}}
</style>
</head><body class="<?= $isDetailedProposal ? 'detailed-proposal' : 'compact-proposal' ?>"><div id="quotation-root" class="wrap">
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'cover')): ?>
<section class="card proposal-cover"><div class="cover-grid"><div><div class="cover-logo"><?php if ($hasLogo): ?><img src="<?= htmlspecialchars($logoSrc, ENT_QUOTES) ?>" alt="<?= htmlspecialchars($companyName, ENT_QUOTES) ?> logo"><?php endif; ?><strong><?= htmlspecialchars($companyName, ENT_QUOTES) ?></strong></div><div class="cover-kicker">Premium Solar Proposal</div><div class="cover-title"><?= htmlspecialchars((string)($proposalSections['cover']['title'] ?? 'Detailed Techno-Commercial Proposal'), ENT_QUOTES) ?></div><div class="cover-subtitle"><?= htmlspecialchars((string)($proposalSections['cover']['subtitle'] ?? ''), ENT_QUOTES) ?></div><?php if (trim((string)($proposalSections['cover']['intro'] ?? '')) !== ''): ?><div class="cover-intro"><?= quotation_sanitize_html((string)$proposalSections['cover']['intro']) ?></div><?php endif; ?></div><div class="cover-meta"><div class="cover-pill"><b>Quote number</b><?= htmlspecialchars((string)($quote['quote_no'] ?? ''), ENT_QUOTES) ?></div><div class="cover-pill"><b>Quotation date</b><?= htmlspecialchars($quotationDateDisplay, ENT_QUOTES) ?></div><div class="cover-pill"><b>Valid until</b><?= htmlspecialchars($validUntilDisplay, ENT_QUOTES) ?></div><div class="cover-pill"><b>Customer</b><?= htmlspecialchars((string)($quote['customer_name'] ?? ''), ENT_QUOTES) ?></div><div class="cover-pill"><b>Location</b><?= htmlspecialchars(trim((string)($quote['city'] ?? '') . ' ' . (string)($quote['state'] ?? '')), ENT_QUOTES) ?></div><div class="cover-pill"><b>System</b><?= htmlspecialchars((string)($quote['system_type'] ?? ''), ENT_QUOTES) ?> · <?= htmlspecialchars((string)($quote['capacity_kwp'] ?? ''), ENT_QUOTES) ?> kWp</div><div class="cover-pill"><b>Proposal reference</b><?= htmlspecialchars($proposalRef, ENT_QUOTES) ?></div><div class="cover-pill"><b>Prepared by</b><?= htmlspecialchars((string)($proposalSections['cover']['prepared_by'] ?? ''), ENT_QUOTES) ?></div><?php if (trim((string)($company['gstin'] ?? '')) !== ''): ?><div class="cover-pill"><b>GSTIN</b><?= htmlspecialchars((string)($company['gstin'] ?? ''), ENT_QUOTES) ?></div><?php endif; ?></div></div><?php if (trim((string)($proposalSections['cover']['validity_wording'] ?? '')) !== ''): ?><div style="position:relative;margin-top:16px;opacity:.92"><?= htmlspecialchars((string)$proposalSections['cover']['validity_wording'], ENT_QUOTES) ?></div><?php endif; ?></section>
<?php endif; ?>
...
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'project_snapshot') && $snapshotFields !== []): ?><section class="card proposal-section"><div class="h sec">🧭 <?= htmlspecialchars((string)($proposalSections['project_snapshot']['title'] ?? 'Project Snapshot'), ENT_QUOTES) ?></div><?php if (trim((string)($proposalSections['project_snapshot']['intro'] ?? '')) !== ''): ?><div class="intro"><?= htmlspecialchars((string)$proposalSections['project_snapshot']['intro'], ENT_QUOTES) ?></div><?php endif; ?><div class="snapshot-grid"><?php foreach ($snapshotFields as $field): ?><div class="detail-tile"><b><?= htmlspecialchars((string)$field[0], ENT_QUOTES) ?></b><div><?= nl2br(htmlspecialchars((string)$field[1], ENT_QUOTES)) ?></div></div><?php endforeach; ?></div></section><?php endif; ?>
...
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'commercial_offer') && $commercialRows !== []): ?><section class="card proposal-section commercial-offer"><div class="h sec">💼 <?= htmlspecialchars((string)($proposalSections['commercial_offer']['title'] ?? 'Commercial Offer'), ENT_QUOTES) ?></div><div class="proposal-detail-grid"><?php foreach ($commercialRows as $row): ?><div class="detail-tile"><b><?= htmlspecialchars((string)$row[0], ENT_QUOTES) ?></b><div><?= htmlspecialchars((string)$row[1], ENT_QUOTES) ?></div></div><?php endforeach; ?></div><?php if (trim((string)($proposalSections['commercial_offer']['notes'] ?? '')) !== ''): ?><div class="intro" style="margin-top:12px"><?= quotation_sanitize_html((string)$proposalSections['commercial_offer']['notes']) ?></div><?php endif; ?></section><?php endif; ?>
...
<?php if ($isDetailedProposal && !empty($quote['proposal_show_technical_sections'])): ?>
<?php if (quotation_section_enabled($proposalSections, 'system_configuration') && $configPoints !== []): ?><section class="card proposal-section"><div class="h sec">🧩 <?= htmlspecialchars((string)($proposalSections['system_configuration']['title'] ?? 'System Configuration'), ENT_QUOTES) ?></div><?php if (trim((string)($proposalSections['system_configuration']['intro'] ?? '')) !== ''): ?><div class="intro"><?= htmlspecialchars((string)$proposalSections['system_configuration']['intro'], ENT_QUOTES) ?></div><?php endif; ?><ul class="premium-list"><?php foreach ($configPoints as $point): ?><li><?= htmlspecialchars((string)$point, ENT_QUOTES) ?></li><?php endforeach; ?></ul></section><?php endif; ?>
<?php if (quotation_section_enabled($proposalSections, 'system_working')): $workingTitle = $isHybridProposal ? (string)($proposalSections['system_working']['hybrid_title'] ?? '') : (string)($proposalSections['system_working']['on_grid_title'] ?? ''); $workingHtml = $isHybridProposal ? (string)($proposalSections['system_working']['hybrid_html'] ?? '') : (string)($proposalSections['system_working']['on_grid_html'] ?? ''); ?><section class="card proposal-section"><div class="h sec">🔆 <?= htmlspecialchars($workingTitle !== '' ? $workingTitle : 'How the Solar System Works', ENT_QUOTES) ?></div><div class="grid2"><div><?= quotation_sanitize_html($workingHtml) ?></div><?php if (trim((string)($proposalSections['system_working']['diagram_url'] ?? '')) !== ''): ?><div><img src="<?= htmlspecialchars((string)$proposalSections['system_working']['diagram_url'], ENT_QUOTES) ?>" alt="Solar system working diagram" style="max-width:100%;border-radius:16px;border:1px solid var(--border-color)"></div><?php endif; ?></div></section><?php endif; ?>
<?php endif; ?>
<?php if ($isDetailedProposal && !empty($quote['proposal_show_scope_matrix']) && quotation_section_enabled($proposalSections, 'scope_matrix') && $scopeRows !== []): ?><section class="card proposal-section"><div class="h sec">📋 <?= htmlspecialchars((string)($proposalSections['scope_matrix']['title'] ?? 'EPC Scope Matrix'), ENT_QUOTES) ?></div><?php if (trim((string)($proposalSections['scope_matrix']['intro'] ?? '')) !== ''): ?><div class="intro"><?= htmlspecialchars((string)$proposalSections['scope_matrix']['intro'], ENT_QUOTES) ?></div><?php endif; ?><div class="scope-table-wrap"><table><thead><tr><th>Sr No</th><th>Scope Item</th><th class="center">Dakshayani Scope</th><th class="center">Customer Scope</th><th>Specification / Description</th></tr></thead><tbody><?php foreach ($scopeRows as $idx => $row): ?><tr><td><?= (int)$idx + 1 ?></td><td><?= htmlspecialchars((string)($row['scope_item'] ?? ''), ENT_QUOTES) ?></td><td class="center"><?= !empty($row['dakshayani_scope']) ? '<span class="scope-check">✓</span>' : '<span class="scope-empty">—</span>' ?></td><td class="center"><?= !empty($row['customer_scope']) ? '<span class="scope-check">✓</span>' : '<span class="scope-empty">—</span>' ?></td><td><?= htmlspecialchars((string)($row['description'] ?? ''), ENT_QUOTES) ?></td></tr><?php endforeach; ?></tbody></table></div></section><?php endif; ?>
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'technical_assumptions')): $assumptionPoints=quotation_lines_to_array($proposalSections['technical_assumptions']['points'] ?? ''); if ($assumptionPoints !== []): ?><section class="card proposal-section"><div class="h sec">🛠️ <?= htmlspecialchars((string)($proposalSections['technical_assumptions']['title'] ?? 'Technical Assumptions & Site Readiness'), ENT_QUOTES) ?></div><div class="intro"><?= htmlspecialchars((string)($proposalSections['technical_assumptions']['intro'] ?? ''), ENT_QUOTES) ?></div><ul class="premium-list"><?php foreach ($assumptionPoints as $point): ?><li><?= htmlspecialchars($point, ENT_QUOTES) ?></li><?php endforeach; ?></ul></section><?php endif; endif; ?>
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'exclusions')): $exclusionPoints=quotation_lines_to_array($proposalSections['exclusions']['points'] ?? ''); if ($exclusionPoints !== []): ?><section class="card proposal-section"><div class="h sec">🚫 <?= htmlspecialchars((string)($proposalSections['exclusions']['title'] ?? 'Exclusions'), ENT_QUOTES) ?></div><div class="intro"><?= htmlspecialchars((string)($proposalSections['exclusions']['intro'] ?? ''), ENT_QUOTES) ?></div><ul class="premium-list"><?php foreach ($exclusionPoints as $point): ?><li><?= htmlspecialchars($point, ENT_QUOTES) ?></li><?php endforeach; ?></ul></section><?php endif; endif; ?>
<?php if ($isDetailedProposal && !empty($quote['proposal_show_safety_section']) && quotation_section_enabled($proposalSections, 'safety_compliance') && $safetyRows !== []): ?><section class="card proposal-section"><div class="h sec">🛡️ <?= htmlspecialchars((string)($proposalSections['safety_compliance']['title'] ?? 'Safety & Compliance'), ENT_QUOTES) ?></div><div class="intro"><?= htmlspecialchars((string)($proposalSections['safety_compliance']['intro'] ?? ''), ENT_QUOTES) ?></div><div class="table-wrap"><table><thead><tr><th>Safety Item</th><th>Description</th></tr></thead><tbody><?php foreach ($safetyRows as $row): ?><tr><td><?= htmlspecialchars((string)($row['item'] ?? $row['safety_item'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($row['description'] ?? ''), ENT_QUOTES) ?></td></tr><?php endforeach; ?></tbody></table></div></section><?php endif; ?>
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'warranty_om')): $warrantyPoints=quotation_lines_to_array($proposalSections['warranty_om']['points'] ?? ''); if ($warrantyPoints !== []): ?><section class="card proposal-section"><div class="h sec">🔧 <?= htmlspecialchars((string)($proposalSections['warranty_om']['title'] ?? 'Warranty, O&M & Service Support'), ENT_QUOTES) ?></div><div class="intro"><?= htmlspecialchars((string)($proposalSections['warranty_om']['intro'] ?? ''), ENT_QUOTES) ?></div><ul class="premium-list"><?php foreach ($warrantyPoints as $point): ?><li><?= htmlspecialchars($point, ENT_QUOTES) ?></li><?php endforeach; ?></ul></section><?php endif; endif; ?>
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'project_timeline')): $milestones=quotation_lines_to_array($proposalSections['project_timeline']['milestones'] ?? ''); if ($milestones !== []): ?><section class="card proposal-section"><div class="h sec">🗓️ <?= htmlspecialchars((string)($proposalSections['project_timeline']['title'] ?? 'Project Execution Timeline'), ENT_QUOTES) ?></div><div class="intro"><?= htmlspecialchars((string)($proposalSections['project_timeline']['intro'] ?? ''), ENT_QUOTES) ?></div><div class="timeline"><?php foreach ($milestones as $milestone): ?><div class="timeline-step"><?= htmlspecialchars($milestone, ENT_QUOTES) ?></div><?php endforeach; ?></div></section><?php endif; endif; ?>
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'payment_terms') && $paymentTermsHtml !== ''): ?><section class="card proposal-section"><div class="h sec">💳 <?= htmlspecialchars((string)($proposalSections['payment_terms']['title'] ?? 'Payment Terms'), ENT_QUOTES) ?></div><?= quotation_sanitize_html($paymentTermsHtml) ?></section><?php endif; ?>
<?php if ($isDetailedProposal && $subsidyHtml !== ''): ?><section class="card proposal-section"><div class="h sec">🏛️ PM Surya Ghar / Subsidy Information</div><?= quotation_sanitize_html($subsidyHtml) ?></section><?php endif; ?>
<?php if ($isDetailedProposal && quotation_section_enabled($proposalSections, 'next_steps')): $nextStepPoints=quotation_lines_to_array($proposalSections['next_steps']['points'] ?? ''); if ($nextStepPoints !== []): ?><section class="card proposal-section"><div class="h sec">🚀 <?= htmlspecialchars((string)($proposalSections['next_steps']['title'] ?? 'Next Steps'), ENT_QUOTES) ?></div><ul class="premium-list"><?php foreach ($nextStepPoints as $point): ?><li><?= htmlspecialchars($point, ENT_QUOTES) ?></li><?php endforeach; ?></ul></section><?php endif; endif; ?>
...
<?php if ($bankFields !== [] && (!$isDetailedProposal || (!empty($quote['proposal_show_bank_details']) && quotation_section_enabled($proposalSections, 'bank_details')))): ?>
<section class="card proposal-section"><div class="h sec"><?= $isDetailedProposal ? '🏦 ' . htmlspecialchars((string)($proposalSections['bank_details']['title'] ?? 'Bank / Payment Details'), ENT_QUOTES) : 'Bank Details' ?></div><?php if ($isDetailedProposal && trim((string)($proposalSections['bank_details']['intro'] ?? '')) !== ''): ?><div class="intro"><?= htmlspecialchars((string)$proposalSections['bank_details']['intro'], ENT_QUOTES) ?></div><?php endif; ?><div class="bank-grid"><?php foreach ($bankFields as $bankField): ?><div class="metric"><div class="h"><?= htmlspecialchars((string) ($bankField['icon'] . ' ' . $bankField['label']), ENT_QUOTES) ?></div><div><?= htmlspecialchars((string) $bankField['value'], ENT_QUOTES) ?></div></div><?php endforeach; ?><?php if ($isDetailedProposal && $bankQrImage !== ''): ?><div class="metric"><div class="h">📱 Payment QR</div><img class="qr-img" src="<?= htmlspecialchars($bankQrImage, ENT_QUOTES) ?>" alt="Payment QR"></div><?php endif; ?></div></section>
<?php endif; ?>
<?php if ($isDetailedProposal && !empty($quote['proposal_show_acceptance']) && quotation_section_enabled($proposalSections, 'acceptance')): ?><section class="card proposal-section"><div class="h sec">✍️ <?= htmlspecialchars((string)($proposalSections['acceptance']['title'] ?? 'Acceptance / Signature'), ENT_QUOTES) ?></div><p><?= htmlspecialchars((string)($proposalSections['acceptance']['wording'] ?? ''), ENT_QUOTES) ?></p><div class="signature-grid"><div class="signature-box"><b>Customer</b><div><?= htmlspecialchars((string)($quote['customer_name'] ?? ''), ENT_QUOTES) ?></div><div class="signature-line">Signature & Date</div></div><div class="signature-box"><b>Vendor</b><div><?= htmlspecialchars($companyName, ENT_QUOTES) ?></div><div class="signature-line">Authorized Signatory & Date</div></div></div></section><?php endif; ?>
