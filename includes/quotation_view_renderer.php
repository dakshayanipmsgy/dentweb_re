...
function quotation_render(array $quote, array $quoteDefaults, array $company, bool $showAdmin = false, string $shareUrl = '', string $viewerType = 'admin', string $viewerId = ''): void
{
    ...
    $upiQrPath = function_exists('documents_normalize_upi_qr_path')
        ? documents_normalize_upi_qr_path($company['upi_qr_path'] ?? '')
        : '';
    $hasUpiQr = $upiQrPath !== '' && function_exists('documents_upi_qr_asset_exists') && documents_upi_qr_asset_exists($upiQrPath);
    $upiQrAlt = 'UPI QR code for ' . $companyName;
...
<?php if ($bankFields !== [] || $hasUpiQr): ?><section class="card bank-card"><div class="section-kicker">Secure payment information</div><div class="h sec">Payment Details</div><div class="payment-layout"><?php if ($bankFields !== []): ?><div><div class="bank-grid"><?php foreach ($bankFields as $bankField): ?><div class="metric"><span class="metric-label"><?= htmlspecialchars((string)$bankField['label'], ENT_QUOTES) ?></span><div><?= htmlspecialchars((string)$bankField['value'], ENT_QUOTES) ?></div></div><?php endforeach; ?></div></div><?php endif; ?><?php if ($hasUpiQr): ?><div class="upi-pay-card"><img src="<?= htmlspecialchars($upiQrPath, ENT_QUOTES) ?>" alt="<?= htmlspecialchars($upiQrAlt, ENT_QUOTES) ?>"><strong>Scan to pay by UPI</strong><?php if (trim((string)($company['upi_id'] ?? '')) !== ''): ?><span>UPI ID: <?= htmlspecialchars(trim((string)$company['upi_id']), ENT_QUOTES) ?></span><?php endif; ?></div><?php endif; ?></div><div class="payment-warning">Please verify account details before payment. Payments should be made only to <?= htmlspecialchars($companyName, ENT_QUOTES) ?>.</div></section><?php endif; ?>
