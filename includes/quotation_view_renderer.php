...
<?php if ($showAdmin): ?>
<?php $statusNorm = documents_quote_normalize_status((string)($quote['status'] ?? 'draft')); $isAdmin = $viewerType === 'admin'; $isEmployee = $viewerType === 'employee';
$workflow = array_merge(documents_quote_workflow_defaults(), is_array($quote['workflow'] ?? null) ? $quote['workflow'] : []);
$docAgreement = $workflow['agreement_id'] !== '' ? documents_get_sales_document('agreement', (string)$workflow['agreement_id']) : null;
$docPi = $workflow['proforma_invoice_id'] !== '' ? documents_get_sales_document('proforma', (string)$workflow['proforma_invoice_id']) : null;
$docInv = $workflow['invoice_id'] !== '' ? documents_get_sales_document('invoice', (string)$workflow['invoice_id']) : null;
$receipts = array_values(array_filter(documents_list_sales_documents('receipt'), static fn(array $d): bool => (string)($d['quotation_id'] ?? '') === (string)($quote['id'] ?? '')));
$deliveryDocs = array_values(array_filter(documents_list_sales_documents('delivery_challan'), static fn(array $d): bool => (string)($d['quotation_id'] ?? '') === (string)($quote['id'] ?? '')));
?>
<div class="card hide-print"><div class="h">Admin Actions</div><a class="chip" href="admin-quotations.php?tab=settings">Quotation Settings</a><div><?= htmlspecialchars($shareUrl, ENT_QUOTES) ?></div>
<form method="post" style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>">
<?php if ($isAdmin && $statusNorm === 'draft' && (string)($quote['created_by_type'] ?? '') === 'employee'): ?><button class="chip" name="action" value="approve_quote" type="submit">Send for Admin Approval / Approve</button><?php endif; ?>
<?php if ($isAdmin && $statusNorm === 'approved'): ?><button class="chip" name="action" value="mark_accepted" type="submit">✅ Mark Accepted by Customer</button><?php endif; ?>
<?php if ($isAdmin && empty($quote['archived_flag'])): ?><button class="chip" name="action" value="archive_quote" type="submit">Archive Quotation</button><?php endif; ?>
<?php if ($isAdmin && !empty($quote['archived_flag'])): ?><button class="chip" name="action" value="unarchive_quote" type="submit">Unarchive Quotation</button><?php endif; ?>
</form></div>
<?php if ($statusNorm === 'accepted' || $statusNorm === 'archived'): ?>
<div class="card hide-print"><div class="h">Workflow Documents</div>
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px">
<div><strong>Agreement</strong><div class="sub"><?= $docAgreement ? htmlspecialchars((string)$docAgreement['id'], ENT_QUOTES) : 'Not created' ?></div><?php if ($isAdmin): ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="create_agreement"><input name="execution_date" type="date" value="<?= date('Y-m-d') ?>"><input name="kwp" placeholder="kWp" value="<?= htmlspecialchars((string)($quote['capacity_kwp'] ?? ''), ENT_QUOTES) ?>"><input name="amount" placeholder="Amount" value="<?= htmlspecialchars((string)($calc['gross_payable'] ?? 0), ENT_QUOTES) ?>"><button class="chip" type="submit">Create Agreement</button></form><?php endif; ?></div>
<div><strong>Receipts</strong><div class="sub"><?= count($receipts) ?> saved</div><?php if ($isAdmin): ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="create_receipt"><input name="receipt_date" type="date" value="<?= date('Y-m-d') ?>"><input name="amount_received" placeholder="Amount received"><input name="mode" placeholder="mode cash/upi/bank"><input name="reference" placeholder="txn/reference"><input name="against" placeholder="against"><button class="chip" type="submit">Add Receipt</button></form><?php endif; ?></div>
<div><strong>Delivery Challans</strong><div class="sub"><?= count($deliveryDocs) ?> saved</div><?php if ($isAdmin || $isEmployee): ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="create_delivery_challan"><input name="challan_date" type="date" value="<?= date('Y-m-d') ?>"><input name="dispatch_from" placeholder="Dispatch from"><input name="vehicle_transporter" placeholder="Vehicle/Transporter"><input name="item_name" placeholder="Item"><input name="item_qty" placeholder="Qty"><input name="item_unit" placeholder="Unit"><input name="item_remarks" placeholder="Remarks"><button class="chip" type="submit">Create DC</button></form><?php endif; ?></div>
<div><strong>Proforma Invoice</strong><div class="sub"><?= $docPi ? htmlspecialchars((string)$docPi['id'], ENT_QUOTES) : 'Not created' ?></div><?php if ($isAdmin): ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="create_proforma"><input name="pi_date" type="date" value="<?= date('Y-m-d') ?>"><input name="pi_number" placeholder="PI Number"><input name="terms" placeholder="Terms"><button class="chip" type="submit">Create PI</button></form><?php endif; ?></div>
<div><strong>Final Invoice</strong><div class="sub"><?= $docInv ? htmlspecialchars((string)$docInv['id'], ENT_QUOTES) : 'Not created' ?></div><?php if ($isAdmin): ?><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="create_invoice"><input name="invoice_date" type="date" value="<?= date('Y-m-d') ?>"><input name="invoice_number" placeholder="Invoice Number"><input name="bill_to" placeholder="Bill To"><input name="ship_to" placeholder="Ship To"><button class="chip" type="submit">Create Invoice</button></form><?php endif; ?></div>
</div>
</div>
<?php endif; ?>
<?php endif; ?>
