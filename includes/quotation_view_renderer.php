...
function compute_financial_clarity(array $quote, array $calc, array $snapshot): array
{
    ...
    $capacitySplit = function_exists('documents_quote_system_capacity_split') ? documents_quote_system_capacity_split($quote) : ['main' => max(0, $toFloat($quote['capacity_kwp'] ?? $quote['system_capacity_kwp'] ?? 0)), 'complimentary' => 0.0, 'complimentary_provided' => false, 'total' => max(0, $toFloat($quote['capacity_kwp'] ?? $quote['system_capacity_kwp'] ?? 0))];
    $capacity = max(0, $toFloat($capacitySplit['total'] ?? 0));
    ...
    return [
        ...
        'capacity_kwp_main' => max(0, $toFloat($capacitySplit['main'] ?? $capacity)),
        'capacity_kwp_complimentary' => max(0, $toFloat($capacitySplit['complimentary'] ?? 0)),
        'capacity_kwp_complimentary_provided' => !empty($capacitySplit['complimentary_provided']),
