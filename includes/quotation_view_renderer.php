...
require_once __DIR__ . '/document_signature.php';
...
.panel-orientation-card{position:relative;overflow:hidden;break-inside:avoid;page-break-inside:avoid;background:linear-gradient(145deg,#ffffff 0%,#f7fbfa 58%,#f5f9ff 100%);border-color:#d9e7e4}
.panel-orientation-card:before{content:"";position:absolute;right:-70px;top:-90px;width:210px;height:210px;border-radius:50%;background:radial-gradient(circle,rgba(20,184,166,.13),rgba(20,184,166,0) 70%);pointer-events:none}
.panel-orientation-intro{position:relative;display:flex;justify-content:space-between;align-items:flex-start;gap:20px}
.panel-orientation-intro p{max-width:760px;margin:4px 0 0;color:#52636b}
.panel-layout-approval-pill{flex:0 0 auto;padding:7px 11px;border:1px solid #b9ddd8;border-radius:999px;background:#ecfdf5;color:#0f766e;font-size:.72rem;font-weight:850;letter-spacing:.02em}
.panel-orientation-diagram-shell{margin:16px 0 14px;padding:10px;border:1px solid #dce8e5;border-radius:18px;background:rgba(255,255,255,.86);box-shadow:0 12px 30px rgba(15,23,42,.06)}
.panel-orientation-diagram{margin:0;border:0;border-radius:14px;background:linear-gradient(160deg,#f8fafc,#f1f7f6);overflow:hidden}
.panel-orientation-diagram svg{display:block;width:100%;height:auto}
.orientation-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}
.orientation-summary .metric{min-height:68px;padding:11px 12px;border:1px solid #e1ebe8;border-radius:13px;background:rgba(255,255,255,.84)}
.orientation-summary .metric>div{margin-top:4px;font-weight:800;color:#123c3a}
.orientation-approval-note{display:flex;gap:10px;align-items:flex-start;margin-top:12px;padding:11px 13px;border-radius:13px;background:#effaf7;color:#315a57;font-size:.88rem}
.orientation-approval-note strong{color:#0f766e;white-space:nowrap}
@media(max-width:600px){.panel-orientation-intro{display:grid}.panel-layout-approval-pill{justify-self:start}.orientation-summary{grid-template-columns:1fr}.orientation-approval-note{display:grid}}
@media print{.panel-orientation-card:before{display:none}.panel-orientation-diagram-shell{padding:2.5mm;margin:3mm 0;box-shadow:none}.orientation-summary{grid-template-columns:repeat(3,1fr);gap:2mm}.orientation-summary .metric{min-height:14mm;padding:2.5mm}.orientation-approval-note{margin-top:2mm;padding:2.5mm}}
</style>
<style>
<?php // Inline the local theme for standalone HTML and bulk-print exports.
readfile(__DIR__ . "/../assets/css/quotation-presentation.css"); ?>
</style>
</head><body>
...
<?php if ($showPanelOrientation): ?><section class="card panel-orientation-card"><div class="panel-orientation-intro"><div><div class="section-kicker">Proposed rooftop arrangement</div><div class="h sec">Solar Panel Orientation / Layout</div><p>This drawing shows the proposed panel arrangement for easy customer review. The visual layout is indicative; small on-site shifts may be made for structural safety, access, shade avoidance and cable routing without changing the agreed system capacity or commercial scope.</p></div><div class="panel-layout-approval-pill">For layout confirmation</div></div><div class="panel-orientation-diagram-shell"><?= $panelOrientationDiagram ?></div><div class="orientation-summary"><div class="metric"><span class="metric-label">Installation area</span><div><?= htmlspecialchars((string)$panelOrientation['site_area_label'], ENT_QUOTES) ?></div></div><div class="metric"><span class="metric-label">Primary facing direction</span><div><?= htmlspecialchars((string)$panelOrientation['default_facing_direction'], ENT_QUOTES) ?></div></div><div class="metric"><span class="metric-label">Approx. tilt</span><div><?= trim((string)$panelOrientation['default_tilt_deg']) !== '' ? htmlspecialchars((string)$panelOrientation['default_tilt_deg'], ENT_QUOTES).'°' : 'To be finalized on site' ?></div></div></div><div class="orientation-approval-note"><strong>Layout basis</strong><span>The Solar Panel Layout Designer drawing above is the customer-facing reference for proposed placement. Major changes that affect capacity, price or scope require confirmation or a revised quotation.</span></div></section><?php endif; ?>
<?= $importantPointsHtml ?>
<?php if ($coverNote !== ''): ?><section class="card friendly-note"><div class="section-kicker">A NOTE</div><div><?= quotation_sanitize_html($coverNote) ?></div><div class="proposal-benefits"><div class="proposal-benefit">Clear system scope</div><div class="proposal-benefit">Transparent pricing</div><div class="proposal-benefit">Subsidy guidance</div><div class="proposal-benefit">Installation &amp; support</div></div></section><?php endif; ?>
