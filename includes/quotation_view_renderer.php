...
function quotation_render(array $quote, array $quoteDefaults, array $company, bool $showAdmin = false, string $shareUrl = '', string $viewerType = 'admin', string $viewerId = ''): void
{
    ...
    $bankRaw = is_array($company['bank'] ?? null) ? $company['bank'] : [];
    $bank = documents_normalize_company_bank_details($bankRaw);
    $paymentFields = [
        'bank_name' => ['label' => 'Bank Name', 'value' => trim((string) ($bank['bank_name'] ?? ''))],
        'account_name' => ['label' => 'Account Name', 'value' => trim((string) ($bank['account_name'] ?? ''))],
        'account_no' => ['label' => 'Account No', 'value' => trim((string) ($bank['account_no'] ?? ''))],
        'ifsc' => ['label' => 'IFSC', 'value' => trim((string) ($bank['ifsc'] ?? ''))],
        'branch' => ['label' => 'Branch', 'value' => trim((string) ($bank['branch'] ?? ''))],
    ];
...
<section class="card"><div class="h sec">🏦 Payment Details</div><div class="grid2"><div class="metric"><b>Bank Transfer</b><?php foreach ($paymentFields as $paymentField): if ($paymentField['value'] === '') { continue; } ?><div style="margin-top:6px"><span class="muted"><?= htmlspecialchars((string) $paymentField['label'], ENT_QUOTES) ?>:</span> <b><?= htmlspecialchars((string) $paymentField['value'], ENT_QUOTES) ?></b></div><?php endforeach; ?></div><div class="metric"><b>UPI</b><?php if ($upiId !== ''): ?><div style="margin-top:8px"><span class="muted">UPI ID:</span><div class="payment-upi-id"><?= htmlspecialchars($upiId, ENT_QUOTES) ?></div></div><?php else: ?><div class="muted" style="margin-top:8px">UPI ID not provided.</div><?php endif; ?></div></div></section>
<?php endif; ?>
