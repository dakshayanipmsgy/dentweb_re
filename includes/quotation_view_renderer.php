...
function compute_financial_clarity(array $quote, array $calc, array $snapshot): array
{
    ...
    $capacityMain = max(0, $toFloat($quote['system_capacity_kwp_main'] ?? 0));
    if ($capacityMain <= 0) {
        $capacityMain = max(0, $toFloat($quote['capacity_kwp'] ?? $quote['system_capacity_kwp'] ?? 0));
    }
    $capacityComplimentaryRaw = $quote['system_capacity_kwp_complimentary'] ?? null;
    $capacityComplimentary = 0.0;
    if (!($capacityComplimentaryRaw === null || (is_string($capacityComplimentaryRaw) && trim($capacityComplimentaryRaw) === ''))) {
        $capacityComplimentary = max(0, $toFloat($capacityComplimentaryRaw));
    }
    $capacity = $capacityMain + $capacityComplimentary;
    ...
    return [
        ...
        'capacity_kwp_main' => $capacityMain,
        'capacity_kwp_complimentary' => $capacityComplimentary,
...
function quotation_render(array $quote, array $quoteDefaults, array $company, bool $showAdmin = false, string $shareUrl = '', string $viewerType = 'admin', string $viewerId = ''): void
{
    ...
    $capacityMainDisplay = (float) ($financialClarity['capacity_kwp_main'] ?? 0);
    $capacityComplimentaryDisplay = (float) ($financialClarity['capacity_kwp_complimentary'] ?? 0);
    $grossPayableLabel = ((float) ($calc['discount_rs'] ?? 0) > 0) ? 'Gross Payable (after discount)' : 'Gross Payable';
...
<section class="card quote-status-row"><span class="quote-status-badge <?= htmlspecialchars($statusBadgeClass, ENT_QUOTES) ?>">Status: <?= htmlspecialchars($statusBadgeLabel, ENT_QUOTES) ?></span></section>
...
const setMetric=(id,value,formatter)=>{
...
const units25=yearly*25;
const saved25=units25*q.unit;
...
setMetric('genMonthly',solar,nUnits);
setMetric('genAnnual',yearly,nUnits);
setMetric('gen25',units25,nUnits);
setMetric('gen25Savings',saved25,r);
</script>
</body></html>
<?php
    $output = ob_get_clean();
