...
function quotation_render(array $quote, array $quoteDefaults, array $company, bool $showAdmin = false, string $shareUrl = '', string $viewerType = 'admin', string $viewerId = ''): void
{
    ...
    $bankRows = [
        'Account Holder' => trim((string) ($bank['account_holder'] ?? $company['bank_account_name'] ?? '')),
        'Bank' => trim((string) ($bank['bank_name'] ?? $company['bank_name'] ?? '')),
        'A/c No.' => trim((string) ($bank['account_number'] ?? $company['bank_account_no'] ?? '')),
        'IFSC' => strtoupper(trim((string) ($bank['ifsc'] ?? $company['bank_ifsc'] ?? ''))),
        'Branch' => trim((string) ($bank['branch'] ?? $company['bank_branch'] ?? '')),
        'UPI' => trim((string) ($bank['upi_id'] ?? $company['upi_id'] ?? '')),
    ];
    $bankRows = array_filter($bankRows, static fn($value): bool => $value !== '');
...
<?php if ($bankRows !== []): ?><section class="card"><div class="h sec">🏦 Bank Details</div><table class="bank-details"><?php foreach ($bankRows as $label => $value): ?><tr><td><?= htmlspecialchars((string) $label, ENT_QUOTES) ?></td><td class="<?= in_array($label, ['A/c No.', 'IFSC'], true) ? 'bank-value-code' : '' ?>"><?= htmlspecialchars((string) $value, ENT_QUOTES) ?></td></tr><?php endforeach; ?></table></section><?php endif; ?>
