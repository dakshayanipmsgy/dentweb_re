...
function solar_finance_default_on_grid_prices(): array
{
    return [
...
function solar_finance_default_hybrid_prices(): array
{
    return [
        ['model' => 'NA', 'solar_rating_kw' => 1, 'kVA' => null, 'phase' => null, 'battery' => null, 'price_loan_upto_2_lakh' => null, 'price_loan_above_2_lakh' => null],
...
function solar_finance_settings_default(): array
{
    return [
        'defaults' => [
            ...
            'tree_equivalence_kg_per_tree_per_year' => 20,
        ],
        'content' => [
            'hero_intro' => 'Simple solar guidance for homes and businesses—understand your suitable system size, costs, EMI, savings, and PM Surya Ghar basics in one place.',
            'what_is_solar_rooftop' => 'Rooftop solar means placing solar panels on your roof to generate electricity for your own use. It reduces your monthly bill and uses clean energy.',
            'pm_surya_ghar' => 'PM Surya Ghar: Muft Bijli Yojana supports eligible residential rooftop solar users through subsidy. Final subsidy eligibility and amount depend on policy and DISCOM processes.',
            'on_grid' => 'On-grid systems connect directly to the electricity grid. They are usually lower cost and ideal when your goal is faster savings on electricity bills.',
            'hybrid' => 'Hybrid systems include battery support, so selected appliances can run during power cuts. They are better for backup but usually cost more than on-grid.',
            'who_should_choose' => 'If your main priority is lower upfront cost, on-grid is usually suitable. If backup during power cuts is important, hybrid can be more suitable.',
            'benefits' => "Lower monthly electricity bill\nClean energy generation\nLong-term savings over 25 years\nHigher property value and energy independence",
            'expectations' => 'Savings depend on roof shadow, tariff changes, day-time usage, system quality, and net-metering rules. Treat estimates as guidance, not guaranteed returns.',
            'faq' => "Q: Will solar remove my bill completely?\nA: Not always. It depends on usage pattern and system size.\n\nQ: Is subsidy guaranteed?\nA: Subsidy is policy-driven and subject to eligibility/approval.\n\nQ: On-grid vs Hybrid which is better?\nA: On-grid is usually lower cost; hybrid adds backup support.",
        ],
        'on_grid_price_table' => solar_finance_default_on_grid_prices(),
        'hybrid_price_table' => solar_finance_default_hybrid_prices(),
    ];
}
...
function solar_finance_settings_load(): array
{
    ...
    if (!is_file($path)) {
        $encodedDefaults = json_encode($defaults, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    ...
    $cache = array_replace_recursive($defaults, $decoded);
    return $cache;
}

function solar_finance_settings_save(array $payload): bool
{
    $defaults = solar_finance_settings_default();
    ...
    $saved = file_put_contents(solar_finance_settings_path(), $encoded, LOCK_EX) !== false;
    return $saved;
}
