...
function customer_acceptance_record(array &$document,string $type,array $input,array $context=[]): array
{
    ...
    if ($type === 'quotation' && function_exists('documents_quote_resolve_important_points')) {
        $snapshot = documents_quote_resolve_important_points($document, ['important_points' => is_array($document['important_points_snapshot'] ?? null) ? $document['important_points_snapshot'] : []]);
        $document['customer_acceptance']['accepted_important_points'] = $snapshot;
        $document['customer_acceptance']['accepted_important_points_hash'] = hash('sha256', json_encode($snapshot, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE));
    }
