...
function customer_acceptance_check_submission(array &$document,string $type,array $input,string $token,array $context=[]): array
{
    ...
    if($type==='challan' && (string)($input['receipt_result']??'')==='issue' && trim((string)($input['remarks']??''))==='')return ['ok'=>false,'code'=>'remarks_required','message'=>'Remarks are required when reporting shortage, damage, or another issue.'];
...
function customer_acceptance_build_whatsapp_message(array $e,string $publicLink=''): string { $verb=$e['document_type']==='challan'?'confirm receipt of the items listed in':'confirm acceptance/review of'; $result=(string)($e['receipt_result']??''); return "I, {$e['customer_name_snapshot']}, {$verb} {$e['document_no']}.\nReference: {$e['acceptance_ref']}\nConfirmed on: {$e['confirmed_at']}".($result!==''?"\nReceipt result: {$result}":'')."\nDocument link: {$publicLink}\nRemarks: ".($e['customer_remarks']?:'None')."\nRegistered mobile ending: {$e['confirmed_mobile_last4']}"; }
...
function customer_acceptance_challan_item_summary(array $document): string { $parts=[]; if(function_exists('documents_challan_public_items')){ $rows=documents_challan_public_items($document); } else { $rows=(array)($document['items']??[]); } foreach($rows as $item){$name=trim((string)($item['name']??'')); if($name!=='')$parts[]=$name.' — '.(string)($item['qty']??'').' '.trim((string)($item['unit']??''));} return implode("\n",$parts); }
function customer_acceptance_challan_default_whatsapp_template(): string
{
    return "Namaste Dakshayani Enterprises,\n\nI, {customer_name}, confirm that I have received the items listed in Delivery Challan {challan_no}, version {challan_version}.\n\nQuotation reference: {quotation_no}\nDispatch Advice reference: {dispatch_advice_no}\nDelivery date: {delivery_date}\nDelivery location: {delivery_address}\nNumber of listed items: {item_count}\n\nItems received:\n{item_summary}\n\nCustomer remarks:\n{customer_remarks}\nReceipt confirmation reference: {acceptance_ref}\nConfirmed on: {confirmed_at}\nRegistered mobile: {customer_mobile_mask}\n\nSecure Challan link:\n{public_link}\n\nRegards,\n{customer_name}";
}
function customer_acceptance_challan_template(array $document,array $e,array $company,string $publicLink,string $template): array
{
    $customer=(array)($document['customer_snapshot']??[]); $items=function_exists('documents_challan_public_items')?documents_challan_public_items($document):(array)($document['items']??[]);
    $values=['customer_name'=>(string)($customer['name']??$document['customer_name_snapshot']??''),'customer_mobile_mask'=>customer_acceptance_mask_mobile((string)($document['customer_mobile']??$customer['mobile']??'')),'challan_no'=>(string)($document['challan_no']??$document['dc_number']??''),'challan_version'=>(string)($document['version_no']??$document['revision_no']??1),'quotation_no'=>(string)($document['linked_quote_no']??$document['quotation_no']??''),'agreement_no'=>(string)($document['agreement_no']??''),'dispatch_advice_no'=>(string)($document['dispatch_advice_no']??''),'delivery_date'=>(string)($document['delivery_date']??''),'delivery_address'=>(string)($document['delivery_address']??$customer['address']??''),'item_count'=>(string)count($items),'item_summary'=>customer_acceptance_challan_item_summary($document),'customer_remarks'=>(string)(($e['customer_remarks']??'')?:'Customer confirmed receipt with no remarks'),'acceptance_ref'=>(string)($e['acceptance_ref']??''),'confirmed_at'=>(string)($e['confirmed_at']??''),'public_link'=>$publicLink,'company_name'=>(string)($company['brand_name']??$company['company_name']??'Dakshayani Enterprises'),'company_phone'=>(string)($company['phone_primary']??''),'company_whatsapp'=>'7070278178'];
    $message=preg_replace_callback('/\{([a-z_]+)\}/',static fn($m)=>array_key_exists($m[1],$values)?$values[$m[1]]:$m[0],$template)??$template; preg_match_all('/\{[a-z_]+\}/',$message,$matches); return ['message'=>$message,'unresolved'=>array_values(array_unique($matches[0]))];
}
function customer_acceptance_challan_whatsapp_payload(array $document, array $company=[]): array { $publicLink=customer_acceptance_challan_public_link((string)($document['public_token']??'')); return customer_acceptance_challan_template($document,(array)($document['customer_acceptance']??$document['customer_receipt']??[]),$company,$publicLink,customer_acceptance_challan_default_whatsapp_template()); }
function customer_acceptance_challan_whatsapp_url(array $document, array $company=[]): string { $built=customer_acceptance_challan_whatsapp_payload($document,$company); return 'https://wa.me/'.CUSTOMER_ACCEPTANCE_WHATSAPP_TARGET.'?text='.rawurlencode($built['message']); }
