...
function business_pulse_accepted_rows(): array {
    $receiptsByQuote=[]; foreach(documents_list_sales_documents('receipt') as $r){ if(!is_array($r)||documents_is_archived($r)||strtolower(trim((string)($r['status']??'')))!=='final') continue; $qid=(string)($r['quotation_id']??''); if($qid!=='') $receiptsByQuote[$qid][]=$r; }
    $rows=[]; foreach(documents_list_quotes() as $q){ if(!is_array($q)||!business_pulse_is_current_accepted_quote($q)) continue; $qid=(string)($q['id']??''); $amount=business_pulse_quote_amount($q); $received=0.0; $lastPay=''; foreach($receiptsByQuote[$qid]??[] as $r){ $received+=(float)($r['amount_rs']??$r['amount_received']??$r['amount']??0); $d=business_pulse_date($r,['date_received','receipt_date','created_at']); if($d>$lastPay)$lastPay=$d; } $due=$amount-$received; $rows[]=['quote'=>$q,'qid'=>$qid,'amount'=>$amount,'received'=>$received,'due'=>$due,'pct'=>business_pulse_collection_pct($received,$amount),'accepted_date'=>business_pulse_date($q),'last_payment_date'=>$lastPay]; }
    return $rows;
}
...
function business_pulse_summary(): array {
    ...
    foreach($sources['invoices'] as $inv){ if(!is_array($inv)||documents_is_archived($inv)) continue; $d=business_pulse_date($inv,['invoice_date','created_at']); if($d!==''&&$d>=$fyStart){ $invoicedFy+=(float)($inv['calc']['grand_total']??$inv['calc']['gross_payable']??$inv['input_total_gst_inclusive']??0); } }
