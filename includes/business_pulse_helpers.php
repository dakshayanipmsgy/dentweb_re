...
function business_pulse_summary(): array {
    ...
    foreach(documents_all_invoices() as $inv){ if(!is_array($inv)||documents_is_archived($inv)||!documents_invoice_is_finalized($inv)) continue; $d=business_pulse_date($inv,['invoice_date','finalized_at','created_at']); if($d!==''&&$d>=$fyStart){ $invoicedFy+=documents_invoice_final_total($inv); } }
