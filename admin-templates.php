...
<div class="grid">
<?php foreach (documents_template_block_defaults() as $key => $_): ?><div style="grid-column:1/-1"><label><?= htmlspecialchars($key === 'quotation_cover_note_html' ? 'Cover Note' : ucwords(str_replace('_', ' ', $key)), ENT_QUOTES) ?></label><textarea name="block_<?= htmlspecialchars($key, ENT_QUOTES) ?>"><?= htmlspecialchars((string)($selectedBlocks['blocks'][$key] ?? ''), ENT_QUOTES) ?></textarea></div><?php endforeach; ?>
