...
$editingFinanceInputs = (is_array($editing) && is_array($editing['finance_inputs'] ?? null)) ? $editing['finance_inputs'] : [];
$editingLoanInputs = is_array($editingFinanceInputs['loan'] ?? null) ? $editingFinanceInputs['loan'] : [];
$savedUnitRateValue = trim((string) ($editingFinanceInputs['unit_rate_rs_per_kwh'] ?? ''));
$savedAnnualGenerationValue = trim((string) ($editingFinanceInputs['annual_generation_per_kw'] ?? ''));
$unitRateFormValue = $savedUnitRateValue !== '' ? $savedUnitRateValue : (string) ($segmentDefaults['unit_rate_rs_per_kwh'] ?? '');
$annualGenerationFormValue = $savedAnnualGenerationValue !== '' ? $savedAnnualGenerationValue : (string) ($quoteDefaults['global']['energy_defaults']['annual_generation_per_kw'] ?? '');
...
<div><label>Unit rate (₹/kWh)</label><input type="number" step="0.01" name="unit_rate_rs_per_kwh" value="<?= htmlspecialchars($unitRateFormValue, ENT_QUOTES) ?>"></div>
<div><label>Annual generation per kW</label><input type="number" step="0.01" name="annual_generation_per_kw" value="<?= htmlspecialchars($annualGenerationFormValue, ENT_QUOTES) ?>"></div>
...
<div><label>Loan amount ₹</label><input type="number" step="0.01" name="loan_amount" value="<?= htmlspecialchars((string)($editingLoanInputs['loan_amount'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label><input type="checkbox" name="loan_enabled" <?= !empty($editingLoanInputs['enabled']) ? 'checked' : '' ?>> Loan enabled</label></div>
<div><label>Loan interest %</label><input type="number" step="0.01" name="loan_interest_pct" value="<?= htmlspecialchars((string)($editingLoanInputs['interest_pct'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Loan tenure years</label><input type="number" step="1" name="loan_tenure_years" value="<?= htmlspecialchars((string)($editingLoanInputs['tenure_years'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Margin money ₹</label><input type="number" step="0.01" name="loan_margin_pct" value="<?= htmlspecialchars((string)($editingLoanInputs['margin_pct'] ?? ''), ENT_QUOTES) ?>"><div class="muted"><a href="#" id="resetLoanDefaults">Reset to defaults</a></div></div>
