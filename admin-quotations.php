...
$editingLoanInputs = is_array($editing['finance_inputs']['loan'] ?? null) ? $editing['finance_inputs']['loan'] : [];
$hasSavedLoanAmount = array_key_exists('loan_amount', $editingLoanInputs) && trim((string) $editingLoanInputs['loan_amount']) !== '';
$hasSavedLoanInterest = array_key_exists('interest_pct', $editingLoanInputs) && trim((string) $editingLoanInputs['interest_pct']) !== '';
$hasSavedLoanTenureYears = array_key_exists('tenure_years', $editingLoanInputs) && trim((string) $editingLoanInputs['tenure_years']) !== '';
$hasSavedMarginMoney = array_key_exists('margin_pct', $editingLoanInputs) && trim((string) $editingLoanInputs['margin_pct']) !== '';
...
<div><label>Loan amount ₹</label><input type="number" step="0.01" name="loan_amount" value="<?= htmlspecialchars((string)($editingLoanInputs['loan_amount'] ?? ''), ENT_QUOTES) ?>" data-has-saved-value="<?= $hasSavedLoanAmount ? '1' : '0' ?>"></div>
<div><label><input type="checkbox" name="loan_enabled" <?= !empty($editing['finance_inputs']['loan']['enabled']) ? 'checked' : '' ?>> Loan enabled</label></div>
<div><label>Loan interest %</label><input type="number" step="0.01" name="loan_interest_pct" value="<?= htmlspecialchars((string)($editingLoanInputs['interest_pct'] ?? ''), ENT_QUOTES) ?>" data-has-saved-value="<?= $hasSavedLoanInterest ? '1' : '0' ?>"></div>
<div><label>Loan tenure years</label><input type="number" step="1" name="loan_tenure_years" value="<?= htmlspecialchars((string)($editingLoanInputs['tenure_years'] ?? ''), ENT_QUOTES) ?>" data-has-saved-value="<?= $hasSavedLoanTenureYears ? '1' : '0' ?>"></div>
<div><label>Margin money ₹</label><input type="number" step="0.01" name="loan_margin_pct" value="<?= htmlspecialchars((string)($editingLoanInputs['margin_pct'] ?? ''), ENT_QUOTES) ?>" data-has-saved-value="<?= $hasSavedMarginMoney ? '1' : '0' ?>"><div class="muted"><a href="#" id="resetLoanDefaults">Reset to defaults</a></div></div>
