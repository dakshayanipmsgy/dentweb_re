...
<?php
$editingFinanceUnitRate = (string) ($editing['finance_inputs']['unit_rate_rs_per_kwh'] ?? '');
$editingFinanceAnnualGeneration = (string) ($editing['finance_inputs']['annual_generation_per_kw'] ?? '');
$resolvedUnitRateValue = trim($editingFinanceUnitRate) !== '' ? $editingFinanceUnitRate : (string) ($segmentDefaults['unit_rate_rs_per_kwh'] ?? '');
$resolvedAnnualGenerationValue = trim($editingFinanceAnnualGeneration) !== '' ? $editingFinanceAnnualGeneration : (string) ($quoteDefaults['global']['energy_defaults']['annual_generation_per_kw'] ?? '');
?>
<div><label>Unit rate (₹/kWh)</label><input type="number" step="0.01" name="unit_rate_rs_per_kwh" value="<?= htmlspecialchars($resolvedUnitRateValue, ENT_QUOTES) ?>"></div>
<div><label>Annual generation per kW</label><input type="number" step="0.01" name="annual_generation_per_kw" value="<?= htmlspecialchars($resolvedAnnualGenerationValue, ENT_QUOTES) ?>"></div>
