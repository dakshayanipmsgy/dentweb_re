...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_settings') {
        ...
        $sections = documents_detailed_proposal_default_sections();
        $currentSections = is_array($d['global']['quotation_sections'] ?? null) ? $d['global']['quotation_sections'] : [];
        $sections = array_replace_recursive($sections, $currentSections);
        $sections['display_style'] = safe_text($_POST['proposal_display_style'] ?? ($sections['display_style'] ?? 'compact')) === 'detailed' ? 'detailed' : 'compact';
        $boolKeys = ['cover','project_snapshot','system_configuration','system_working','scope_matrix','technical_assumptions','exclusions','safety_compliance','commercial_offer','payment_terms','warranty_om','project_timeline','next_steps','bank_details','acceptance'];
        foreach ($boolKeys as $key) {
            $sections[$key]['show'] = isset($_POST['section_' . $key . '_show']);
        }
        $textMap = [
            'cover' => ['title','subtitle','intro','prepared_by','validity_wording'],
            'project_snapshot' => ['title','intro'],
            'system_configuration' => ['title','intro','on_grid_points','hybrid_points'],
            'system_working' => ['on_grid_title','on_grid_html','hybrid_title','hybrid_html','diagram_url'],
            'scope_matrix' => ['title','intro'],
            'technical_assumptions' => ['title','intro','points'],
            'exclusions' => ['title','intro','points'],
            'safety_compliance' => ['title','intro'],
            'commercial_offer' => ['title','notes'],
            'payment_terms' => ['title','fallback_html'],
            'warranty_om' => ['title','intro','points'],
            'project_timeline' => ['title','intro','milestones'],
            'next_steps' => ['title','points'],
            'bank_details' => ['title','intro','qr_image_url'],
            'acceptance' => ['title','wording'],
        ];
        foreach ($textMap as $sectionKey => $fieldNames) {
            foreach ($fieldNames as $fieldName) {
                $postKey = 'section_' . $sectionKey . '_' . $fieldName;
                if (array_key_exists($postKey, $_POST)) {
                    $sections[$sectionKey][$fieldName] = trim((string) $_POST[$postKey]);
                }
            }
        }
        foreach (['scope_matrix_rows_json' => ['scope_matrix','rows'], 'safety_rows_json' => ['safety_compliance','rows']] as $postKey => $target) {
            $rawJson = trim((string) ($_POST[$postKey] ?? ''));
            if ($rawJson !== '') {
                $decodedRows = json_decode($rawJson, true);
                if (is_array($decodedRows)) {
                    $sections[$target[0]][$target[1]] = array_values($decodedRows);
                }
            }
        }
        $d['global']['quotation_sections'] = $sections;
    ...
    if ($action === 'save_quote') {
        ...
        $styleOverride = safe_text($_POST['proposal_display_style'] ?? '');
        $quote['proposal_display_style'] = in_array($styleOverride, ['', 'compact', 'detailed'], true) ? $styleOverride : '';
        $quote['proposal_show_technical_sections'] = isset($_POST['proposal_show_technical_sections']);
        $quote['proposal_show_scope_matrix'] = isset($_POST['proposal_show_scope_matrix']);
        $quote['proposal_show_safety_section'] = isset($_POST['proposal_show_safety_section']);
        $quote['proposal_show_bank_details'] = isset($_POST['proposal_show_bank_details']);
        $quote['proposal_show_acceptance'] = isset($_POST['proposal_show_acceptance']);
...
<div><label>Quotation display style</label><select name="proposal_display_style"><option value="" <?= (($editing['proposal_display_style'] ?? '')==='')?'selected':'' ?>>Use global default</option><option value="compact" <?= (($editing['proposal_display_style'] ?? '')==='compact')?'selected':'' ?>>Compact Customer Quotation</option><option value="detailed" <?= (($editing['proposal_display_style'] ?? '')==='detailed')?'selected':'' ?>>Detailed Techno-Commercial Proposal</option></select></div>
<div style="grid-column:1/-1"><label><input type="checkbox" name="proposal_show_technical_sections" <?= !empty($editing['proposal_show_technical_sections']) ? 'checked' : '' ?>> Show detailed technical sections</label> <label><input type="checkbox" name="proposal_show_scope_matrix" <?= !empty($editing['proposal_show_scope_matrix']) ? 'checked' : '' ?>> Show scope matrix</label> <label><input type="checkbox" name="proposal_show_safety_section" <?= !empty($editing['proposal_show_safety_section']) ? 'checked' : '' ?>> Show safety section</label> <label><input type="checkbox" name="proposal_show_bank_details" <?= !empty($editing['proposal_show_bank_details']) ? 'checked' : '' ?>> Show bank details</label> <label><input type="checkbox" name="proposal_show_acceptance" <?= !empty($editing['proposal_show_acceptance']) ? 'checked' : '' ?>> Show acceptance/signature</label></div>
...
<?php if ($tab === "settings"): $d = $quoteDefaults; ?><div class="card"><h2>Quotation Settings</h2><form method="post" class="grid"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ""), ENT_QUOTES) ?>"><input type="hidden" name="action" value="save_settings"><div><label>Primary color</label><div style="display:flex;gap:6px"><input type="color" name="ui_primary" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['primary'] ?? "#0ea5e9"), ENT_QUOTES) ?>"><input name="ui_primary_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['primary'] ?? "#0ea5e9"), ENT_QUOTES) ?>"></div></div><div><label>Accent color</label><div style="display:flex;gap:6px"><input type="color" name="ui_accent" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['accent'] ?? "#22c55e"), ENT_QUOTES) ?>"><input name="ui_accent_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['accent'] ?? "#22c55e"), ENT_QUOTES) ?>"></div></div><div><label>Text color</label><div style="display:flex;gap:6px"><input type="color" name="ui_text" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['text'] ?? "#0f172a"), ENT_QUOTES) ?>"><input name="ui_text_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['text'] ?? "#0f172a"), ENT_QUOTES) ?>"></div></div><div><label>Muted text color</label><div style="display:flex;gap:6px"><input type="color" name="ui_muted_text" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['muted_text'] ?? "#475569"), ENT_QUOTES) ?>"><input name="ui_muted_text_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['muted_text'] ?? "#475569"), ENT_QUOTES) ?>"></div></div><div><label>Page background color</label><div style="display:flex;gap:6px"><input type="color" name="ui_page_bg" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['page_bg'] ?? "#f8fafc"), ENT_QUOTES) ?>"><input name="ui_page_bg_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['page_bg'] ?? "#f8fafc"), ENT_QUOTES) ?>"></div></div><div><label>Card background color</label><div style="display:flex;gap:6px"><input type="color" name="ui_card_bg" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['card_bg'] ?? "#ffffff"), ENT_QUOTES) ?>"><input name="ui_card_bg_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['card_bg'] ?? "#ffffff"), ENT_QUOTES) ?>"></div></div><div><label>Border color</label><div style="display:flex;gap:6px"><input type="color" name="ui_border" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['border'] ?? "#e2e8f0"), ENT_QUOTES) ?>"><input name="ui_border_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['colors']['border'] ?? "#e2e8f0"), ENT_QUOTES) ?>"></div></div><div><label>Header gradient A</label><div style="display:flex;gap:6px"><input type="color" name="header_gradient_a" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['header']['a'] ?? "#0ea5e9"), ENT_QUOTES) ?>"><input name="header_gradient_a_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['header']['a'] ?? "#0ea5e9"), ENT_QUOTES) ?>"></div></div><div><label>Header gradient B</label><div style="display:flex;gap:6px"><input type="color" name="header_gradient_b" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['header']['b'] ?? "#22c55e"), ENT_QUOTES) ?>"><input name="header_gradient_b_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['header']['b'] ?? "#22c55e"), ENT_QUOTES) ?>"></div></div><div><label>Footer gradient A</label><div style="display:flex;gap:6px"><input type="color" name="footer_gradient_a" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['footer']['a'] ?? "#0ea5e9"), ENT_QUOTES) ?>"><input name="footer_gradient_a_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['footer']['a'] ?? "#0ea5e9"), ENT_QUOTES) ?>"></div></div><div><label>Footer gradient B</label><div style="display:flex;gap:6px"><input type="color" name="footer_gradient_b" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['footer']['b'] ?? "#22c55e"), ENT_QUOTES) ?>"><input name="footer_gradient_b_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['gradients']['footer']['b'] ?? "#22c55e"), ENT_QUOTES) ?>"></div></div><div><label>Header font color</label><div style="display:flex;gap:6px"><input type="color" name="header_text_color" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['header_footer']['header_text_color'] ?? "#ffffff"), ENT_QUOTES) ?>"><input name="header_text_color_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['header_footer']['header_text_color'] ?? "#ffffff"), ENT_QUOTES) ?>"></div></div><div><label>Footer font color</label><div style="display:flex;gap:6px"><input type="color" name="footer_text_color" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['header_footer']['footer_text_color'] ?? "#ffffff"), ENT_QUOTES) ?>"><input name="footer_text_color_hex" value="<?= htmlspecialchars((string)($d['global']['ui_tokens']['header_footer']['footer_text_color'] ?? "#ffffff"), ENT_QUOTES) ?>"></div></div><div><label>Header gradient direction</label><select name="header_gradient_direction"><option value="to right">left→right</option><option value="to bottom">top→bottom</option></select></div><div><label><input type="checkbox" name="header_gradient_enabled" <?= !empty($d['global']['ui_tokens']['gradients']['header']['enabled'])?'checked':'' ?>> Enable header gradient</label></div><div><label>Footer gradient direction</label><select name="footer_gradient_direction"><option value="to right">left→right</option><option value="to bottom">top→bottom</option></select></div><div><label><input type="checkbox" name="footer_gradient_enabled" <?= !empty($d['global']['ui_tokens']['gradients']['footer']['enabled'])?'checked':'' ?>> Enable footer gradient</label></div><div><label>Default interest rate (%)</label><input type="number" step="0.01" name="res_interest_pct" value="<?= htmlspecialchars((string)($d['segments']['RES']['loan_bestcase']['interest_pct'] ?? 6), ENT_QUOTES) ?>"></div><div><label>Default loan tenure (years)</label><input type="number" name="res_tenure_years" value="<?= htmlspecialchars((string)($d['segments']['RES']['loan_bestcase']['tenure_years'] ?? 10), ENT_QUOTES) ?>"></div><div><label>Annual generation per kW</label><input type="number" step="0.01" name="annual_generation_per_kw" value="<?= htmlspecialchars((string)($d['global']['energy_defaults']['annual_generation_per_kw'] ?? 1450), ENT_QUOTES) ?>"></div><div><label>Emission factor (kg CO2/kWh)</label><input type="number" step="0.01" name="emission_factor_kg_per_kwh" value="<?= htmlspecialchars((string)($d['global']['energy_defaults']['emission_factor_kg_per_kwh'] ?? 0.82), ENT_QUOTES) ?>"></div><div><label>CO2 absorbed per tree per year (kg)</label><input type="number" step="0.01" name="tree_absorption_kg_per_tree_per_year" value="<?= htmlspecialchars((string)($d['global']['energy_defaults']['tree_absorption_kg_per_tree_per_year'] ?? 20), ENT_QUOTES) ?>"></div><div><label><input type="checkbox" name="show_decimals" <?= !empty($d['global']['quotation_ui']['show_decimals'])?'checked':'' ?>> Show INR decimals in quotation</label></div><div><label>QR target</label><select name="qr_target"><option value="quotation" <?= (($d['global']['quotation_ui']['qr_target'] ?? "quotation")==="quotation")?'selected':'' ?>>This quotation link</option><option value="website" <?= (($d['global']['quotation_ui']['qr_target'] ?? "quotation")==="website")?'selected':'' ?>>Company website</option></select></div><div style="grid-column:1/-1"><label>Why Dakshayani points (one per line)</label><textarea name="why_dakshayani_points"><?= htmlspecialchars(implode("\n", (array)($d['global']['quotation_ui']['why_dakshayani_points'] ?? [])), ENT_QUOTES) ?></textarea></div><div style="grid-column:1/-1"><label>Footer disclaimer</label><textarea name="footer_disclaimer"><?= htmlspecialchars((string)($d['global']['quotation_ui']['footer_disclaimer'] ?? ''), ENT_QUOTES) ?></textarea></div><div style="grid-column:1/-1"><label>WhatsApp quotation message template</label><textarea name="whatsapp_message_template" placeholder="Use placeholders like {{name}}, {{city}}, {{system_size}}, {{system_type}}, {{price}}, {{quotation_link}}"><?= htmlspecialchars((string)($d['global']['quotation_ui']['whatsapp_message_template'] ?? $quotationDefaultWhatsappTemplate), ENT_QUOTES) ?></textarea><div class="muted">Supported placeholders: {{name}}, {{city}}, {{system_size}}, {{system_type}}, {{price}}, {{quotation_link}}</div></div><?php $sections = array_replace_recursive(documents_detailed_proposal_default_sections(), is_array($d['global']['quotation_sections'] ?? null) ? $d['global']['quotation_sections'] : []); ?>
<div style="grid-column:1/-1"><h3>Detailed Proposal Sections</h3><div class="muted">These defaults power the premium customer-facing techno-commercial proposal. Use JSON fields for editable matrix/table rows.</div></div>
<div><label>Quotation display style</label><select name="proposal_display_style"><option value="compact" <?= (($sections['display_style'] ?? 'compact')==='compact')?'selected':'' ?>>Compact Customer Quotation</option><option value="detailed" <?= (($sections['display_style'] ?? 'compact')==='detailed')?'selected':'' ?>>Detailed Techno-Commercial Proposal</option></select></div>
<div><label><input type="checkbox" name="section_cover_show" <?= !empty($sections['cover']['show'])?'checked':'' ?>> Show Proposal Cover / Intro</label></div>
<div><label><input type="checkbox" name="section_project_snapshot_show" <?= !empty($sections['project_snapshot']['show'])?'checked':'' ?>> Show Project Snapshot</label></div>
<div><label><input type="checkbox" name="section_system_configuration_show" <?= !empty($sections['system_configuration']['show'])?'checked':'' ?>> Show System Configuration</label></div>
<div><label><input type="checkbox" name="section_system_working_show" <?= !empty($sections['system_working']['show'])?'checked':'' ?>> Show How System Works</label></div>
<div><label><input type="checkbox" name="section_scope_matrix_show" <?= !empty($sections['scope_matrix']['show'])?'checked':'' ?>> Show EPC Scope Matrix</label></div>
<div><label><input type="checkbox" name="section_technical_assumptions_show" <?= !empty($sections['technical_assumptions']['show'])?'checked':'' ?>> Show Technical Assumptions</label></div>
<div><label><input type="checkbox" name="section_exclusions_show" <?= !empty($sections['exclusions']['show'])?'checked':'' ?>> Show Exclusions</label></div>
<div><label><input type="checkbox" name="section_safety_compliance_show" <?= !empty($sections['safety_compliance']['show'])?'checked':'' ?>> Show Safety & Compliance</label></div>
<div><label><input type="checkbox" name="section_commercial_offer_show" <?= !empty($sections['commercial_offer']['show'])?'checked':'' ?>> Show Commercial Offer</label></div>
<div><label><input type="checkbox" name="section_payment_terms_show" <?= !empty($sections['payment_terms']['show'])?'checked':'' ?>> Show Payment Terms fallback</label></div>
<div><label><input type="checkbox" name="section_warranty_om_show" <?= !empty($sections['warranty_om']['show'])?'checked':'' ?>> Show Warranty/O&M</label></div>
<div><label><input type="checkbox" name="section_project_timeline_show" <?= !empty($sections['project_timeline']['show'])?'checked':'' ?>> Show Project Timeline</label></div>
<div><label><input type="checkbox" name="section_next_steps_show" <?= !empty($sections['next_steps']['show'])?'checked':'' ?>> Show Next Steps</label></div>
<div><label><input type="checkbox" name="section_bank_details_show" <?= !empty($sections['bank_details']['show'])?'checked':'' ?>> Show Bank/Payment Details</label></div>
<div><label><input type="checkbox" name="section_acceptance_show" <?= !empty($sections['acceptance']['show'])?'checked':'' ?>> Show Acceptance/Signature</label></div>
<div style="grid-column:1/-1"><label>Cover title</label><input name="section_cover_title" value="<?= htmlspecialchars((string)($sections['cover']['title'] ?? ''), ENT_QUOTES) ?>"></div>
<div style="grid-column:1/-1"><label>Cover subtitle</label><input name="section_cover_subtitle" value="<?= htmlspecialchars((string)($sections['cover']['subtitle'] ?? ''), ENT_QUOTES) ?>"></div>
<div style="grid-column:1/-1"><label>Cover intro paragraph</label><textarea name="section_cover_intro"><?= htmlspecialchars((string)($sections['cover']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div><label>Prepared-by text</label><input name="section_cover_prepared_by" value="<?= htmlspecialchars((string)($sections['cover']['prepared_by'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Proposal validity wording</label><input name="section_cover_validity_wording" value="<?= htmlspecialchars((string)($sections['cover']['validity_wording'] ?? ''), ENT_QUOTES) ?>"></div>
<?php foreach ([['project_snapshot','Project Snapshot'],['system_configuration','System Configuration'],['scope_matrix','EPC Scope Matrix'],['technical_assumptions','Technical Assumptions'],['exclusions','Exclusions'],['safety_compliance','Safety & Compliance'],['commercial_offer','Commercial Offer'],['payment_terms','Payment Terms'],['warranty_om','Warranty & O&M'],['project_timeline','Project Timeline'],['next_steps','Next Steps'],['bank_details','Bank / Payment Details'],['acceptance','Acceptance / Signature']] as $secMeta): $secKey=$secMeta[0]; ?>
<div style="grid-column:1/-1"><label><?= htmlspecialchars($secMeta[1], ENT_QUOTES) ?> title</label><input name="section_<?= htmlspecialchars($secKey, ENT_QUOTES) ?>_title" value="<?= htmlspecialchars((string)($sections[$secKey]['title'] ?? ''), ENT_QUOTES) ?>"></div>
<?php endforeach; ?>
<div style="grid-column:1/-1"><label>Project Snapshot intro</label><textarea name="section_project_snapshot_intro"><?= htmlspecialchars((string)($sections['project_snapshot']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>EPC Scope Matrix intro</label><textarea name="section_scope_matrix_intro"><?= htmlspecialchars((string)($sections['scope_matrix']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Technical assumptions intro</label><textarea name="section_technical_assumptions_intro"><?= htmlspecialchars((string)($sections['technical_assumptions']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Exclusions intro</label><textarea name="section_exclusions_intro"><?= htmlspecialchars((string)($sections['exclusions']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Safety & Compliance intro</label><textarea name="section_safety_compliance_intro"><?= htmlspecialchars((string)($sections['safety_compliance']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Project Timeline intro</label><textarea name="section_project_timeline_intro"><?= htmlspecialchars((string)($sections['project_timeline']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>System configuration intro</label><textarea name="section_system_configuration_intro"><?= htmlspecialchars((string)($sections['system_configuration']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Default on-grid configuration points (one per line)</label><textarea name="section_system_configuration_on_grid_points"><?= htmlspecialchars((string)($sections['system_configuration']['on_grid_points'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Default hybrid configuration points (one per line)</label><textarea name="section_system_configuration_hybrid_points"><?= htmlspecialchars((string)($sections['system_configuration']['hybrid_points'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>On-grid explainer title</label><input name="section_system_working_on_grid_title" value="<?= htmlspecialchars((string)($sections['system_working']['on_grid_title'] ?? ''), ENT_QUOTES) ?>"></div>
<div style="grid-column:1/-1"><label>On-grid explainer HTML</label><textarea name="section_system_working_on_grid_html"><?= htmlspecialchars((string)($sections['system_working']['on_grid_html'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Hybrid explainer title</label><input name="section_system_working_hybrid_title" value="<?= htmlspecialchars((string)($sections['system_working']['hybrid_title'] ?? ''), ENT_QUOTES) ?>"></div>
<div style="grid-column:1/-1"><label>Hybrid explainer HTML</label><textarea name="section_system_working_hybrid_html"><?= htmlspecialchars((string)($sections['system_working']['hybrid_html'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Optional diagram image URL</label><input name="section_system_working_diagram_url" value="<?= htmlspecialchars((string)($sections['system_working']['diagram_url'] ?? ''), ENT_QUOTES) ?>"></div>
<div style="grid-column:1/-1"><label>EPC Scope Matrix rows (JSON)</label><textarea name="scope_matrix_rows_json"><?= htmlspecialchars(json_encode((array)($sections['scope_matrix']['rows'] ?? []), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Technical assumptions bullet points</label><textarea name="section_technical_assumptions_points"><?= htmlspecialchars((string)($sections['technical_assumptions']['points'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Exclusions bullet points</label><textarea name="section_exclusions_points"><?= htmlspecialchars((string)($sections['exclusions']['points'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Safety rows (JSON)</label><textarea name="safety_rows_json"><?= htmlspecialchars(json_encode((array)($sections['safety_compliance']['rows'] ?? []), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Commercial offer notes</label><textarea name="section_commercial_offer_notes"><?= htmlspecialchars((string)($sections['commercial_offer']['notes'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Payment terms fallback HTML</label><textarea name="section_payment_terms_fallback_html"><?= htmlspecialchars((string)($sections['payment_terms']['fallback_html'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Warranty/O&M intro</label><textarea name="section_warranty_om_intro"><?= htmlspecialchars((string)($sections['warranty_om']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Warranty/O&M points</label><textarea name="section_warranty_om_points"><?= htmlspecialchars((string)($sections['warranty_om']['points'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Project timeline milestones</label><textarea name="section_project_timeline_milestones"><?= htmlspecialchars((string)($sections['project_timeline']['milestones'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Next steps points</label><textarea name="section_next_steps_points"><?= htmlspecialchars((string)($sections['next_steps']['points'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Bank details intro</label><textarea name="section_bank_details_intro"><?= htmlspecialchars((string)($sections['bank_details']['intro'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Bank QR image URL</label><input name="section_bank_details_qr_image_url" value="<?= htmlspecialchars((string)($sections['bank_details']['qr_image_url'] ?? ''), ENT_QUOTES) ?>"></div>
<div style="grid-column:1/-1"><label>Acceptance wording</label><textarea name="section_acceptance_wording"><?= htmlspecialchars((string)($sections['acceptance']['wording'] ?? ''), ENT_QUOTES) ?></textarea></div>
<div style="grid-column:1/-1"><label>Rate chart — On-Grid (JSON)</label><textarea name="rate_chart_on_grid_json"><?= htmlspecialchars(json_encode((array)($d['rate_chart']['on_grid'] ?? []), JSON_PRETTY_PRINT), ENT_QUOTES) ?></textarea></div><div style="grid-column:1/-1"><label>Rate chart — Hybrid (JSON)</label><textarea name="rate_chart_hybrid_json"><?= htmlspecialchars(json_encode((array)($d['rate_chart']['hybrid'] ?? []), JSON_PRETTY_PRINT), ENT_QUOTES) ?></textarea></div><div style="grid-column:1/-1"><button class="btn" type="submit">Save Settings</button></div></form></div><?php endif; ?>
...
(function(){
  ...
  if(editorCard){
    ...
    if(grid){
      const sections=[
        {title:'Section 1 — Basic Details', names:['Template Set','Party Type','Mobile','Name']},
        {title:'Section 2 — System & Quotation Basics', names:['System Type','Capacity kWp','Quotation Date','Valid Until','Pricing Mode','Tax Profile']},
        ...
        {title:'Section 7 — Funding Scenario Financial Inputs', names:['Selected Primary Scenario Price (including GST) ₹','Finance mode','Margin %','Loan %','Margin ₹','Loan ₹','Interest %','Tenure years'], containsHeadings:['Section 7 — Funding Scenario Financial Inputs','Loan up to ₹2 lacs','Loan above ₹2 lacs'], className:'funding-inputs'},
        {title:'Section 8 — Hybrid Configuration', names:['Hybrid Inverter (kVA)','Hybrid Phase','Hybrid Battery Count'], className:'hybrid-config'},
        {title:'Section 9 — Annexure Overrides', names:['Cover Notes','System Inclusions','Payment Terms','Warranty','System Type Explainer','Transportation','Terms & Conditions','PM Subsidy Info','Completion Milestones','Next Steps','Base font px','Heading scale','Density','Watermark image path'], containsHeadings:['Annexure Overrides','Typography & Watermark Overrides']},
      ];
      ...
      if(lookupForm){
        ...
        heading.textContent='Section 1 — Customer Lookup & Basic Details';
