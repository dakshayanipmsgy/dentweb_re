...
require_once __DIR__ . '/includes/quotation_bulk_import.php';
...
if (safe_text((string) ($_GET['action'] ?? '')) === 'download_quote_csv_template') {
    if ((string) (current_user()['role_name'] ?? '') !== 'admin') { http_response_code(403); exit('Admin access required.'); }
    quotation_bulk_send_template();
}
if (safe_text((string) ($_GET['action'] ?? '')) === 'download_quote_reference_data') {
    if ((string) (current_user()['role_name'] ?? '') !== 'admin') { http_response_code(403); exit('Admin access required.'); }
    quotation_bulk_send_reference_data($templates, $quoteDefaults);
}
if (safe_text((string) ($_GET['action'] ?? '')) === 'download_quote_validation_results') {
    if ((string) (current_user()['role_name'] ?? '') !== 'admin') { http_response_code(403); exit('Admin access required.'); }
    $preview = is_array($_SESSION['quotation_bulk_preview'] ?? null) ? $_SESSION['quotation_bulk_preview'] : [];
    $rows = is_array($preview['rows'] ?? null) ? $preview['rows'] : [];
    $headers = ['row_number','import_key','customer','system_model','dcr','non_dcr','total','template','amount','item_count','status','messages'];
    $csvRows = array_map(static function (array $row): array { $row['messages'] = implode(' | ', (array) ($row['messages'] ?? [])); return $row; }, $rows);
    quotation_bulk_send_csv('quotation-validation-results.csv', $headers, $csvRows);
}
if (safe_text((string) ($_GET['action'] ?? '')) === 'download_quote_import_result') {
    if ((string) (current_user()['role_name'] ?? '') !== 'admin') { http_response_code(403); exit('Admin access required.'); }
    $batchId = safe_text((string) ($_GET['batch_id'] ?? '')); $found = [];
    foreach (quotation_bulk_recent_batches() as $batch) { if ((string) ($batch['batch_id'] ?? '') === $batchId) { $found = $batch; break; } }
    $headers = ['row_number','import_key','status','created_quote_id','created_quote_no','messages'];
    $rows = array_map(static function (array $row): array { $row['messages'] = implode(' | ', (array) ($row['messages'] ?? [])); return $row; }, (array) ($found['results'] ?? []));
    quotation_bulk_send_csv('quotation-import-result.csv', $headers, $rows);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'quotation_bulk_upload_csv') {
        if ((string) (current_user()['role_name'] ?? '') !== 'admin') { $redirectWith('error', 'Only administrators can import quotations.'); }
        $upload = quotation_bulk_read_csv_upload($_FILES['quotation_csv'] ?? null);
        if (!($upload['ok'] ?? false)) { header('Location: admin-quotations.php?' . http_build_query(['tab' => 'bulk', 'status' => 'error', 'message' => (string) ($upload['error'] ?? 'Upload failed.')])); exit; }
        $preview = quotation_bulk_validate_rows((array) $upload['rows'], $templates, $quoteDefaults, (string) $upload['filename']);
        $preview['filename'] = (string) $upload['filename'];
        $preview['created_at'] = date('c');
        $_SESSION['quotation_bulk_preview'] = $preview;
        header('Location: admin-quotations.php?' . http_build_query(['tab' => 'bulk', 'status' => 'success', 'message' => 'CSV validated. Review the preview before creating drafts.']));
        exit;
    }

    if ($action === 'quotation_bulk_create_valid') {
        if ((string) (current_user()['role_name'] ?? '') !== 'admin') { $redirectWith('error', 'Only administrators can import quotations.'); }
        $preview = is_array($_SESSION['quotation_bulk_preview'] ?? null) ? $_SESSION['quotation_bulk_preview'] : [];
        $rows = is_array($preview['rows'] ?? null) ? $preview['rows'] : [];
        if ($rows === []) { header('Location: admin-quotations.php?' . http_build_query(['tab' => 'bulk', 'status' => 'error', 'message' => 'No validated CSV preview is available.'])); exit; }
        $batchId = 'qbi_' . date('YmdHis') . '_' . bin2hex(random_bytes(3));
        $user = current_user(); $results = []; $created = 0; $failed = 0; $skipped = 0;
        foreach ($rows as $row) {
            $messages = (array) ($row['messages'] ?? []);
            if ((string) ($row['status'] ?? '') === 'Error' || in_array('Duplicate import key or row hash detected.', $messages, true)) { $skipped++; $results[] = ['row_number'=>$row['row_number']??'', 'import_key'=>$row['import_key']??'', 'row_hash'=>$row['row_hash']??'', 'status'=>'skipped', 'created_quote_id'=>'', 'created_quote_no'=>'', 'messages'=>[in_array('Duplicate import key or row hash detected.', $messages, true) ? 'Duplicate row was skipped.' : 'Invalid preview row was skipped.']]; continue; }
            $meta = ['batch_id'=>$batchId,'import_key'=>(string)($row['import_key']??''),'row_number'=>(int)($row['row_number']??0),'source_filename'=>(string)($row['filename']??($preview['filename']??'')),'row_hash'=>(string)($row['row_hash']??''),'imported_at'=>date('c'),'imported_by_id'=>(string)($user['id']??''),'imported_by_name'=>(string)($user['full_name']??'Admin')];
            $built = documents_build_quote_draft_from_input((array) ($row['input'] ?? []), ['dry_run'=>false,'templates'=>$templates,'quote_defaults'=>$quoteDefaults,'import_meta'=>$meta]);
            if ($built['ok'] ?? false) { $created++; $q = (array) $built['quote']; $results[] = $meta + ['status'=>'created','created_quote_id'=>(string)($q['id']??''),'created_quote_no'=>(string)($q['quote_no']??''),'messages'=>$messages]; }
            else { $failed++; $results[] = $meta + ['status'=>'failed','created_quote_id'=>'','created_quote_no'=>'','messages'=>(array)($built['messages']??['Failed to create quotation.'])]; }
        }
        quotation_bulk_append_batch(['batch_id'=>$batchId,'source_filename'=>(string)($preview['filename']??''),'created_at'=>date('c'),'imported_by'=>(string)($user['full_name']??'Admin'),'totals'=>['total'=>count($rows),'valid'=>(int)($preview['counts']['valid']??0),'invalid'=>(int)($preview['counts']['invalid']??0),'created'=>$created,'failed'=>$failed,'skipped'=>$skipped],'results'=>$results]);
        unset($_SESSION['quotation_bulk_preview']);
        header('Location: admin-quotations.php?' . http_build_query(['tab' => 'bulk', 'status' => 'success', 'message' => 'Bulk import finished: created ' . $created . ', failed ' . $failed . ', skipped ' . $skipped . '.', 'batch_id'=>$batchId]));
        exit;
    }
    ...
    if ($action === 'save_settings') {
        ...
        $importantRaw = is_array($_POST['important_points'] ?? null) ? $_POST['important_points'] : [];
        $importantPoints = [];
        foreach ($importantRaw as $idx => $row) {
            if (!is_array($row)) { continue; }
            $text = trim((string) ($row['text'] ?? ''));
            if ($text === '') { continue; }
            $importantPoints[] = [
                'id' => safe_filename((string) ($row['id'] ?? '')) ?: ('important_point_' . ($idx + 1) . '_' . substr(hash('sha1', $text), 0, 8)),
                'text' => $text,
                'active' => isset($row['active']),
                'sort_order' => (int) ($row['sort_order'] ?? (($idx + 1) * 10)),
            ];
        }
        $d['important_points'] = documents_quote_normalize_important_points_settings([
            'enabled' => isset($_POST['important_points_enabled']),
            'title' => (string) ($_POST['important_points_title'] ?? ''),
            'intro' => (string) ($_POST['important_points_intro'] ?? ''),
    ...
    if ($action === 'save_quote') {
        ...
        if (array_key_exists('panel_orientation_enabled', $_POST) || array_key_exists('panel_orientation_present', $_POST)) {
            $rawOrientation = [
                'enabled' => isset($_POST['panel_orientation_enabled']),
                'site_area_type' => $_POST['panel_orientation_site_area_type'] ?? '',
                'site_area_label' => $_POST['panel_orientation_site_area_label'] ?? '',
                'default_facing_direction' => $_POST['panel_orientation_default_facing_direction'] ?? '',
                'default_tilt_deg' => $_POST['panel_orientation_default_tilt_deg'] ?? '',
                'row_layout' => $_POST['panel_orientation_row_layout'] ?? '',
                'shade_note' => $_POST['panel_orientation_shade_note'] ?? '',
                'customer_note' => $_POST['panel_orientation_customer_note'] ?? '',
                'groups' => [],
                'layout_mode' => 'generated',
            ];
            $labels = is_array($_POST['panel_orientation_group_label'] ?? null) ? $_POST['panel_orientation_group_label'] : [];
            $roofs = is_array($_POST['panel_orientation_group_roof_section'] ?? null) ? $_POST['panel_orientation_group_roof_section'] : [];
            $counts = is_array($_POST['panel_orientation_group_panel_count'] ?? null) ? $_POST['panel_orientation_group_panel_count'] : [];
            $directions = is_array($_POST['panel_orientation_group_facing_direction'] ?? null) ? $_POST['panel_orientation_group_facing_direction'] : [];
            $tilts = is_array($_POST['panel_orientation_group_tilt_deg'] ?? null) ? $_POST['panel_orientation_group_tilt_deg'] : [];
            $layouts = is_array($_POST['panel_orientation_group_row_layout'] ?? null) ? $_POST['panel_orientation_group_row_layout'] : [];
            $rows = is_array($_POST['panel_orientation_group_rows'] ?? null) ? $_POST['panel_orientation_group_rows'] : [];
            $cols = is_array($_POST['panel_orientation_group_columns'] ?? null) ? $_POST['panel_orientation_group_columns'] : [];
            $orients = is_array($_POST['panel_orientation_group_panel_orientation'] ?? null) ? $_POST['panel_orientation_group_panel_orientation'] : [];
            $xs = is_array($_POST['panel_orientation_group_x'] ?? null) ? $_POST['panel_orientation_group_x'] : [];
            $ys = is_array($_POST['panel_orientation_group_y'] ?? null) ? $_POST['panel_orientation_group_y'] : [];
            $rowGaps = is_array($_POST['panel_orientation_group_row_gap'] ?? null) ? $_POST['panel_orientation_group_row_gap'] : [];
            $colGaps = is_array($_POST['panel_orientation_group_column_gap'] ?? null) ? $_POST['panel_orientation_group_column_gap'] : [];
            $remarks = is_array($_POST['panel_orientation_group_remarks'] ?? null) ? $_POST['panel_orientation_group_remarks'] : [];
            $maxGroups = max(count($labels), count($roofs), count($counts), count($rows), count($cols), 1);
            for ($i = 0; $i < $maxGroups; $i++) {
                $rawOrientation['groups'][] = [
                    'label' => $labels[$i] ?? '', 'roof_section' => $roofs[$i] ?? '', 'panel_count' => $counts[$i] ?? 0,
                    'rows' => $rows[$i] ?? 1, 'columns' => $cols[$i] ?? 1, 'panel_orientation' => $orients[$i] ?? '',
                    'x' => $xs[$i] ?? 10, 'y' => $ys[$i] ?? 10, 'row_gap' => $rowGaps[$i] ?? 8, 'column_gap' => $colGaps[$i] ?? 6,
                    'facing_direction' => $directions[$i] ?? '', 'tilt_deg' => $tilts[$i] ?? '', 'row_layout' => $layouts[$i] ?? '', 'remarks' => $remarks[$i] ?? '',
                ];
            }
            $obLabels = is_array($_POST['panel_orientation_obstruction_label'] ?? null) ? $_POST['panel_orientation_obstruction_label'] : [];
            $obXs = is_array($_POST['panel_orientation_obstruction_x'] ?? null) ? $_POST['panel_orientation_obstruction_x'] : [];
            $obYs = is_array($_POST['panel_orientation_obstruction_y'] ?? null) ? $_POST['panel_orientation_obstruction_y'] : [];
            $obWs = is_array($_POST['panel_orientation_obstruction_width'] ?? null) ? $_POST['panel_orientation_obstruction_width'] : [];
            $obHs = is_array($_POST['panel_orientation_obstruction_height'] ?? null) ? $_POST['panel_orientation_obstruction_height'] : [];
            $obRemarks = is_array($_POST['panel_orientation_obstruction_remarks'] ?? null) ? $_POST['panel_orientation_obstruction_remarks'] : [];
            for ($i = 0, $m = max(count($obLabels), 1); $i < $m; $i++) $rawOrientation['obstructions'][] = ['label'=>$obLabels[$i]??'', 'x'=>$obXs[$i]??70, 'y'=>$obYs[$i]??15, 'width'=>$obWs[$i]??12, 'height'=>$obHs[$i]??12, 'remarks'=>$obRemarks[$i]??''];
            $panelOrientationJson = trim((string)($_POST['panel_orientation_json'] ?? ''));
            if ($panelOrientationJson !== '') {
                $decodedOrientation = json_decode($panelOrientationJson, true);
                if (is_array($decodedOrientation)) {
                    $rawOrientation['layout_mode'] = 'grid_editor';
                    $rawOrientation['grid'] = is_array($decodedOrientation['grid'] ?? null) ? $decodedOrientation['grid'] : [];
        ...
        foreach (['Self Funded Price' => $priceSelfFunded, 'Loan up to ₹2 lacs Price' => $priceLoanUp2, 'Loan above ₹2 lacs Price' => $priceLoanAbove2, 'Transportation' => $transportationRs, 'Subsidy' => $subsidyExpectedRs, 'Discount' => $discountRs] as $financeFieldLabel => $financeFieldValue) {
            if (!is_finite((float) $financeFieldValue) || (float) $financeFieldValue < 0) {
                $redirectWith('error', $financeFieldLabel . ' must be a valid non-negative number.');
            }
        }
        $priceSelfFunded = round($priceSelfFunded, 2);
        $priceLoanUp2 = round($priceLoanUp2, 2);
        $priceLoanAbove2 = round($priceLoanAbove2, 2);
        $transportationRs = round(max(0, $transportationRs), 2);
        $subsidyExpectedRs = round(max(0, $subsidyExpectedRs), 2);
        $discountRs = round(max(0, $discountRs), 2);
        ...
        $buildLoanScenario = static function (string $name, float $price, bool $applicable) use ($postData, $subsidyExpectedRs, $savedScenario, $savedLoanInputs): array {
            ...
            $marginRatioRaw = (float) $value($name . '_margin_ratio_pct', $saved['margin_ratio_pct'] ?? $defaultMarginRatio);
            $loanRatioRaw = (float) $value($name . '_loan_ratio_pct', $saved['loan_ratio_pct'] ?? $defaultLoanRatio);
            if (!is_finite($marginRatioRaw) || !is_finite($loanRatioRaw)) {
                $marginRatioRaw = $defaultMarginRatio;
                $loanRatioRaw = $defaultLoanRatio;
            }
            $marginRatio = max(0, min(100, round($marginRatioRaw, 2)));
            $loanRatio = max(0, min(100, round($loanRatioRaw, 2)));
            ...
            if (!is_finite($marginMoney)) $marginMoney = 0.0;
            if (!is_finite($loanAmount)) $loanAmount = 0.0;
            if ($isUpTo2) {
                $loanAmount = min($loanAmount, 200000, $price);
                $marginMoney = max(0, $price - $loanAmount);
                $marginRatio = $price > 0 ? round(($marginMoney / $price) * 100, 2) : 0.0;
                $loanRatio = max(0, round(100 - $marginRatio, 2));
            }
            $marginMoney = round(max(0, $marginMoney), 2);
            $loanAmount = round(max(0, $loanAmount), 2);
            $interestPct = (float) $value($name . '_interest_pct', $saved['interest_pct'] ?? $defaultInterestPct);
            if (!is_finite($interestPct)) $interestPct = $defaultInterestPct;
            $interestPct = round(max(0, min(100, $interestPct)), 2);
            $tenureYears = (float) $value($name . '_tenure_years', $saved['tenure_years'] ?? ($savedLoanInputs['tenure_years'] ?? 0));
            if (!is_finite($tenureYears)) $tenureYears = 0.0;
            $tenureYears = round(max(0, $tenureYears), 2);
            return [
                'price' => $price,
        ...
        if ($existing !== null && safe_text((string)($quote['revised_from_quote_id'] ?? '')) !== '') {
            $oldOrientation = documents_quote_normalize_panel_orientation(is_array($existing['panel_orientation'] ?? null) ? $existing['panel_orientation'] : []);
            $newOrientation = documents_quote_normalize_panel_orientation(is_array($quote['panel_orientation'] ?? null) ? $quote['panel_orientation'] : []);
            $oldHash = hash('sha256', json_encode($oldOrientation, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE));
            $newHash = hash('sha256', json_encode($newOrientation, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE));
            if ($oldHash !== $newHash) {
                $quote = documents_quote_append_customer_visible_history($quote, 'panel_orientation_changed', 'Solar panel orientation/layout was updated in this revised quotation.');
            }
        }
...
<link rel="stylesheet" href="assets/css/quote-panel-layout-designer.css">
<style>
/* Quotation-only layout helpers. Shared commercial chrome comes from admin-unified.css. */
.section-card{border:1px solid var(--admin-border);border-radius:14px;padding:0;background:#fff;margin-bottom:.85rem;overflow:hidden;box-shadow:var(--admin-shadow-soft)}
.section-card>h3{padding:1rem;margin:0}.section-card>summary{cursor:pointer;padding:1rem;font-size:1rem;font-weight:800;list-style:none}.section-card>summary::-webkit-details-marker{display:none}.section-card>summary:after{content:'+';float:right;color:var(--admin-muted)}.section-card[open]>summary:after{content:'−'}
.section-grid{display:grid;grid-template-columns:repeat(4,minmax(170px,1fr));gap:.8rem;padding:0 1rem 1rem}.section-grid .full-span{grid-column:1/-1}.section-card.savings{background:#f8fbff;border-color:#bfdbfe}form.section-card{padding:1rem}
.editor-intro,.form-quick-actions{display:flex;gap:.75rem;align-items:flex-end;justify-content:space-between;flex-wrap:wrap}.form-quick-actions{position:sticky;top:.5rem;z-index:20;background:#eff6ff;border:1px solid #bfdbfe;border-radius:13px;padding:.65rem .75rem;margin-bottom:.85rem;align-items:center}.list-table-wrap{overflow-x:auto}.quote-meta{font-size:.75rem;color:var(--admin-muted);margin-top:.2rem}.sticky-head th{position:sticky;top:0;z-index:2}
.ux-backdrop{position:fixed;inset:0;background:rgba(15,23,42,.42);display:none;z-index:60}.ux-modal{position:fixed;inset:6vh 4vw;background:#fff;border-radius:14px;display:none;z-index:61;box-shadow:0 22px 56px rgba(0,0,0,.25);overflow:hidden}.ux-modal iframe{width:100%;height:100%;border:none}.ux-modal-head{padding:.7rem .85rem;border-bottom:1px solid var(--admin-border);display:flex;justify-content:space-between;align-items:center}.ux-open{display:block}.toast{position:fixed;right:1rem;top:1rem;z-index:120;background:#111827;color:#fff;padding:.65rem .85rem;border-radius:10px;display:none;max-width:420px}.toast.show{display:block}.toast.err{background:#b91c1c}
@media (max-width:1100px){.section-grid{grid-template-columns:repeat(2,minmax(170px,1fr));}}@media (max-width:700px){.section-grid{grid-template-columns:1fr}.form-quick-actions{position:static}}
...
</style><?php require_once __DIR__ . '/includes/pwa_head.php'; ?></head>
<body class="admin-shell commercial-admin"><?php require_once __DIR__ . '/includes/mobile_app_nav.php'; ?><main class="wrap commercial-shell">
...
<?php
$orientation = documents_quote_normalize_panel_orientation(is_array($editing['panel_orientation'] ?? null) ? $editing['panel_orientation'] : ['enabled' => $editing['id'] === '']);
$areaOptions = documents_quote_panel_orientation_allowed_area_types();
$directionOptions = documents_quote_panel_orientation_allowed_directions();
$layoutOptions = documents_quote_panel_orientation_allowed_layouts();
$orientationGroups = array_slice((array)($orientation['groups'] ?? []), 0, 8);
while (count($orientationGroups) < 6) { $n=count($orientationGroups); $orientationGroups[] = ['label'=>'','roof_section'=>'','panel_count'=>'','rows'=>1,'columns'=>4,'panel_orientation'=>'portrait','x'=>10+$n*12,'y'=>10+$n*10,'row_gap'=>8,'column_gap'=>6,'facing_direction'=>$orientation['default_facing_direction'],'tilt_deg'=>$orientation['default_tilt_deg'],'row_layout'=>$orientation['row_layout'],'remarks'=>'']; }
$orientationObstructions = array_slice((array)($orientation['obstructions'] ?? []), 0, 4);
while (count($orientationObstructions) < 3) { $orientationObstructions[] = ['label'=>'','x'=>70,'y'=>15,'width'=>12,'height'=>12,'remarks'=>'']; }
?>
<div style="grid-column:1/-1" class="form-section-card"><h3>Solar Panel Orientation / Layout</h3><input type="hidden" name="panel_orientation_present" value="1"><label><input type="checkbox" name="panel_orientation_enabled" <?= !empty($orientation['enabled']) ? 'checked' : '' ?>> Show solar panel orientation in quotation</label><div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin-top:10px"><div><label>Fast preset / layout style</label><select name="panel_orientation_row_layout"><?php foreach($layoutOptions as $value=>$label): ?><option value="<?= htmlspecialchars($value, ENT_QUOTES) ?>" <?= $orientation['row_layout']===$value?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select><div class="muted">Preset is only a starting point; edit groups below for custom/freeform layouts.</div></div><div><label>Roof / area</label><select name="panel_orientation_site_area_type"><?php foreach($areaOptions as $value=>$label): ?><option value="<?= htmlspecialchars($value, ENT_QUOTES) ?>" <?= $orientation['site_area_type']===$value?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></div><div><label>Area label / roof section name</label><input name="panel_orientation_site_area_label" value="<?= htmlspecialchars((string)$orientation['site_area_label'], ENT_QUOTES) ?>"></div><div><label>Default facing direction</label><select name="panel_orientation_default_facing_direction"><?php foreach($directionOptions as $label): ?><option value="<?= htmlspecialchars($label, ENT_QUOTES) ?>" <?= $orientation['default_facing_direction']===$label?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></div><div><label>Default tilt angle in degrees</label><input type="number" min="0" max="45" step="0.1" name="panel_orientation_default_tilt_deg" value="<?= htmlspecialchars((string)$orientation['default_tilt_deg'], ENT_QUOTES) ?>"></div></div>
<h3>Solar Panel Layout Designer</h3><input type="hidden" name="panel_orientation_json" id="panelOrientationJson" value="<?= htmlspecialchars(json_encode(['grid'=>$orientation['grid'] ?? ['columns'=>36,'rows'=>24,'cell_unit'=>'grid','editor_cell_px'=>18,'major_line_every'=>5,'customer_grid_visible'=>false], 'objects'=>$orientation['objects'] ?? []], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), ENT_QUOTES) ?>"><div class="panel-layout-designer" data-panel-layout-designer data-initial-layout="<?= htmlspecialchars(json_encode(['grid'=>$orientation['grid'] ?? ['columns'=>36,'rows'=>24,'cell_unit'=>'grid','editor_cell_px'=>18,'major_line_every'=>5,'customer_grid_visible'=>false], 'objects'=>$orientation['objects'] ?? []], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), ENT_QUOTES) ?>"><div class="panel-layout-toolbar"><div><strong>Drag palette items to the grid</strong><div class="muted">Panels and labels snap to grid units; overlaps are rejected.</div></div><div class="panel-layout-actions"><button type="button" class="btn secondary" data-layout-reset>Reset</button><button type="button" class="btn secondary" data-layout-clear>Clear</button></div></div><div class="panel-layout-controls"><div><label>Grid width</label><input type="number" min="8" max="100" step="1" data-layout-columns value="<?= (int)($orientation['grid']['columns'] ?? 36) ?>"></div><div><label>Grid height</label><input type="number" min="8" max="100" step="1" data-layout-rows value="<?= (int)($orientation['grid']['rows'] ?? 24) ?>"></div><div><label>Zoom</label><div class="panel-layout-zoom"><button type="button" data-layout-zoom-out>−</button><span data-layout-zoom-label>100%</span><button type="button" data-layout-zoom-in>+</button><button type="button" data-layout-fit>Fit</button></div></div><div class="muted">Use a smaller grid for small systems and a larger grid for bigger roofs or more panels. Panels snap to the grid and cannot overlap.</div></div><div class="panel-layout-main"><div class="panel-layout-scroll" data-layout-scroll><div class="panel-layout-grid" data-layout-grid tabindex="0" aria-label="Solar panel layout grid"></div></div><aside class="panel-layout-palette"><button type="button" data-add-layout-item="panel-portrait">Portrait panel</button><button type="button" data-add-layout-item="panel-landscape">Landscape panel</button><button type="button" data-add-layout-item="text">Text label</button><button type="button" data-add-layout-item="obstruction">Keep-out</button><button type="button" data-add-layout-item="arrow">North arrow</button><hr><input data-layout-text placeholder="Selected label/text"><div class="panel-layout-nudges"><button type="button" data-layout-move="up">↑</button><button type="button" data-layout-move="left">←</button><button type="button" data-layout-move="right">→</button><button type="button" data-layout-move="down">↓</button></div><button type="button" data-layout-rotate>Rotate</button><button type="button" data-layout-duplicate>Duplicate</button><button type="button" data-layout-delete>Delete</button><div class="panel-layout-status" data-layout-status>Ready</div></aside></div><div class="muted">This designer is preferred for public rendering when objects are present; the group table below remains as fallback/summary data.</div></div>
<h4>Editable layout groups</h4><div class="responsive-table"><table><thead><tr><th>Label</th><th>Roof section</th><th>Panels</th><th>Rows</th><th>Columns</th><th>Panel orientation</th><th>X%</th><th>Y%</th><th>Row gap</th><th>Column gap</th><th>Facing</th><th>Tilt</th><th>Remarks</th></tr></thead><tbody><?php foreach($orientationGroups as $group): ?><tr><td><input name="panel_orientation_group_label[]" value="<?= htmlspecialchars((string)($group['label'] ?? ''), ENT_QUOTES) ?>"></td><td><input name="panel_orientation_group_roof_section[]" value="<?= htmlspecialchars((string)($group['roof_section'] ?? ''), ENT_QUOTES) ?>"></td><td><input type="number" min="0" step="1" name="panel_orientation_group_panel_count[]" value="<?= htmlspecialchars((string)($group['panel_count'] ?? ''), ENT_QUOTES) ?>"></td><td><input type="number" min="1" max="12" name="panel_orientation_group_rows[]" value="<?= htmlspecialchars((string)($group['rows'] ?? 1), ENT_QUOTES) ?>"></td><td><input type="number" min="1" max="24" name="panel_orientation_group_columns[]" value="<?= htmlspecialchars((string)($group['columns'] ?? 1), ENT_QUOTES) ?>"></td><td><select name="panel_orientation_group_panel_orientation[]"><option value="portrait" <?= ($group['panel_orientation']??'')==='portrait'?'selected':'' ?>>Portrait</option><option value="landscape" <?= ($group['panel_orientation']??'')==='landscape'?'selected':'' ?>>Landscape</option></select></td><td><input type="number" min="0" max="100" name="panel_orientation_group_x[]" value="<?= htmlspecialchars((string)($group['x'] ?? 10), ENT_QUOTES) ?>"></td><td><input type="number" min="0" max="100" name="panel_orientation_group_y[]" value="<?= htmlspecialchars((string)($group['y'] ?? 10), ENT_QUOTES) ?>"></td><td><input type="number" min="0" max="40" name="panel_orientation_group_row_gap[]" value="<?= htmlspecialchars((string)($group['row_gap'] ?? 8), ENT_QUOTES) ?>"></td><td><input type="number" min="0" max="40" name="panel_orientation_group_column_gap[]" value="<?= htmlspecialchars((string)($group['column_gap'] ?? 6), ENT_QUOTES) ?>"></td><td><select name="panel_orientation_group_facing_direction[]"><?php foreach($directionOptions as $label): ?><option value="<?= htmlspecialchars($label, ENT_QUOTES) ?>" <?= (string)($group['facing_direction'] ?? '')===$label?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></td><td><input type="number" min="0" max="45" step="0.1" name="panel_orientation_group_tilt_deg[]" value="<?= htmlspecialchars((string)($group['tilt_deg'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="panel_orientation_group_row_layout[]" value="<?= htmlspecialchars((string)($group['row_layout'] ?? $orientation['row_layout']), ENT_QUOTES) ?>"></td><td><input name="panel_orientation_group_remarks[]" value="<?= htmlspecialchars((string)($group['remarks'] ?? ''), ENT_QUOTES) ?>"></td></tr><?php endforeach; ?></tbody></table></div><h4>Obstructions / keep-out areas</h4><div class="responsive-table"><table><thead><tr><th>Label</th><th>X%</th><th>Y%</th><th>Width%</th><th>Height%</th><th>Remarks</th></tr></thead><tbody><?php foreach($orientationObstructions as $ob): ?><tr><td><input name="panel_orientation_obstruction_label[]" value="<?= htmlspecialchars((string)($ob['label'] ?? ''), ENT_QUOTES) ?>"></td><td><input type="number" min="0" max="100" name="panel_orientation_obstruction_x[]" value="<?= htmlspecialchars((string)($ob['x'] ?? 70), ENT_QUOTES) ?>"></td><td><input type="number" min="0" max="100" name="panel_orientation_obstruction_y[]" value="<?= htmlspecialchars((string)($ob['y'] ?? 15), ENT_QUOTES) ?>"></td><td><input type="number" min="1" max="100" name="panel_orientation_obstruction_width[]" value="<?= htmlspecialchars((string)($ob['width'] ?? 12), ENT_QUOTES) ?>"></td><td><input type="number" min="1" max="100" name="panel_orientation_obstruction_height[]" value="<?= htmlspecialchars((string)($ob['height'] ?? 12), ENT_QUOTES) ?>"></td><td><input name="panel_orientation_obstruction_remarks[]" value="<?= htmlspecialchars((string)($ob['remarks'] ?? ''), ENT_QUOTES) ?>"></td></tr><?php endforeach; ?></tbody></table></div><label>Shade / obstruction / site note</label><textarea name="panel_orientation_shade_note"><?= htmlspecialchars((string)$orientation['shade_note'], ENT_QUOTES) ?></textarea><label>Customer-facing orientation note</label><textarea name="panel_orientation_customer_note"><?= htmlspecialchars((string)$orientation['customer_note'], ENT_QUOTES) ?></textarea><div class="muted">Use X/Y positions, rows/columns, gaps, and separate roof-section labels to create single-row, multi-row, mixed, staggered, or separated roof layouts. The quotation renders a deterministic SVG preview.</div></div>
<?php $editorImportantSettings = documents_quote_resolve_important_points($editing, $quoteDefaults); $editorImportantPreview = documents_quote_render_important_points($editorImportantSettings); ?><div class="full-span" style="grid-column:1/-1;padding:12px;border:1px solid #f3d47a;border-radius:12px;background:#fffdf4"><h3 style="margin-top:0">Important Points shown to customer</h3><p class="muted">Draft quotations use current global Important Points. Approved, shared, or accepted quotations keep a saved snapshot for auditability.</p><div class="quotation-preview"><?= $editorImportantPreview !== '' ? $editorImportantPreview : '<p class="muted">Important Points are disabled or no active points are configured.</p>' ?></div></div><div style="grid-column:1/-1"><label>Special Requests From Consumer (Inclusive in the rate)</label><textarea name="special_requests_text"><?= htmlspecialchars((string)($editing['special_requests_text'] ?: $editing['special_requests_inclusive']), ENT_QUOTES) ?></textarea><div class="muted">In case of conflict between annexures and special requests, special requests will be prioritized.</div></div>
...
<div><label>Loan up to ₹2 lacs Price ₹</label><input type="number" min="0" step="0.01" name="scenario_price_loan_upto_2_lacs" value="<?= htmlspecialchars(quotation_number_input_value($editing['scenario_prices']['loan_upto_2_lacs']['price'] ?? $editing['input_total_gst_inclusive'] ?? 0), ENT_QUOTES) ?>"></div>
<div><label>Loan above ₹2 lacs Price ₹</label><input type="number" min="0" step="0.01" name="scenario_price_loan_above_2_lacs" value="<?= htmlspecialchars(quotation_number_input_value($editing['scenario_prices']['loan_above_2_lacs']['price'] ?? $editing['input_total_gst_inclusive'] ?? 0), ENT_QUOTES) ?>"></div>
...
<div><label>Selected Primary Scenario Price (including GST) ₹</label><input type="number" min="0" step="0.01" readonly required name="system_total_incl_gst_rs" value="<?= htmlspecialchars((string)($editing['input_total_gst_inclusive'] ?? 0), ENT_QUOTES) ?>"></div>
...
<div><label>Monthly electricity bill (₹)</label><input type="number" min="0" step="0.01" name="monthly_bill_rs" value="<?= htmlspecialchars((string)($editing['finance_inputs']['monthly_bill_rs'] ?? ''), ENT_QUOTES) ?>"><div class="muted">Suggested bill based on generation & tariff. You can change it. <a href="#" id="resetMonthlySuggestion">Reset suggestion</a></div></div>
<input type="hidden" name="monthly_bill_touched" id="monthlyBillTouched" value="0">
<div><label>Unit rate (₹/kWh)</label><input type="number" min="0" step="0.01" name="unit_rate_rs_per_kwh" value="<?= htmlspecialchars((string)($savedUnitRateForEdit !== '' ? $savedUnitRateForEdit : ($segmentDefaults['unit_rate_rs_per_kwh'] ?? '')), ENT_QUOTES) ?>"></div>
<div><label>Annual generation per kW</label><input type="number" min="0" step="0.01" name="annual_generation_per_kw" value="<?= htmlspecialchars((string)($savedAnnualGenerationForEdit !== '' ? $savedAnnualGenerationForEdit : ($quoteDefaults['global']['energy_defaults']['annual_generation_per_kw'] ?? '')), ENT_QUOTES) ?>"></div>
<div><label>Transportation ₹</label><input type="number" min="0" step="0.01" name="transportation_rs" value="<?= htmlspecialchars((string)($editing['finance_inputs']['transportation_rs'] ?? ''), ENT_QUOTES) ?>"></div>
...
<div><label>Subsidy ₹</label><input type="number" min="0" step="0.01" name="subsidy_expected_rs" value="<?= htmlspecialchars((string)($editing['finance_inputs']['subsidy_expected_rs'] ?? ''), ENT_QUOTES) ?>"><div class="muted"><a href="#" id="resetSubsidyDefault">Reset to scheme default</a></div></div>
...
<div><label>Margin %</label><input type="number" min="0" max="100" step="0.01" name="loan_upto_2_lacs_margin_ratio_pct" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanUp2Scenario['margin_ratio_pct'] ?? 10), ENT_QUOTES) ?>"></div>
<div><label>Loan %</label><input type="number" min="0" max="100" step="0.01" name="loan_upto_2_lacs_loan_ratio_pct" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanUp2Scenario['loan_ratio_pct'] ?? 90), ENT_QUOTES) ?>"></div>
<div><label>Margin ₹</label><input type="number" min="0" step="0.01" name="loan_upto_2_lacs_margin_money_rs" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanUp2Scenario['margin_money_rs'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Loan ₹</label><input type="number" min="0" step="0.01" name="loan_upto_2_lacs_loan_amount_rs" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanUp2Scenario['loan_amount_rs'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Interest %</label><input type="number" min="0" max="100" step="0.01" name="loan_upto_2_lacs_interest_pct" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanUp2Scenario['interest_pct'] ?? 5.75), ENT_QUOTES) ?>"></div>
<div><label>Tenure years</label><input type="number" min="0" step="0.01" name="loan_upto_2_lacs_tenure_years" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanUp2Scenario['tenure_years'] ?? ($editing['finance_inputs']['loan']['tenure_years'] ?? '')), ENT_QUOTES) ?>"></div>
...
<div><label>Margin %</label><input type="number" min="0" max="100" step="0.01" name="loan_above_2_lacs_margin_ratio_pct" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanAbove2Scenario['margin_ratio_pct'] ?? 20), ENT_QUOTES) ?>"></div>
<div><label>Loan %</label><input type="number" min="0" max="100" step="0.01" name="loan_above_2_lacs_loan_ratio_pct" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanAbove2Scenario['loan_ratio_pct'] ?? 80), ENT_QUOTES) ?>"></div>
<div><label>Margin ₹</label><input type="number" min="0" step="0.01" name="loan_above_2_lacs_margin_money_rs" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanAbove2Scenario['margin_money_rs'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Loan ₹</label><input type="number" min="0" step="0.01" name="loan_above_2_lacs_loan_amount_rs" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanAbove2Scenario['loan_amount_rs'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Interest %</label><input type="number" min="0" max="100" step="0.01" name="loan_above_2_lacs_interest_pct" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanAbove2Scenario['interest_pct'] ?? 8.15), ENT_QUOTES) ?>"></div>
<div><label>Tenure years</label><input type="number" min="0" step="0.01" name="loan_above_2_lacs_tenure_years" value="<?= htmlspecialchars(quotation_number_input_value($editingLoanAbove2Scenario['tenure_years'] ?? ''), ENT_QUOTES) ?>"></div>
...
<?php $bulkPreview = is_array($_SESSION['quotation_bulk_preview'] ?? null) ? $_SESSION['quotation_bulk_preview'] : []; $bulkRows = is_array($bulkPreview['rows'] ?? null) ? $bulkPreview['rows'] : []; $bulkCounts = is_array($bulkPreview['counts'] ?? null) ? $bulkPreview['counts'] : []; $recentBulkBatches = quotation_bulk_recent_batches(); ?>
<section style="margin:14px 0;padding:14px;border:1px solid #dbe2ee;border-radius:12px;background:#f8fafc"><h3>CSV import workspace — create draft quotations</h3><p class="muted">Download the template/reference data, upload a UTF-8 CSV for validation, then create only valid draft quotations. Preview does not consume quotation numbers.</p><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px"><a class="btn secondary" href="admin-quotations.php?action=download_quote_csv_template">Download quotation CSV template</a><a class="btn secondary" href="admin-quotations.php?action=download_quote_reference_data">Download reference data</a><?php if ($bulkRows !== []): ?><a class="btn secondary" href="admin-quotations.php?action=download_quote_validation_results">Download validation-results CSV</a><?php endif; ?></div><form method="post" enctype="multipart/form-data" style="display:flex;gap:10px;align-items:end;flex-wrap:wrap"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="quotation_bulk_upload_csv"><div><label>Upload quotation CSV (max 1 MB / 200 rows)</label><input type="file" name="quotation_csv" accept=".csv,text/csv" required></div><button class="btn" type="submit">Validate CSV</button></form><?php if ($bulkRows !== []): ?><div class="status-strip" style="margin-top:12px"><span>Total: <?= (int)($bulkCounts['total'] ?? 0) ?></span><span>Valid: <?= (int)($bulkCounts['valid'] ?? 0) ?></span><span>Invalid: <?= (int)($bulkCounts['invalid'] ?? 0) ?></span><span>Warnings: <?= (int)($bulkCounts['warning'] ?? 0) ?></span><span>Duplicates: <?= (int)($bulkCounts['duplicate'] ?? 0) ?></span></div><div class="responsive-table"><table><thead><tr><th>Row</th><th>Import key</th><th>Customer</th><th>System/model</th><th>DCR</th><th>Non-DCR</th><th>Total</th><th>Template</th><th>Amount</th><th>Items</th><th>Status</th><th>Messages</th></tr></thead><tbody><?php foreach ($bulkRows as $row): ?><tr><td><?= (int)($row['row_number'] ?? 0) ?></td><td><?= htmlspecialchars((string)($row['import_key'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($row['customer'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($row['system_model'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($row['dcr'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($row['non_dcr'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($row['total'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($row['template'] ?? ''), ENT_QUOTES) ?></td><td><?= number_format((float)($row['amount'] ?? 0), 2) ?></td><td><?= (int)($row['item_count'] ?? 0) ?></td><td><?= htmlspecialchars((string)($row['status'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars(implode(' | ', (array)($row['messages'] ?? [])), ENT_QUOTES) ?></td></tr><?php endforeach; ?></tbody></table></div><form method="post" onsubmit="return confirm('Create valid draft quotations from this preview?');"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string)($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="quotation_bulk_create_valid"><button class="btn" type="submit">Create Valid Draft Quotations</button></form><?php endif; ?><?php if ($recentBulkBatches !== []): ?><h3>Recent CSV import batches</h3><div class="responsive-table"><table><thead><tr><th>Batch</th><th>File</th><th>When</th><th>Totals</th><th>Created quotations</th><th>Results</th></tr></thead><tbody><?php foreach ($recentBulkBatches as $batch): $tot=(array)($batch['totals'] ?? []); ?><tr><td><?= htmlspecialchars((string)($batch['batch_id'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($batch['source_filename'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($batch['created_at'] ?? ''), ENT_QUOTES) ?></td><td><?= 'valid '.(int)($tot['valid'] ?? 0).', invalid '.(int)($tot['invalid'] ?? 0).', created '.(int)($tot['created'] ?? 0).', failed '.(int)($tot['failed'] ?? 0).', skipped '.(int)($tot['skipped'] ?? 0) ?></td><td><?php foreach ((array)($batch['results'] ?? []) as $result): if (($result['created_quote_id'] ?? '') === '') continue; ?><a href="quotation-view.php?id=<?= urlencode((string)$result['created_quote_id']) ?>"><?= htmlspecialchars((string)($result['created_quote_no'] ?? $result['created_quote_id']), ENT_QUOTES) ?></a> <?php endforeach; ?></td><td><a href="admin-quotations.php?action=download_quote_import_result&amp;batch_id=<?= urlencode((string)($batch['batch_id'] ?? '')) ?>">CSV</a></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></section>
<h3>Existing quotation bulk status actions</h3>
<form method="post">
<input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES) ?>">
...
(function(){
  ...
  if(editorCard){
    ...
    if(grid && quoteForm){
      ...
      const cancel=document.createElement('a');cancel.className='btn secondary';cancel.href='admin-quotations.php?tab=quotations';cancel.textContent='Cancel / Back to Manage Quotations';saveCard.querySelector('.save-actions').appendChild(cancel);grid.appendChild(saveCard);
...
(function () {
    ...
    const importantTable = document.querySelector('#importantPointsTable tbody');
    const addImportantPoint = document.getElementById('addImportantPoint');
    const renumberImportantPoints = () => {
      if (!importantTable) return;
      importantTable.querySelectorAll('tr').forEach((row, idx) => {
        row.querySelector('td:first-child').childNodes[0].textContent = String(idx + 1);
        row.querySelectorAll('[name]').forEach((field) => { field.name = field.name.replace(/important_points\[\d+\]/, `important_points[${idx}]`); });
      });
    };
    if (importantTable) importantTable.addEventListener('click', (event) => {
      if (event.target && event.target.classList.contains('important-remove')) { event.target.closest('tr')?.remove(); renumberImportantPoints(); }
    });
    if (addImportantPoint && importantTable) addImportantPoint.addEventListener('click', () => {
      const idx = importantTable.querySelectorAll('tr').length;
