...
(function(){
  ...
  if(editorCard){
    ...
    if(grid){
      ...
      grid.innerHTML='';
      if(lookupForm){sectionCards[0].el.insertBefore(lookupForm, sectionCards[0].inner);}
      sectionCards.forEach((sec)=>{if(sec===sectionCards[0] || sec.inner.children.length){grid.appendChild(sec.el);}});
      const submitBtn=quoteForm.querySelector('button[type=\"submit\"]');
      if(submitBtn){submitBtn.classList.add('btn');saveInner.appendChild(submitBtn);}
      const backBtn=document.createElement('button');backBtn.type='button';backBtn.className='btn secondary';backBtn.textContent='Cancel / Back to list';backBtn.addEventListener('click',()=>setTab('list'));saveInner.appendChild(backBtn);
