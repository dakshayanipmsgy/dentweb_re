...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_quote') {
        ...
        if (array_key_exists('panel_orientation_enabled', $_POST) || array_key_exists('panel_orientation_present', $_POST)) {
            ...
            $maxGroups = max(count($labels), count($roofs), count($counts), 1);
            for ($i = 0; $i < $maxGroups; $i++) {
                $rawOrientation['groups'][] = [
                    'label' => $labels[$i] ?? '',
                    'roof_section' => $roofs[$i] ?? '',
                    'panel_count' => $counts[$i] ?? 0,
                    'facing_direction' => $directions[$i] ?? '',
                    'tilt_deg' => $tilts[$i] ?? '',
                    'row_layout' => $layouts[$i] ?? '',
                    'remarks' => $remarks[$i] ?? '',
                ];
            }
...
$orientationGroups = array_slice((array)($orientation['groups'] ?? []), 0, 3);
while (count($orientationGroups) < 3) { $orientationGroups[] = ['label'=>'','roof_section'=>'','panel_count'=>'','facing_direction'=>$orientation['default_facing_direction'],'tilt_deg'=>$orientation['default_tilt_deg'],'row_layout'=>$orientation['row_layout'],'remarks'=>'']; }
?>
<div style="grid-column:1/-1" class="form-section-card"><h3>Solar Panel Orientation / Layout</h3><input type="hidden" name="panel_orientation_present" value="1"><label><input type="checkbox" name="panel_orientation_enabled" <?= !empty($orientation['enabled']) ? 'checked' : '' ?>> Show solar panel orientation in quotation</label><div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin-top:10px"><div><label>Roof / area</label><select name="panel_orientation_site_area_type"><?php foreach($areaOptions as $value=>$label): ?><option value="<?= htmlspecialchars($value, ENT_QUOTES) ?>" <?= $orientation['site_area_type']===$value?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></div><div><label>Area label / roof section name</label><input name="panel_orientation_site_area_label" value="<?= htmlspecialchars((string)$orientation['site_area_label'], ENT_QUOTES) ?>"></div><div><label>Panel facing direction</label><select name="panel_orientation_default_facing_direction"><?php foreach($directionOptions as $label): ?><option value="<?= htmlspecialchars($label, ENT_QUOTES) ?>" <?= $orientation['default_facing_direction']===$label?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></div><div><label>Tilt angle in degrees</label><input type="number" min="0" max="45" step="0.1" name="panel_orientation_default_tilt_deg" value="<?= htmlspecialchars((string)$orientation['default_tilt_deg'], ENT_QUOTES) ?>"></div><div><label>Panel row direction / layout style</label><select name="panel_orientation_row_layout"><?php foreach($layoutOptions as $value=>$label): ?><option value="<?= htmlspecialchars($value, ENT_QUOTES) ?>" <?= $orientation['row_layout']===$value?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></div></div><h4>Orientation groups</h4><div class="responsive-table"><table><thead><tr><th>Label</th><th>Roof section</th><th>Panels</th><th>Facing</th><th>Tilt</th><th>Layout</th><th>Remarks</th></tr></thead><tbody><?php foreach($orientationGroups as $group): ?><tr><td><input name="panel_orientation_group_label[]" value="<?= htmlspecialchars((string)($group['label'] ?? ''), ENT_QUOTES) ?>"></td><td><input name="panel_orientation_group_roof_section[]" value="<?= htmlspecialchars((string)($group['roof_section'] ?? ''), ENT_QUOTES) ?>"></td><td><input type="number" min="0" step="1" name="panel_orientation_group_panel_count[]" value="<?= htmlspecialchars((string)($group['panel_count'] ?? ''), ENT_QUOTES) ?>"></td><td><select name="panel_orientation_group_facing_direction[]"><?php foreach($directionOptions as $label): ?><option value="<?= htmlspecialchars($label, ENT_QUOTES) ?>" <?= (string)($group['facing_direction'] ?? '')===$label?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></td><td><input type="number" min="0" max="45" step="0.1" name="panel_orientation_group_tilt_deg[]" value="<?= htmlspecialchars((string)($group['tilt_deg'] ?? ''), ENT_QUOTES) ?>"></td><td><select name="panel_orientation_group_row_layout[]"><?php foreach($layoutOptions as $value=>$label): ?><option value="<?= htmlspecialchars($value, ENT_QUOTES) ?>" <?= (string)($group['row_layout'] ?? '')===$value?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></td><td><input name="panel_orientation_group_remarks[]" value="<?= htmlspecialchars((string)($group['remarks'] ?? ''), ENT_QUOTES) ?>"></td></tr><?php endforeach; ?></tbody></table></div><label>Shade / obstruction / site note</label><textarea name="panel_orientation_shade_note"><?= htmlspecialchars((string)$orientation['shade_note'], ENT_QUOTES) ?></textarea><label>Customer-facing orientation note</label><textarea name="panel_orientation_customer_note"><?= htmlspecialchars((string)$orientation['customer_note'], ENT_QUOTES) ?></textarea><div class="muted">Generated deterministic SVG preview appears in the customer quotation and print view.</div></div>
