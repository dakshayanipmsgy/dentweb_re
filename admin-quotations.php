...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_quote') {
        ...
        $structuredLineTotals = is_array($_POST['quote_item_line_total_incl_gst'] ?? null) ? $_POST['quote_item_line_total_incl_gst'] : [];
        $structuredItems = [];
        $itemSummaryRows = [];
        ...
        for ($i = 0; $i < $structuredCount; $i++) {
            ...
            if ($lineType === 'kit') {
                ...
                $structuredItems[] = [
                    ...
                    'line_total_incl_gst' => max(0, (float) ($structuredLineTotals[$i] ?? 0)),
...
if ($quoteFormOld !== []) {
    ...
    foreach ((array) ($quoteFormOld['quote_item_type'] ?? []) as $ix => $type) {
        $postedItems[] = [
            ...
            'line_total_incl_gst' => (float) (($quoteFormOld['quote_item_line_total_incl_gst'][$ix] ?? 0)),
        ];
    }
