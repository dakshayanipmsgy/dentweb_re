...
if (safe_text($_GET['action'] ?? '') === 'download_quotation_csv_template') {
    $sample = documents_quote_import_generate_sample_row();
    if (isset($sample['error'])) { http_response_code(409); header('Content-Type: text/plain; charset=UTF-8'); echo $sample['error']; exit; }
    ...
    fputcsv($out, $headers);
    fputcsv($out, array_map(static fn(string $header): string => documents_quote_import_safe_csv_cell($sample[$header] ?? ''), $headers));
    exit;
}
if (safe_text($_GET['action'] ?? '') === 'download_quotation_reference_data') {
    ...
    $out = fopen('php://output', 'w');
    documents_quote_reference_stream_csv($out);
    exit;
}
...
if (safe_text($_GET['action'] ?? '') === 'download_quotation_validation_results') {
    ...
    $out = fopen('php://output', 'w');
    fputcsv($out, ['source_row','import_key','customer_name','customer_mobile','template','system_type','selected_model_number','main_solar_kwp','non_dcr_kwp','total_capacity','primary_amount','structured_item_count','basic_amount','gst','tax_mode','subsidy','transportation','discount','final_payable','matched_kit','resolved_template_id','resolved_template_name','resolved_template_display_name','resolved_segment','annexure_count','annexure_keys','template_attachment_count','template_messages','status','messages','commit_result']);
    foreach ((array)($batch['rows'] ?? []) as $r) { $in=is_array($r['input']??null)?$r['input']:[]; $v=is_array($r['validation']??null)?$r['validation']:[]; $msgs=array_map(static fn($m): string => (string)($m['field']??'row').': '.(string)($m['message']??''), (array)($v['messages']??[])); fputcsv($out, array_map('documents_quote_import_safe_csv_cell', [(string)($r['source_row']??''),(string)($in['import_key']??''),(string)($in['customer_name']??''),(string)($in['customer_mobile']??''),(string)($in['template_set']??''),(string)($in['system_type']??''),(string)($in['selected_model_number']??''),(string)($in['main_solar_kwp']??''),(string)($in['non_dcr_kwp']??''),(string)(((float)($in['main_solar_kwp']??0))+((float)($in['non_dcr_kwp']??0))),(string)($in['scenario_price_self_funded']??''),(string)($v['preview']['item_count']??'0'),(string)($v['preview']['basic_amount']??''),(string)($v['preview']['gst']??''),(string)($v['preview']['tax_mode']??''),(string)($v['preview']['subsidy']??''),(string)($v['preview']['transportation']??''),(string)($v['preview']['discount']??''),(string)($v['preview']['final_payable']??''),(string)($v['preview']['kit']??''),(string)($v['preview']['template_id']??''),(string)($v['preview']['template_name']??''),(string)($v['preview']['template_display_name']??''),(string)($v['preview']['segment']??''),(string)($v['preview']['annexure_count']??'0'),implode('|',(array)($v['preview']['annexure_keys']??[])),(string)($v['preview']['attachment_count']??'0'),implode('; ', array_map(static fn($m): string => (string)($m['field']??'template_set').': '.(string)($m['message']??''), (array)($v['preview']['template_warnings']??[]))),(string)($r['status']??''),implode('; ', $msgs),(string)($r['commit_result']??'')])); }
    exit;
}

if (safe_text($_GET['action'] ?? '') === 'quotation_browser_export_render') {
    $user = current_user();
    ...
    $token = safe_text((string)($_GET['token'] ?? ''));
    $id = safe_text((string)($_GET['id'] ?? ''));
    $ids = quotation_browser_export_token_ids($token);
    if ($token === '' || $id === '' || !in_array($id, $ids, true)) {
        http_response_code(403);
        echo 'Invalid or expired quotation export token.';
        exit;
    }
    ...
    echo quotation_browser_export_render_html($quote, $quoteDefaults, documents_get_company_profile_for_quotes());
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'quotation_csv_upload') {
        ...
        while (($data = fgetcsv($fh)) !== false) {
            ...
            $originalPartyType = (string)($input['party_type'] ?? '');
            $input['party_type_original'] = $originalPartyType;
            $input = documents_quote_import_normalize_input($input);
            $result=documents_validate_quote_draft_input($input, ['source'=>'csv','original_party_type'=>$originalPartyType]);
    ...
    if ($action === 'quotation_csv_repair_preview' || $action === 'quotation_csv_repair_commit') {
        $user=current_user(); if ((string)($user['role_name'] ?? '') !== 'admin') { $redirectWith('error', 'Only administrators can repair CSV imports.'); }
        $batchId=safe_text($_POST['batch_id'] ?? ''); $batch=documents_quote_import_load_batch($batchId); if(!$batch) $redirectWith('error','Import batch not found.');
        $result=documents_quote_repair_imported_batch($batch);
        $batch[$action === 'quotation_csv_repair_preview' ? 'repair_preview' : 'repair_result']=$result;
        $batch['status']=$action === 'quotation_csv_repair_preview' ? (string)($batch['status'] ?? 'Validated') : ((string)($batch['status'] ?? 'Committed') . ' / Repaired');
        documents_quote_import_save_batch($batch);
        header('Location: admin-quotations.php?' . http_build_query(['tab'=>'bulk_create','batch'=>$batchId,'status'=>'success','message'=>($action === 'quotation_csv_repair_preview' ? 'Repair preview processed. ' : 'Repair processed. ') . 'Repaired '.$result['repaired'].', skipped '.$result['skipped'].', failed '.$result['failed'].'.'])); exit;
    }
    ...
    if ($action === 'quotation_browser_export_session') {
        $user = current_user();
        if (!is_array($user) || (string) ($user['role_name'] ?? '') !== 'admin') {
            http_response_code(403);
            header('Content-Type: application/json; charset=UTF-8');
        ...
        try {
            $payload = quotation_browser_export_create_token(quotation_bulk_normalize_selected_ids($_POST['selected_ids'] ?? []));
            header('Content-Type: application/json; charset=UTF-8');
            ...
            header('Content-Type: application/json; charset=UTF-8');
            echo json_encode(['ok' => false, 'message' => $e->getMessage(), 'limit' => QUOTATION_BROWSER_EXPORT_LIMIT]);
            exit;
        }
    }
...
<div class="list-table-wrap"><table><thead><tr><th>Source row</th><th>Import key</th><th>Customer</th><th>Template</th><th>Resolved system/model</th><th>DCR</th><th>Non-DCR</th><th>Total</th><th>Basic</th><th>GST</th><th>Tax mode</th><th>Subsidy</th><th>Transport</th><th>Discount</th><th>Final payable</th><th>Matched kit/items</th><th>Status</th><th>Messages</th><th>Result</th></tr></thead><tbody>
<?php foreach ((array)$selectedImportBatch['rows'] as $r): $in=is_array($r['input']??null)?$r['input']:[]; $v=is_array($r['validation']??null)?$r['validation']:[]; $preview=is_array($v['preview']??null)?$v['preview']:[]; $msgs=array_map(static fn($m): string => (string)($m['field']??'row').': '.(string)($m['message']??''), (array)($v['messages']??[])); $main=(float)($in['main_solar_kwp']??0); $non=(float)($in['complimentary_non_dcr_kwp']??$in['non_dcr_kwp']??0); ?>
<tr><td><?= (int)($r['source_row']??0) ?></td><td><?= htmlspecialchars((string)($in['import_key']??''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($in['customer_name']??''), ENT_QUOTES) ?><br><span class="quote-meta"><?= htmlspecialchars((string)($in['customer_mobile']??''), ENT_QUOTES) ?></span></td><td><?= htmlspecialchars((string)($in['template_set']??''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($preview['system_type']??$in['system_type']??''), ENT_QUOTES) ?><br><?= htmlspecialchars((string)($preview['model_number']??$in['selected_model_number']??''), ENT_QUOTES) ?><br><span class="quote-meta">Template: <?= htmlspecialchars((string)($preview['template_id']??''), ENT_QUOTES) ?> / <?= htmlspecialchars((string)($preview['template_display_name']??''), ENT_QUOTES) ?>; Segment <?= htmlspecialchars((string)($preview['segment']??''), ENT_QUOTES) ?>; Annexures <?= (int)($preview['annexure_count']??0) ?> (<?= htmlspecialchars(implode('|',(array)($preview['annexure_keys']??[])), ENT_QUOTES) ?>); Attachments <?= (int)($preview['attachment_count']??0) ?></span></td><td><?= htmlspecialchars((string)($in['main_solar_kwp']??''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($in['complimentary_non_dcr_kwp']??$in['non_dcr_kwp']??''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($main+$non), ENT_QUOTES) ?></td><td>₹<?= number_format((float)($preview['basic_amount']??0), 2) ?></td><td>₹<?= number_format((float)($preview['gst']??0), 2) ?></td><td><?= htmlspecialchars((string)($preview['tax_mode']??''), ENT_QUOTES) ?></td><td>₹<?= number_format((float)($preview['subsidy']??0), 2) ?></td><td>₹<?= number_format((float)($preview['transportation']??0), 2) ?></td><td>₹<?= number_format((float)($preview['discount']??0), 2) ?></td><td>₹<?= number_format((float)($preview['final_payable']??0), 2) ?></td><td><?= htmlspecialchars((string)($preview['kit']??''), ENT_QUOTES) ?><br><span class="quote-meta"><?= (int)($preview['item_count']??0) ?> item(s)</span></td><td><span class="status-pill"><?= htmlspecialchars((string)($r['status']??''), ENT_QUOTES) ?></span></td><td><?= htmlspecialchars(implode('; ', $msgs), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string)($r['commit_result']??''), ENT_QUOTES) ?></td></tr>
<?php endforeach; ?>
...
<?php if ($tab === 'bulk'): ?>
<?php $quotationPdfCapabilities = quotation_browser_pdf_capabilities(); $quotationPdfStatusText = quotation_bulk_pdf_engine_status_text($quotationPdfCapabilities); $quotationManagedInstallAvailable = quotation_browser_managed_install_available(); ?>
