...
<?php
$loanAmountSaved = trim((string)($editing['finance_inputs']['loan']['loan_amount'] ?? '')) !== '';
$loanInterestSaved = trim((string)($editing['finance_inputs']['loan']['interest_pct'] ?? '')) !== '';
$loanTenureSaved = trim((string)($editing['finance_inputs']['loan']['tenure_years'] ?? '')) !== '';
$loanMarginSaved = trim((string)($editing['finance_inputs']['loan']['margin_pct'] ?? '')) !== '';
?>
<div><label>Loan amount ₹</label><input type="number" step="0.01" name="loan_amount" data-has-saved-value="<?= ($editing['id'] !== '' && $loanAmountSaved) ? '1' : '0' ?>" value="<?= htmlspecialchars((string)($editing['finance_inputs']['loan']['loan_amount'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label><input type="checkbox" name="loan_enabled" <?= !empty($editing['finance_inputs']['loan']['enabled']) ? 'checked' : '' ?>> Loan enabled</label></div>
<div><label>Loan interest %</label><input type="number" step="0.01" name="loan_interest_pct" data-has-saved-value="<?= ($editing['id'] !== '' && $loanInterestSaved) ? '1' : '0' ?>" value="<?= htmlspecialchars((string)($editing['finance_inputs']['loan']['interest_pct'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Loan tenure years</label><input type="number" step="1" name="loan_tenure_years" data-has-saved-value="<?= ($editing['id'] !== '' && $loanTenureSaved) ? '1' : '0' ?>" value="<?= htmlspecialchars((string)($editing['finance_inputs']['loan']['tenure_years'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Margin money ₹</label><input type="number" step="0.01" name="loan_margin_pct" data-has-saved-value="<?= ($editing['id'] !== '' && $loanMarginSaved) ? '1' : '0' ?>" value="<?= htmlspecialchars((string)($editing['finance_inputs']['loan']['margin_pct'] ?? ''), ENT_QUOTES) ?>"><div class="muted"><a href="#" id="resetLoanDefaults">Reset to defaults</a></div></div>
