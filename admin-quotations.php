    ...
    if ($action === 'save_quote') {
        ...
        if (array_key_exists('panel_orientation_enabled', $_POST) || array_key_exists('panel_orientation_present', $_POST)) {
            $rawOrientation = [
                ...
                // Legacy group/note inputs are intentionally no longer editable.
                // Keep the schema keys for backward-compatible normalization while the visual
                // grid/object designer is the authoritative source for new quotation layouts.
                'shade_note' => '',
                'customer_note' => '',
                'groups' => [],
                'obstructions' => [],
...
?>
<div style="grid-column:1/-1" class="form-section-card panel-layout-section">
  <div class="panel-layout-section-head">
    <div>
      <div class="panel-layout-eyebrow">Customer-facing layout</div>
      <h3>Solar Panel Orientation / Layout</h3>
      <p class="muted">Create the proposed rooftop arrangement visually. This same layout is used in the customer quotation.</p>
    </div>
    <label class="panel-layout-toggle"><input type="hidden" name="panel_orientation_present" value="1"><input type="checkbox" name="panel_orientation_enabled" <?= !empty($orientation['enabled']) ? 'checked' : '' ?>><span>Show in quotation</span></label>
  </div>

  <div class="panel-layout-meta-grid">
    <div><label>Layout intent</label><select name="panel_orientation_row_layout"><?php foreach($layoutOptions as $value=>$label): ?><option value="<?= htmlspecialchars($value, ENT_QUOTES) ?>" <?= $orientation['row_layout']===$value?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select><div class="muted">Overall description only; individual panel orientation is controlled in the designer.</div></div>
    <div><label>Roof / area</label><select name="panel_orientation_site_area_type"><?php foreach($areaOptions as $value=>$label): ?><option value="<?= htmlspecialchars($value, ENT_QUOTES) ?>" <?= $orientation['site_area_type']===$value?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></div>
    <div><label>Area label / roof section name</label><input name="panel_orientation_site_area_label" value="<?= htmlspecialchars((string)$orientation['site_area_label'], ENT_QUOTES) ?>"></div>
    <div><label>Default facing direction</label><select name="panel_orientation_default_facing_direction"><?php foreach($directionOptions as $label): ?><option value="<?= htmlspecialchars($label, ENT_QUOTES) ?>" <?= $orientation['default_facing_direction']===$label?'selected':'' ?>><?= htmlspecialchars($label, ENT_QUOTES) ?></option><?php endforeach; ?></select></div>
    <div><label>Default tilt angle in degrees</label><input type="number" min="0" max="45" step="0.1" name="panel_orientation_default_tilt_deg" value="<?= htmlspecialchars((string)$orientation['default_tilt_deg'], ENT_QUOTES) ?>"></div>
  </div>

  <div class="panel-layout-designer-heading">
    <div>
      <span class="panel-layout-designer-kicker">Visual designer</span>
      <h3>Solar Panel Layout Designer</h3>
      <p class="muted">Add panels and rooftop markers, then drag them into position. Items snap to the grid and cannot overlap.</p>
    </div>
  ...
  <div class="panel-layout-designer" data-panel-layout-designer data-initial-layout="<?= htmlspecialchars(json_encode(['grid'=>$orientation['grid'] ?? ['columns'=>36,'rows'=>24,'cell_unit'=>'grid','editor_cell_px'=>18,'major_line_every'=>5,'customer_grid_visible'=>false], 'objects'=>$orientation['objects'] ?? []], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE), ENT_QUOTES) ?>">
    <div class="panel-layout-toolbar">
      <div><strong>Build the proposed roof layout</strong><div class="muted">Use Keep-out inside the designer for tanks, access zones or other areas where panels should not be placed.</div></div>
      <div class="panel-layout-actions"><button type="button" class="btn secondary" data-layout-reset>Reset layout</button><button type="button" class="btn secondary" data-layout-clear>Clear all</button></div>
    </div>

    <div class="panel-layout-controls">
      <div><label>Grid width</label><input type="number" min="8" max="100" step="1" data-layout-columns value="<?= (int)($orientation['grid']['columns'] ?? 36) ?>"></div>
      <div><label>Grid height</label><input type="number" min="8" max="100" step="1" data-layout-rows value="<?= (int)($orientation['grid']['rows'] ?? 24) ?>"></div>
      <div><label>Canvas zoom</label><div class="panel-layout-zoom"><button type="button" data-layout-zoom-out aria-label="Zoom out">−</button><span data-layout-zoom-label>100%</span><button type="button" data-layout-zoom-in aria-label="Zoom in">+</button><button type="button" data-layout-fit>Fit</button></div></div>
      <div class="panel-layout-control-help">Resize the grid only when the roof needs more or less working space. Panel dimensions remain standardized.</div>
    </div>

    <div class="panel-layout-main">
      <div class="panel-layout-scroll" data-layout-scroll><div class="panel-layout-grid" data-layout-grid tabindex="0" aria-label="Solar panel layout grid"></div></div>
      <aside class="panel-layout-palette">
        <div class="panel-layout-palette-title"><span>Add to layout</span><small>Click to place, then drag</small></div>
        <button type="button" class="panel-layout-add-btn" data-add-layout-item="panel-portrait"><span class="panel-layout-add-icon">▯</span><span><strong>Portrait panel</strong><small>Standard vertical module</small></span></button>
        <button type="button" class="panel-layout-add-btn" data-add-layout-item="panel-landscape"><span class="panel-layout-add-icon">▭</span><span><strong>Landscape panel</strong><small>Standard horizontal module</small></span></button>
        <button type="button" class="panel-layout-add-btn" data-add-layout-item="text"><span class="panel-layout-add-icon">T</span><span><strong>Text label</strong><small>Name a roof section</small></span></button>
        <button type="button" class="panel-layout-add-btn" data-add-layout-item="obstruction"><span class="panel-layout-add-icon">×</span><span><strong>Keep-out</strong><small>Tank, access or no-panel zone</small></span></button>
        <button type="button" class="panel-layout-add-btn" data-add-layout-item="arrow"><span class="panel-layout-add-icon">N</span><span><strong>North arrow</strong><small>Show roof direction</small></span></button>

        <div class="panel-layout-palette-divider"></div>
        <div class="panel-layout-selected-title">Selected item</div>
    ...
    <div class="panel-layout-designer-footer"><span>Tip:</span> Keep the drawing simple and readable—the customer quotation automatically crops the visual to the occupied roof area.</div>
  </div>
</div>
