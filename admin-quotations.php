...
<?php
    $savedUnitRate = (string)($editing['finance_inputs']['unit_rate_rs_per_kwh'] ?? '');
    $savedAnnualGeneration = (string)($editing['finance_inputs']['annual_generation_per_kw'] ?? '');
    $hasSavedUnitRate = trim($savedUnitRate) !== '';
    $hasSavedAnnualGeneration = trim($savedAnnualGeneration) !== '';
?>
<div><label>Unit rate (₹/kWh)</label><input type="number" step="0.01" name="unit_rate_rs_per_kwh" value="<?= htmlspecialchars($hasSavedUnitRate ? $savedUnitRate : (string)($segmentDefaults['unit_rate_rs_per_kwh'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Annual generation per kW</label><input type="number" step="0.01" name="annual_generation_per_kw" value="<?= htmlspecialchars($hasSavedAnnualGeneration ? $savedAnnualGeneration : (string)($quoteDefaults['global']['energy_defaults']['annual_generation_per_kw'] ?? ''), ENT_QUOTES) ?>"></div>
