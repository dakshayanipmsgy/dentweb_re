...
<table><thead><tr><th>Quote No</th><th>Name</th><th>Created By</th><th>Status</th><th>Amount</th><th>Updated</th><th>Share Link</th><th>Actions</th></tr></thead><tbody>
...
<td><?= !empty(($q['share']['enabled'] ?? false)) ? 'Enabled' : 'Disabled' ?><?php if (!empty(($q['share']['enabled'] ?? false)) && !empty(($q['share']['token'] ?? ''))): ?> · <a href="quote.php?t=<?= urlencode((string)$q['share']['token']) ?>" target="_blank">Open</a><?php endif; ?></td><td><a class="btn secondary" href="quotation-view.php?id=<?= urlencode((string)$q['id']) ?>">View</a> <?php if (documents_quote_can_edit($q, 'admin')): ?><a class="btn secondary" href="admin-quotations.php?edit=<?= urlencode((string)$q['id']) ?>">Edit</a><?php endif; ?></td>
</tr><?php endforeach; if ($allQuotes===[]): ?><tr><td colspan="8">No quotations yet.</td></tr><?php endif; ?></tbody></table>
</div>
</main></body></html>
