...
$financeInputs = is_array($editing['finance_inputs'] ?? null) ? $editing['finance_inputs'] : [];
$financeLoanInputs = is_array($financeInputs['loan'] ?? null) ? $financeInputs['loan'] : [];
$financeFieldOrDefault = static function (array $source, string $key, $default = '') {
...
$monthlyBillTouchedInitial = ((string) ($editing['id'] ?? '') !== '' && trim((string) ($financeInputs['monthly_bill_rs'] ?? '')) !== '') ? '1' : '0';
?>
<!doctype html>
...
<div><label>Monthly electricity bill (₹)</label><input type="number" step="0.01" name="monthly_bill_rs" value="<?= htmlspecialchars((string)($financeInputs['monthly_bill_rs'] ?? ''), ENT_QUOTES) ?>"><div class="muted">Suggested bill based on generation & tariff. You can change it. <a href="#" id="resetMonthlySuggestion">Reset suggestion</a></div></div>
<input type="hidden" name="monthly_bill_touched" id="monthlyBillTouched" value="<?= htmlspecialchars($monthlyBillTouchedInitial, ENT_QUOTES) ?>">
<div><label>Unit rate (₹/kWh)</label><input type="number" step="0.01" name="unit_rate_rs_per_kwh" value="<?= htmlspecialchars((string)$financeFieldOrDefault($financeInputs, 'unit_rate_rs_per_kwh', ($segmentDefaults['unit_rate_rs_per_kwh'] ?? '')), ENT_QUOTES) ?>"></div>
<div><label>Annual generation per kW</label><input type="number" step="0.01" name="annual_generation_per_kw" value="<?= htmlspecialchars((string)$financeFieldOrDefault($financeInputs, 'annual_generation_per_kw', ($quoteDefaults['global']['energy_defaults']['annual_generation_per_kw'] ?? '')), ENT_QUOTES) ?>"></div>
<div><label>Transportation ₹</label><input type="number" step="0.01" name="transportation_rs" value="<?= htmlspecialchars((string)($financeInputs['transportation_rs'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Discount (₹)</label><input type="number" step="0.01" min="0" name="discount_rs" value="<?= htmlspecialchars((string)($financeInputs['discount_rs'] ?? ($editing['discount_rs'] ?? '0')), ENT_QUOTES) ?>"></div>
<div><label>Discount note</label><input name="discount_note" value="<?= htmlspecialchars((string)($financeInputs['discount_note'] ?? ($editing['discount_note'] ?? '')), ENT_QUOTES) ?>" placeholder="Optional (e.g. Festival Offer)"></div>
<div><label>Subsidy ₹</label><input type="number" step="0.01" name="subsidy_expected_rs" value="<?= htmlspecialchars((string)($financeInputs['subsidy_expected_rs'] ?? ''), ENT_QUOTES) ?>"><div class="muted"><a href="#" id="resetSubsidyDefault">Reset to scheme default</a></div></div>
<div><label>Loan amount ₹</label><input type="number" step="0.01" name="loan_amount" value="<?= htmlspecialchars((string)($financeLoanInputs['loan_amount'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label><input type="checkbox" name="loan_enabled" <?= !empty($financeLoanInputs['enabled']) ? 'checked' : '' ?>> Loan enabled</label></div>
<div><label>Loan interest %</label><input type="number" step="0.01" name="loan_interest_pct" value="<?= htmlspecialchars((string)($financeLoanInputs['interest_pct'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Loan tenure years</label><input type="number" step="1" name="loan_tenure_years" value="<?= htmlspecialchars((string)($financeLoanInputs['tenure_years'] ?? ''), ENT_QUOTES) ?>"></div>
<div><label>Margin money ₹</label><input type="number" step="0.01" name="loan_margin_pct" value="<?= htmlspecialchars((string)($financeLoanInputs['margin_pct'] ?? ''), ENT_QUOTES) ?>"><div class="muted"><a href="#" id="resetLoanDefaults">Reset to defaults</a></div></div>
