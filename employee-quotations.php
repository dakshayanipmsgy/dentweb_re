    ...
    if ($action === 'save_quote') {
        ...
        for ($i=0; $i<$count; $i++) {
            $rawItems[] = [
                ...
                'gst_slab' => safe_text((string) (($quote['items'][$i]['gst_slab'] ?? '5'))),
                'basic_amount' => (float) (($quote['items'][$i]['basic_amount'] ?? 0)),
            ];
        }
        ...
        $systemPriceInclGst = (float) ($_POST['system_price_incl_gst'] ?? 0);
        $quote['input_total_gst_inclusive'] = max(0, $systemPriceInclGst);
        $quote['calc'] = documents_calc_pricing_from_items($quote['items'], $pricingMode, $taxType, $transportationRs, $subsidyExpectedRs, $quote['input_total_gst_inclusive']);
...
<div><label>System price incl GST (₹)</label><input type="number" step="0.01" name="system_price_incl_gst" value="<?= htmlspecialchars((string)($editing['input_total_gst_inclusive'] ?? 0), ENT_QUOTES) ?>"></div>
