...
$catalogItemsActive = documents_list_catalog_items(false);
$gstSchemesActive = documents_list_gst_schemes(false);
...
<script>document.addEventListener('click',function(e){if(e.target&&e.target.id==='addItemBtn'){const tb=document.querySelector('#itemsTable tbody');if(!tb)return;const tr=document.createElement('tr');const dH='<?= htmlspecialchars((string)($quoteDefaults['defaults']['hsn_solar'] ?? '8541'), ENT_QUOTES) ?>';const catalogOptions=<?= json_encode(array_map(static fn($cat)=>['id'=>$cat['id'],'name'=>$cat['name'],'description'=>$cat['description'],'hsn'=>$cat['hsn'],'unit'=>$cat['unit'],'default_gst_scheme_id'=>$cat['default_gst_scheme_id']],$catalogItemsActive), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;const schemeOptions=<?= json_encode(array_map(static fn($sch)=>['id'=>$sch['id'],'name'=>$sch['name']],$gstSchemesActive), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;const catalogHtml=['<option value="">Custom</option>'].concat(catalogOptions.map(c=>'<option value="'+c.id+'" data-name="'+(c.name||'')+'" data-description="'+(c.description||'')+'" data-hsn="'+(c.hsn||'')+'" data-unit="'+(c.unit||'')+'" data-scheme="'+(c.default_gst_scheme_id||'')+'">'+(c.name||'')+'</option>')).join('');const schemeHtml=['<option value="">-- None --</option>'].concat(schemeOptions.map(s=>'<option value="'+s.id+'">'+s.name+'</option>')).join('');tr.innerHTML='<td></td><td><select name="item_catalog_id[]" class="catalog-select">'+catalogHtml+'</select></td><td><input name="item_name[]"></td><td><input name="item_description[]"></td><td><input name="item_hsn[]" value="'+dH+'"></td><td><input type="number" step="0.01" name="item_qty[]" value="1"></td><td><input name="item_unit[]" value="set"></td><td><select name="item_gst_scheme_id[]">'+schemeHtml+'</select></td><td><input type="number" step="0.01" min="0" name="item_basic_amount[]" value="0"></td><td><button type="button" class="btn secondary rm-item">Remove</button></td>';tb.appendChild(tr);ren();}if(e.target&&e.target.classList.contains('rm-item')){e.target.closest('tr')?.remove();ren();}});document.addEventListener('change',function(e){if(!(e.target instanceof HTMLSelectElement)||!e.target.classList.contains('catalog-select'))return;const tr=e.target.closest('tr');if(!tr)return;const opt=e.target.selectedOptions[0];if(!opt)return;const name=tr.querySelector('input[name="item_name[]"]');const desc=tr.querySelector('input[name="item_description[]"]');const hsn=tr.querySelector('input[name="item_hsn[]"]');const unit=tr.querySelector('input[name="item_unit[]"]');const scheme=tr.querySelector('select[name="item_gst_scheme_id[]"]');if(name&&!name.value)name.value=opt.getAttribute('data-name')||'';if(desc&&!desc.value)desc.value=opt.getAttribute('data-description')||'';if(hsn&&!hsn.value)hsn.value=opt.getAttribute('data-hsn')||'';if(unit&&!unit.value)unit.value=opt.getAttribute('data-unit')||'set';if(scheme&&!scheme.value)scheme.value=opt.getAttribute('data-scheme')||'';});function ren(){document.querySelectorAll('#itemsTable tbody tr').forEach((tr,i)=>{const td=tr.querySelector('td');if(td)td.textContent=String(i+1);});}ren();</script>
