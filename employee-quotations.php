...
$redirect = static function (string $type, string $message): void {
    header('Location: employee-quotations.php?' . http_build_query(['status' => $type, 'message' => $message]));
    exit;
};
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (safe_text($_POST['action'] ?? '') === 'create_quote') {
        ...
        $number = documents_next_quote_number((string) ($template['segment'] ?? 'RES'));
        if (!$number['ok']) {
        ...
        $ann = [];
        foreach (['system_inclusions', 'payment_terms', 'warranty', 'system_type_explainer', 'transportation', 'terms_conditions'] as $key) {
        ...
        $quote = [
            ...
            'segment' => (string) ($template['segment'] ?? 'RES'),
            'created_by_type' => 'employee',
            'created_by_id' => (string) ($employee['id'] ?? ''),
            'created_by_name' => (string) ($employee['name'] ?? 'Employee'),
            'created_at' => $now,
            'updated_at' => $now,
            'party_type' => safe_text($_POST['party_type'] ?? 'customer'),
...
$quotes = array_values(array_filter($allQuotes, static fn(array $q): bool => (string) ($q['created_by_type'] ?? '') === 'employee' && (string) ($q['created_by_id'] ?? '') === (string) ($employee['id'] ?? '')));
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Employee Quotations</title>
...
<div class="card"><h1>My Quotations</h1><a class="btn" href="employee-dashboard.php">Back to Dashboard</a></div>
...
<div class="card"><h2>Create quotation</h2><form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars((string) ($_SESSION['csrf_token'] ?? ''), ENT_QUOTES) ?>"><input type="hidden" name="action" value="create_quote"><div class="grid">
...
<div><label>Mobile</label><input name="customer_mobile" required></div><div><label>Name</label><input name="customer_name"></div>
<div><label>System Type</label><select name="system_type"><option>Ongrid</option><option>Hybrid</option><option>Offgrid</option><option>Product</option></select></div><div><label>Capacity kWp</label><input name="capacity_kwp" required></div><div><label>Valid Until</label><input type="date" name="valid_until" value="<?= date('Y-m-d', strtotime('+15 days')) ?>"></div><div><label>Pricing Mode</label><select name="pricing_mode"><option value="solar_split_70_30">Solar split 70/30</option><option value="flat_5">Flat 5%</option></select></div><div><label>Total (GST inclusive)</label><input type="number" min="0" step="0.01" name="input_total_gst_inclusive" required></div><div><label>Place of Supply State</label><input name="place_of_supply_state" value="Jharkhand"></div><div><label>Tax Type (optional override)</label><select name="tax_type"><option value="">Auto</option><option value="CGST_SGST">CGST+SGST</option><option value="IGST">IGST</option></select></div>
<div><label>District</label><input name="district"></div><div><label>City</label><input name="city"></div><div><label>State</label><input name="state" value="Jharkhand"></div><div><label>PIN</label><input name="pin"></div><div><label>Project Summary</label><input name="project_summary_line"></div>
<div style="grid-column:1/-1"><label>Billing Address</label><textarea name="billing_address"></textarea></div><div style="grid-column:1/-1"><label>Site Address</label><textarea name="site_address"></textarea></div>
...
<div class="card"><h2>My quote list</h2><table><thead><tr><th>Quote No</th><th>Name</th><th>Status</th><th>Created</th><th>Action</th></tr></thead><tbody><?php foreach ($quotes as $q): ?><tr><td><?= htmlspecialchars((string) ($q['quote_no'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($q['customer_name'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($q['status'] ?? ''), ENT_QUOTES) ?></td><td><?= htmlspecialchars((string) ($q['created_at'] ?? ''), ENT_QUOTES) ?></td><td><a class="btn" href="quotation-view.php?id=<?= urlencode((string) ($q['id'] ?? '')) ?>">View</a></td></tr><?php endforeach; ?></tbody></table></div>
</main></body></html>
