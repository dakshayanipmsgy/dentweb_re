...
function documents_inventory_sort_transactions_chronological(array $transactions): array
{
    $rows = [];
    foreach ($transactions as $idx => $tx) {
        $rows[] = [
            'idx' => $idx,
            'tx' => array_merge(documents_inventory_transaction_defaults(), is_array($tx) ? $tx : []),
        ];
    }
    ...
    usort($rows, static function (array $a, array $b): int {
        $aTx = (array) ($a['tx'] ?? []);
        $bTx = (array) ($b['tx'] ?? []);
        $at = (string) ($aTx['created_at'] ?? '');
        $bt = (string) ($bTx['created_at'] ?? '');
        $cmp = strcmp($at, $bt);
        if ($cmp !== 0) {
            return $cmp;
        }
...
function documents_inventory_apply_transaction_to_stock(array $stockState, array $txRow, array $componentRow): array
{
    $componentId = (string) ($componentRow['id'] ?? '');
    $variantId = (string) ($txRow['variant_id'] ?? '');
    $entry = documents_inventory_component_stock($stockState, $componentId, $variantId);
    $isCuttable = !empty($componentRow['is_cuttable']);
    $txType = strtoupper((string) ($txRow['type'] ?? 'IN'));
    $txLocationId = trim((string) ($txRow['location_id'] ?? ''));
    $txConsumeLocationId = trim((string) ($txRow['consume_location_id'] ?? ''));
    $txId = trim((string) ($txRow['id'] ?? ''));
    $txCreatedAt = (string) ($txRow['created_at'] ?? date('c'));

    if ($txType === 'IN') {
        if ($isCuttable) {
            $pieceCount = max(0, (int) ($txRow['piece_count'] ?? 0));
            $pieceLength = max(0, (float) ($txRow['piece_length_ft'] ?? 0));
            if ($pieceCount <= 0 && max(0, (float) ($txRow['length_ft'] ?? 0)) > 0 && $pieceLength > 0) {
                $pieceCount = (int) floor(((float) ($txRow['length_ft'] ?? 0)) / $pieceLength);
            }
            ...
            for ($i = 0; $i < $pieceCount; $i++) {
                $lot = [
                    'lot_id' => ($txId !== '' ? $txId : 'TX') . '-LOT-' . str_pad((string) ($i + 1), 3, '0', STR_PAD_LEFT),
                    'received_at' => $txCreatedAt,
    ...
    if ($txType === 'OUT') {
        if ($isCuttable) {
            ...
            return [
                ...
                'tx' => [
                    'qty' => 0,
                    'unit' => 'ft',
                    'length_ft' => $requiredFt,
                    'piece_count' => 0,
                    'piece_length_ft' => 0,
                    'lots_created' => [],
                    'lot_consumption' => (array) ($consumed['lot_consumption'] ?? []),
                    'location_consumption' => [],
                    'location_id' => $txConsumeLocationId !== '' ? $txConsumeLocationId : 'mixed',
                    'consume_location_id' => $txConsumeLocationId,
                ],
            ];
        }
        ...
        return [
            ...
            'tx' => [
                ...
                'location_id' => (string) ($consumed['location_id'] ?? ''),
                'consume_location_id' => $txConsumeLocationId,
            ],
        ];
    }
...
function rebuild_stock_from_transactions(array $transactions): array
{
    $components = documents_inventory_components(true);
    $componentMap = [];
    foreach ($components as $component) {
    ...
    $sorted = documents_inventory_sort_transactions_chronological($transactions);
    $rebuiltStock = documents_inventory_stock_defaults();
    $computedById = [];

    foreach ($sorted as $row) {
        $tx = array_merge(documents_inventory_transaction_defaults(), (array) ($row['tx'] ?? []));
        $txId = (string) ($tx['id'] ?? '');
        $componentId = (string) ($tx['component_id'] ?? '');
        $component = $componentMap[$componentId] ?? null;
        if (!is_array($component)) {
            continue;
        }

        $applied = documents_inventory_apply_transaction_to_stock($rebuiltStock, $tx, $component);
        if (!($applied['ok'] ?? false)) {
            return [
                'ok' => false,
                'error' => (string) ($applied['error'] ?? 'Invalid transaction.'),
                'tx_id' => $txId,
            ];
        }
        ...
        if ($txId !== '') {
            $computedById[$txId] = (array) ($applied['tx'] ?? []);
        }
    }
    ...
    return [
        'ok' => true,
        'stock' => $rebuiltStock,
        'computed_by_tx_id' => $computedById,
    ];
}
