...
function documents_dispatch_advice_defaults(): array { return ['id'=>'','dispatch_advice_no'=>'','quotation_id'=>'','quotation_no'=>'','agreement_id'=>'','agreement_no'=>'','segment'=>'RES','customer_name'=>'','customer_mobile'=>'','delivery_address'=>'','planned_dispatch_date'=>'','items'=>[],'customer_note'=>'','disclaimer'=>'This advice lists items planned for dispatch. It is not proof of dispatch or delivery.','status'=>'draft','revision_no'=>1,'supersedes_id'=>'','public_share_enabled'=>false,'public_token'=>'','public_expires_at'=>'','share_audit'=>[],'acknowledged_at'=>'','created_at'=>'','updated_at'=>'']; }
function documents_normalize_dispatch_advice_items(array $items): array { $out=[]; foreach($items as $r){if(!is_array($r))continue;$x=['name'=>safe_text((string)($r['name']??'')),'description'=>safe_text((string)($r['description']??'')),'brand_model'=>safe_text((string)($r['brand_model']??'')),'qty'=>max(0,(float)($r['qty']??$r['required_qty']??0)),'unit'=>safe_text((string)($r['unit']??'Nos')),'remarks'=>safe_text((string)($r['remarks']??$r['planned_note']??''))]; if($x['name']!==''&&$x['qty']>0)$out[]=$x;} return $out; }
function documents_dispatch_advice_suggested_items(array $quote): array { $rows=[];$pack=documents_get_packing_list_for_quote((string)($quote['id']??''),false); foreach((array)($pack['required_items']??[]) as $r)$rows[]=['name'=>$r['component_name_snapshot']??'','description'=>$r['planned_note']??'','qty'=>$r['required_qty']??0,'unit'=>$r['unit']??'Nos','remarks'=>$r['remarks']??'']; if(!$rows)foreach((array)($quote['items']??[]) as $r)$rows[]=['name'=>$r['name']??$r['name_snapshot']??'','description'=>$r['description']??$r['description_snapshot']??'','qty'=>$r['qty']??0,'unit'=>$r['unit']??'Nos']; return documents_normalize_dispatch_advice_items($rows); }
...
function documents_latest_finalized_dispatch_advice(string $quoteId): ?array {foreach(documents_list_dispatch_advices() as $d)if((string)$d['quotation_id']===$quoteId&&in_array($d['status'],['finalized','shared','acknowledged'],true))return $d;return null;}
...
/* Verified customer delivery workflow (file-backed; no WhatsApp delivery claims). */
function documents_dispatch_catalogue_defaults(): array { return array_map(static fn($n)=>['id'=>strtolower(str_replace([' / ',' '],['_','_'],$n)),'name'=>$n,'description'=>'','brand_model'=>'','qty'=>1,'unit'=>'Nos','remarks'=>'','reusable'=>true], ['Solar Panels','Inverter','Batteries','Mounting Structure','Wire / Cables','Earthing','Lightning Arrestor']); }
function documents_dispatch_catalogue(): array { $s=json_load(documents_dispatch_advice_settings_path(),[]); return documents_normalize_dispatch_advice_items((array)($s['catalogue']??documents_dispatch_catalogue_defaults())); }
function documents_workflow_audit(string $entity,string $id,string $action,array $details=[]): void { require_once dirname(__DIR__,2).'/includes/audit_log.php'; $a=audit_current_actor(); log_audit_event($a['actor_type'],$a['actor_id'],$entity,$id,$action,$details); }
...
function documents_create_draft_challan_from_dispatch_advice(array $d): array { $existing=documents_challan_for_dispatch_advice((string)$d['id']); if($existing)return ['ok'=>true,'challan'=>$existing,'created'=>false]; $num=documents_generate_challan_number((string)$d['segment']); if(empty($num['ok']))return ['ok'=>false,'error'=>$num['error']??'Numbering failed']; $c=documents_challan_defaults();$c['id']='dc_'.date('YmdHis').'_'.bin2hex(random_bytes(3));$c['challan_no']=$num['challan_no'];$c['status']='Draft';$c['linked_quote_id']=$d['quotation_id'];$c['linked_quote_no']=$d['quotation_no'];$c['agreement_id']=$d['agreement_id'];$c['dispatch_advice_id']=$d['id'];$c['dispatch_advice_no']=$d['dispatch_advice_no'];$c['customer_snapshot']=array_merge(documents_customer_snapshot_defaults(),['name'=>$d['customer_name'],'mobile'=>$d['customer_mobile'],'address'=>$d['delivery_address']]);$c['delivery_address']=$d['delivery_address'];$c['delivery_date']=$d['planned_dispatch_date'];$c['items']=array_map(static fn($i)=>['name'=>$i['name'],'description'=>$i['description'],'brand_model'=>$i['brand_model'],'qty'=>$i['qty'],'unit'=>$i['unit'],'remarks'=>$i['remarks']],documents_normalize_dispatch_advice_items($d['items']));$c['created_at']=$c['updated_at']=date('c');$r=documents_save_challan($c);if(empty($r['ok']))return $r;documents_workflow_audit('challan',$c['id'],'draft_challan_created',['dispatch_advice_id'=>$d['id']]);return ['ok'=>true,'challan'=>$c,'created'=>true]; }
function documents_verify_dispatch_acceptance(array $d,bool $approved,string $adminNote=''): array { $d['acceptance']['admin_decision_at']=date('c');$d['acceptance']['admin_note']=safe_text($adminNote);$d['acceptance']['status']=$approved?'accepted_verified':'rejected';$d['status']=$approved?'accepted_verified':$d['status'];documents_save_dispatch_advice($d);documents_workflow_audit('dispatch_advice',(string)$d['id'],$approved?'admin_verification':'admin_rejection');if($approved)documents_create_draft_challan_from_dispatch_advice($d);return $d; }
function documents_delivered_challan_snapshots(string $quoteId): array { $out=[];foreach(documents_list_challans() as $c)if((string)($c['linked_quote_id']??'')===$quoteId&&!empty($c['materially_delivered_at']))$out[]=['challan_id'=>$c['id'],'challan_no'=>$c['challan_no'],'delivery_date'=>$c['delivery_date'],'items'=>documents_normalize_dispatch_advice_items((array)($c['items']??$c['lines']??[])),'reference_only'=>true];return $out; }
function documents_create_or_refresh_invoice_draft(array $quote): array { $invoice=null;foreach(glob(documents_invoices_dir().'/*.json')?:[] as $f){$x=json_load($f,[]);if(is_array($x)&&(string)($x['linked_quote_id']??'')===(string)$quote['id']){$invoice=array_merge(documents_invoice_defaults(),$x);break;}}if($invoice&&strtolower((string)$invoice['status'])!=='draft')return ['ok'=>false,'error'=>'Finalized invoice cannot be changed silently.','invoice'=>$invoice];if(!$invoice){$r=documents_create_invoice_from_quote($quote);if(empty($r['ok']))return $r;$invoice=documents_get_invoice($r['invoice_id']);$action='invoice_created';}else{$action='invoice_refreshed';}$invoice['delivery_details']=documents_delivered_challan_snapshots((string)$quote['id']);$invoice['delivery_details_notice']='Material delivery details — for reference only; not separately priced';$invoice['updated_at']=date('c');documents_save_invoice($invoice);documents_workflow_audit('invoice',(string)$invoice['id'],$action);return ['ok'=>true,'invoice'=>$invoice]; }
