...
<div class="responsive-table list-table-wrap"><table class="sticky-head"><thead><tr><th>Quotation</th><th>Customer</th><th>Status</th><th class="quote-amount">Amount</th><th>Updated</th><th>Actions</th></tr></thead><tbody>
...
<td><span class="status-badge"><?= htmlspecialchars(documents_status_label($q, 'admin'), ENT_QUOTES) ?></span></td>
...
$canReopenApprovedQuote = $isQuotationAdmin && documents_quote_can_reopen_approved_for_edit($q);
$changeRequest = is_array($q['customer_change_request'] ?? null) ? $q['customer_change_request'] : [];
?>
<div class="row-action-group list-actions">
...
<details class="more-actions"><summary class="btn secondary">More</summary><div class="more-actions__menu more-menu"><div class="secondary-actions">
...
<?php if ($canReopenApprovedQuote): ?><details style="margin-top:8px"><summary class="btn secondary" style="cursor:pointer">Edit Approved Quotation</summary><form method="post" class="js-quote-action" style="margin-top:8px;display:grid;gap:6px" data-confirm="Reopen this approved quotation for editing? It must be approved again before customer acceptance."><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES) ?>"><input type="hidden" name="action" value="reopen_approved_quote"><input type="hidden" name="quote_id" value="<?= htmlspecialchars($quoteId, ENT_QUOTES) ?>"><label>Reason for editing approved quotation</label><textarea name="approved_edit_reason" minlength="5" required placeholder="Enter a clear reason"></textarea><button class="btn" type="submit" onclick="return confirm('Reopen this approved quotation for editing? It must be approved again before customer acceptance.');">Reopen for Editing</button></form></details><?php endif; ?>
...
<?php endforeach; if ($allQuotes===[]): ?><tr><td colspan="6" class="empty-state"><strong>No quotations found.</strong><br>Create a new quotation to start the commercial lifecycle.</td></tr><?php endif; ?></tbody></table></div>
</div>
