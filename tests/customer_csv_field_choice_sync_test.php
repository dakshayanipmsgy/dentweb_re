...
field_choice_assert($blankPreview['rows'][0]['fields']['address']['choice']==='csv','CSV value automatically fills a saved blank');
field_choice_assert($blankPreview['rows'][0]['fields']['city']['choice']==='keep','blank CSV automatically preserves saved data');
$blankConfirmed=customer_bulk_mobile_sync_confirm($blankPreview,[]);
field_choice_assert(($blankConfirmed['rows'][0]['customer_changes']['address']['to']??'')==='Filled address' && !isset($blankConfirmed['rows'][0]['customer_changes']['city']),'automatic blank-field rules create a safe plan');
...
@unlink($dir.'-blank/customers.json'); @unlink($dir.'-blank/customers.lock'); @rmdir($dir.'-blank');
