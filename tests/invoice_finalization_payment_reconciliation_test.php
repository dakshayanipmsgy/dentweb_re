...
$bad=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>100,'quotation_id'=>'another_project','allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>1]]],[$invoice]); ok784(!$bad['ok'] && in_array('cross_project_allocation',$bad['errors'],true),'17 cross project rejected');
$bad=documents_receipt_allocations_normalize(['status'=>'final','quotation_id'=>'q784','amount_rs'=>100,'allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>60],['invoice_id'=>'inv_784_a','amount_rs'=>50,'authorized_override'=>true]]],[$invoice]); ok784(!$bad['ok'] && in_array('duplicate_invoice_allocation',$bad['errors'],true),'18 duplicate invoice allocation rejected');

$split=documents_receipt_allocations_normalize(['status'=>'final','quotation_id'=>'q784','amount_rs'=>350000,'allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>300000],['invoice_id'=>'inv_784_b','amount_rs'=>50000]]],[$invoice, array_merge($invoice,['id'=>'inv_784_b','customer_mobile'=>'9999999999'])]); ok784($split['ok'] && count($split['allocations'])===2,'19 split receipt valid');
