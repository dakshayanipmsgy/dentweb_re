...
$sameProject=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>100,'quotation_id'=>'q784','customer_mobile'=>'1111111111','allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>1]]],[$invoice]); ok784($sameProject['ok'] && count($sameProject['allocations'])===1,'17 same-project allocation survives a corrected customer mobile');
$otherProjectInvoice=array_merge($invoice,['linked_quote_id'=>'q785']);
$bad=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>100,'quotation_id'=>'q784','customer_mobile'=>'1111111111','allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>1]]],[$otherProjectInvoice]); ok784(!$bad['ok'] && in_array('cross_customer_allocation',$bad['errors'],true),'18 cross-project customer allocation rejected');
$bad=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>100,'allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>60],['invoice_id'=>'inv_784_a','amount_rs'=>50,'authorized_override'=>true]]],[$invoice]); ok784(!$bad['ok'] && in_array('allocation_exceeds_receipt',$bad['errors'],true),'19 allocation cannot exceed receipt');

$cancelledInvoice=array_merge($invoice,['status'=>'cancelled']);
$historical=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>100,'quotation_id'=>'q784','allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>100]]],[$cancelledInvoice]); ok784($historical['ok'] && count($historical['allocations'])===1,'20 historical cancelled-invoice allocation remains valid');

$split=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>350000,'allocations'=>[['invoice_id'=>'inv_784_a','amount_rs'=>300000],['invoice_id'=>'inv_784_b','amount_rs'=>50000]]],[$invoice, array_merge($invoice,['id'=>'inv_784_b','customer_mobile'=>'9999999999'])]); ok784($split['ok'] && count($split['allocations'])===2,'21 split receipt valid');
$amb=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>10,'quotation_id'=>'q784'],[$invoice,array_merge($invoice,['id'=>'inv_784_b'])]); ok784($amb['allocations']===[],'22 ambiguous quote receipt unallocated');
$one=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>10,'quotation_id'=>'q784'],[$invoice]); $two=documents_receipt_allocations_normalize(['status'=>'final','amount_rs'=>10,'quotation_id'=>'q784'],[$invoice]); ok784($one==$two && count($one['allocations'])===1,'23 unambiguous migration idempotent');

$f=documents_invoice_finalize($invoice,['id'=>'u1','name'=>'Admin']); ok784($f['ok'],'24 draft finalizes without full payment'); $fin=$f['invoice']; ok784(documents_invoice_is_finalized($fin),'25 finalized document status separate'); ok784(documents_invoice_payment_status($fin)==='partially_paid','26 payment status remains partial'); ok784(!empty($fin['finalized_snapshot']['pricing']) && !empty($fin['finalized_at']),'27 freezes pricing and actor timestamp');
$rev=documents_invoice_start_revision($fin,['id'=>'u1','name'=>'Admin'],'Correction'); ok784($rev['ok'] && $rev['invoice']['revision_no']===2 && $rev['invoice']['status']==='draft','28 revision reason creates draft and increments'); ok784(($rev['invoice']['revisions'][0]['status']??'')==='finalized','29 previous finalized snapshot preserved');
ok784(!documents_invoice_start_revision($fin,['id'=>'u1'],'')['ok'],'30 revision requires reason'); ok784(!documents_invoice_cancel($fin,['id'=>'u1'],'')['ok'],'31 cancel requires reason'); $can=documents_invoice_cancel($fin,['id'=>'u1'],'Customer requested cancellation'); ok784($can['ok'] && documents_invoice_is_cancelled($can['invoice']),'32 cancellation preserves history status');
