...
file_put_contents($employees,json_encode(['employees'=>[
 ['id'=>'legacy-a','name'=>'Employee A','login_id'=>'employee.a@example.test','phone'=>'9876543210','designation'=>'Engineer','status'=>'active','password_hash'=>$hash],
...
cw_assert($db->query("SELECT phone FROM employee_profiles WHERE user_id=".(int)$user['id'])->fetchColumn()==='9876543210','phone migration failed');
cw_assert((int)$db->query('SELECT COUNT(*) FROM schema_migrations WHERE version=91701')->fetchColumn()===1,'schema version missing');
foreach(['task_messages','task_events','task_attachments','task_occurrences'] as $table)cw_assert((int)$db->query("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=".$db->quote($table))->fetchColumn()===1,$table.' missing');
...
$missing=$saved;unset($missing['version']);try{$repo->saveLegacyShape($missing);throw new RuntimeException('missing version accepted');}catch(RuntimeException $e){cw_assert(str_contains($e->getMessage(),'version is required'),'wrong missing-version error');}
...
cw_assert(($dry['backup_integrity']??'')==='ok'&&($dry['restore_rehearsal']??'')==='ok','backup validation evidence missing');
