...
try {
    ...
    public_pay_assert(str_contains($html, 'upi://pay?') && str_contains($html, 'data-copy'), 'public page must retain the UPI action and copy controls');
    ...
    foreach (['paid', 'cancelled', 'archived'] as $status) {
        ...
        public_pay_assert(!str_contains($inactive, 'upi://pay?') && str_contains($inactive, 'Online payment is not available'), 'inactive request must expose no active Pay button');
    }
