...
$assert(str_contains($js, 'zipSync(files') && str_contains($js, 'showDownload(new Blob([files'), 'one PDF and multi ZIP handling');
...
$assert(str_contains($js, 'asset_missing') && str_contains($js, 'browser_feature_unsupported'), 'distinct browser export error handling');
$assert(str_contains($admin, 'Download using browser') && str_contains($admin, 'quotationBrowserAssetStatus'), 'forced browser-export UI and status');
...
$assetStatus = quotation_browser_export_asset_status();
$assert(($assetStatus['ok'] ?? false) === true, 'browser export assets pass integrity verification');
