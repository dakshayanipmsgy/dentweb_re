const CACHE_NAME='dakshayani-pwa-v4-work-static';
...
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE_NAME).then(cache=>cache.addAll([...SAFE_URLS])).catch(()=>undefined)));
...
self.addEventListener('push',event=>{
  ...
  event.waitUntil(Promise.all([self.registration.showNotification(title,{body,icon:new URL('assets/icons/employee/icon-192.png',BASE).href,badge:new URL('assets/icons/employee/icon-192.png',BASE).href,tag:id?`work-notification-${id}`:'work-notification',renotify:false,data:{notificationId:id}}),unread&&self.registration.setAppBadge?self.registration.setAppBadge(unread):Promise.resolve()]));
});
