...
The generator uses Asia/Kolkata dates and due times, excludes personal, completed, cancelled, archived, unofficial, and inactive-user work, and creates employee due-today/overdue/unacknowledged plus admin priority-overdue/unacknowledged/long-blocked reminders. Overdue keys include type, task, series, occurrence, recipient, the canonical due instant in UTC, and a task-relative cadence window. For example: `reminder:overdue_employee:task:42:series::occurrence:1:user:7:due:20260801T062900Z:window:0`. Concurrent inserts use the unique key in short transactions; notification and recipient status are committed or rolled back together.
...
Event wall times are interpreted explicitly as Asia/Kolkata and converted to `Y-m-dTH:i:sZ` before comparison with the once-persisted UTC rollout cutoff. Before-cutoff events receive one durable `skipped/pre_rollout` checkpoint; exactly-at and later events project. Retries cannot move the cutoff or reverse a checkpoint. Operational reminders remain based on current Kolkata business time and are unaffected by event rollout skips. Due-today begins at the configured hour and stops at the due instant; overdue begins only after it, so one run cannot create both. An absent due time means 23:59. Each overdue cadence is anchored to that task's current canonical due instant: `window = floor((now - due_at) / cadence_seconds)`. One second overdue is window 0; exactly one cadence is window 1; one second before the next cadence remains in the preceding window; and one second after it is in the next window. Employee and admin windows use their independently configured hours. The UTC due identity changes when either date or time changes, starting a fresh deduplication cycle without weakening unchanged-schedule retries. Invalid schedules are safely ineligible. Unacknowledged cadence begins at current assignment. A keep-blocked admin response updates task activity, so the blocked threshold restarts; resolution/closure/archive removes eligibility.
