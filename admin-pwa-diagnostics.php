...
$checks = [
    'HTTPS is enabled' => $scheme === 'https',
    'Manifest file is available' => $exists('manifest.webmanifest'),
    'Service worker file is available' => $exists('service-worker.js'),
    'Offline fallback is available' => $exists('offline.html'),
    'Required SVG icons are available' => count(array_filter($requiredIcons, $exists)) === count($requiredIcons),
];
?><!doctype html>
<html lang="en">
<head>
  ...
  <style>.diag{max-width:980px;margin:2rem auto;padding:1rem}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:1rem}.card{background:#fff;border:1px solid #e2e8f0;border-radius:18px;padding:1rem;box-shadow:0 12px 30px rgba(15,23,42,.06)}.ok{color:#047857;font-weight:800}.bad{color:#b91c1c;font-weight:800}.mono{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;word-break:break-all}.list{margin:.5rem 0 0;padding-left:1.2rem;line-height:1.7}.top{display:flex;justify-content:space-between;gap:1rem;align-items:center;flex-wrap:wrap}</style>
</head>
<body class="admin-shell"><main class="diag">
  <div class="top"><div><p class="admin-muted">Admin setup/testing only</p><h1>PWA Diagnostics</h1></div><a class="btn btn-ghost" href="<?= htmlspecialchars($asset('admin-dashboard.php'), ENT_QUOTES) ?>">Back to dashboard</a></div>
  ...
  <section class="card" style="margin-top:1rem"><h2>Install readiness checklist</h2><ul class="list"><?php foreach ($checks as $label => $pass): ?><li><span class="<?= $pass ? 'ok' : 'bad' ?>"><?= $pass ? 'Pass' : 'Needs attention' ?></span> — <?= htmlspecialchars($label, ENT_QUOTES) ?></li><?php endforeach; ?><li>Use Chrome DevTools Application panel to verify only safe static assets are cached.</li><li>Test from both domain root and a cPanel subdirectory before inviting users.</li></ul></section>
</main></body></html>
