...
if (!empty($sourceRepair['ok'])) {
    ...
    foreach (['customer_acceptance_mask_mobile', 'documents_challan_share_message', 'load_company_profile', 'documents_mark_challan_dispatched'] as $requiredHelper) {
        if (!function_exists($requiredHelper)) {
            error_log('Challan view dependency unavailable: ' . $requiredHelper);
            http_response_code(500);
            echo 'Unable to load challan details right now. Please try again or contact support.';
            exit;
        }
    }
    ...
    if (!$customerView && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'save_operational' || $action === 'mark_dispatched') {
            ...
            if ($challan['driver_mobile'] !== '' && !preg_match('/^[0-9+()\-\s]{7,20}$/', $challan['driver_mobile'])) {
                $go('error', 'Enter a valid driver mobile number or leave it blank.');
            }
            $challan['dispatch_date']=$challan['delivery_date'];
    ...
    $registeredMobileMask = customer_acceptance_mask_mobile((string)($challan['customer_snapshot']['mobile'] ?? $challan['customer_mobile'] ?? ''));
    ...
    <?php if ($customerView): ?><?= document_signature_render($challan, 'Authorised Signatory') ?><?php else: ?><?= document_signature_admin_controls($challan, 'challan', 'challan-view.php?id=' . urlencode((string)$challan['id']), strtolower((string)($challan['status'] ?? 'draft')) === 'draft' || strtolower((string)($challan['workflow_status'] ?? '')) === 'created') ?><?php endif; ?>
...
<?php if ($customerView): ?><?= document_signature_render($challan, 'Authorised Signatory') ?><?php else: ?><?= document_signature_admin_controls($challan, 'challan', 'challan-view.php?id=' . urlencode((string)$challan['id']), $editable) ?><?php endif; ?>
